...
GROUND TRUTH SOURCES
  ...
  - migrations + core-table row counts: the project database. Two paths:
      1. KERNEL_DB_DSN set -> query directly via psycopg2 (preferred in CI/worker)
      2. otherwise -> emit the exact SQL to run via the project's DB connector
         and mark those facts UNVERIFIED (loud), so a human/agent supplies them.

PROJECT CONFIGURATION (the kernel carries no product schema)
  KERNEL_DB_DSN           - DSN for the direct-query path.
  KERNEL_CORE_TABLES      - comma-separated table names whose row counts are
                            load-bearing for "has the pipeline run?". Unset
                            means the row-count question is NOT ASKED; it never
                            means "row counts verified".
  KERNEL_MIGRATIONS_TABLE - schema-qualified migrations table (e.g.
                            supabase_migrations.schema_migrations). Unset means
                            no migration facts are gathered — likewise never a
...
# project-specific by nature, so they are named in the environment rather than
# baked into the kernel. Every entry is validated to be a plain identifier
# before it can reach the row-count SQL below (or the connector display
# string). The check is an explicit raise, not an `assert`: these values are
# now operator input, and `python -O` must not be able to disable the guard.
CORE_TABLES = [t.strip() for t in os.getenv("KERNEL_CORE_TABLES", "").split(",")
               if t.strip()]
_BAD_TABLES = [t for t in CORE_TABLES if not re.fullmatch(r"[a-z_][a-z0-9_]*", t)]
if _BAD_TABLES:
    # Exit 3 = usage/environment error, per this script's documented codes.
    print("session_reconcile: KERNEL_CORE_TABLES entries must be plain SQL "
          f"identifiers ([a-z_][a-z0-9_]*); rejected: {_BAD_TABLES}",
          file=sys.stderr)
    raise SystemExit(3)

# Schema-qualified migrations table, e.g. "supabase_migrations.schema_migrations".
# Composed via psycopg2.sql.Identifier below — never interpolated into executed
# SQL — and validated here so a malformed value fails loud at import.
MIGRATIONS_TABLE = os.getenv("KERNEL_MIGRATIONS_TABLE", "").strip()
if MIGRATIONS_TABLE and not re.fullmatch(
        r"[A-Za-z_][A-Za-z0-9_]*(\.[A-Za-z_][A-Za-z0-9_]*)?", MIGRATIONS_TABLE):
    print("session_reconcile: KERNEL_MIGRATIONS_TABLE must be `table` or "
          f"`schema.table` with plain identifiers; rejected: {MIGRATIONS_TABLE!r}",
          file=sys.stderr)
    raise SystemExit(3)
...
def gather_db(dsn):
    ...
    the exact SQL to run via the project's DB connector (loud, not silent)."""
    if not dsn:
        # Display-only strings (never executed here) so a human/agent can run
        # them through whatever connector the project uses. An unconfigured
        # table list yields an explicit "nothing registered" string — the
        # absence is stated, never rendered as an empty-but-plausible query.
        return {
            "verified": False,
            "reason": "KERNEL_DB_DSN not set",
            "run_via_connector": {
                "migrations": (
                    f"select version, name from {MIGRATIONS_TABLE} order by version;"
                    if MIGRATIONS_TABLE
                    else "(KERNEL_MIGRATIONS_TABLE unset — no migrations query registered)"
                ),
                "row_counts": (
                    "select " + ", ".join(
                        f"(select count(*) from {t}) as {t}" for t in CORE_TABLES) + ";"
                    if CORE_TABLES
                    else "(KERNEL_CORE_TABLES unset — no row-count query registered)"
                ),
            },
            "migrations": None,
            "row_counts": None,
    ...
    try:
        with psycopg2.connect(dsn) as conn:
            with conn.cursor() as cur:
                if MIGRATIONS_TABLE:
                    # Compose the (possibly schema-qualified) identifier via
                    # psycopg2.sql — never f-string a table name into executed
                    # SQL, even a validated one.
                    parts = MIGRATIONS_TABLE.split(".")
                    cur.execute(sql.SQL("select version from {} order by version").format(
...
def main():
    ...
    ap.add_argument("--dsn", default=os.getenv("KERNEL_DB_DSN"))
    ...
    if material:
        ...
        return 2
    if unverified:
        print("\nRESULT: some critical facts UNVERIFIED — verify via the "
              "project's DB connector (SQL above) before trusting STATE.md. "
              "(exit 2)")
        return 2
