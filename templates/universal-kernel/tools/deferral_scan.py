...
Greppable summary: the project's Recording rule: every "for now" / "check
...
# The list is deliberately WIDE (a directory that does not exist is skipped for
# free): under-scanning is a silent-deferral hole, over-scanning costs nothing.
# A project adds its own trees here — it never removes one to quiet a finding.
SCAN_DIRS = ["api", "worker", "ai", "tools", "web", "tests", ".github",
             "supabase", "src", "app", "lib", "libs", "pkg", "cmd", "scripts",
             "services", "packages", "migrations", "db", "infra"]
EXTRA_FILES = ["tools/validate"]  # bash, no extension
# ".claude": transient parallel-agent worktrees (gitignored scratch clones)
# — the scan's subject is
# the repository's own tree, never embedded scratch copies (PR #51).
# A project extends this set; it never shrinks it to quiet a finding.
