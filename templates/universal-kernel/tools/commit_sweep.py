...
# Code trees and migration trees are matched by PREFIX, so the tuples are wide
# by design: a prefix that no repo uses simply never matches. A project adds
# its own prefixes; it never removes one to quiet a finding.
CODE_DIRS = ("worker/", "ai/", "api/", "tools/", "src/", "app/", "lib/",
             "libs/", "pkg/", "cmd/", "scripts/", "services/", "packages/")
TEST_DIR = "tests/"
MIGRATION_DIRS = ("supabase/migrations/", "migrations/", "db/migrations/",
...
def check_migration_without_test(commits: list[str], findings: Findings) -> None:
    for sha in commits:
        files = _commit_files(sha)
        migrations = [f for f in files if f.startswith(MIGRATION_DIRS) and f.endswith(".sql")]
        if not migrations:
            continue
        test_touched = [f for f in files if f.startswith(TEST_DIR)]
        if not test_touched:
