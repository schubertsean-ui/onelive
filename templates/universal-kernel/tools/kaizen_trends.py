...
SUMMARY: the measurement layer the Kaizen model requires — everything trends
toward zero/perfect, and the trend itself must be COMPUTED, not asserted.
Parses docs/metrics/KAIZEN_LEDGER.md and emits a machine-stamped trend
report:
...
def build_report(ledger_text: str) -> tuple[str, list[str]]:
    ...
    lines.append("note: class counts rely on the kebab-token convention documented in docs/KAIZEN.md — ledger rows written before a project adopted it may UNDERCOUNT; treat pre-convention numbers as floors, never as history")
