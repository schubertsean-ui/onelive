...
def _row_fields(node: _Node, *, base_url: str, page_html: str = "",
    ...
    if occs_here:
        ...
        if not when or ("T" in str(when) and str(when)[-1].isdigit() and "-" not in str(when)[10:]):
            when = stated
    ...
    return {
        ...
        "card_text": card_text,
        "when_occs": occs_here,
        "place_text": _ws(" ".join(place_parts)) or place_from_card_text(card_blob or card_text)[0],
