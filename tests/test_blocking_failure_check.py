...
@pytest.mark.parametrize("line,expected", [
    ...
    # Substring matching catches every SPELLING without enumerating them.
    # #73 r13 replaced it with exact matching and silently un-rejected these
    # two; reverted at r14, pinned here so it cannot happen again quietly.
    ("python -m pytest -q --ignore-glob='tests/*'", False),
    ("python -m pytest -q -mperf", False),
    ("python -m pytest -q -knot_slow", False),
    # #73 r14 (evaluator): --collect-only runs NO tests, --lf a cached subset.
    ("python -m pytest --collect-only", False),
    ("python -m pytest -q --lf", False),
    ("python -m pytest -q --last-failed", False),
    ("python -m pytest -q --stepwise", False),
])
