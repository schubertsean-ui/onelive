...
| `tools/provenance_lint.py` | Overstated-provenance class fix (PR #18 r1 + PR #50 r1, hardened r3–r5): (a) ARTIFACT RULE — research/strategy docs declaring an unread/unreachable primary source (paragraph-level detection) must self-identify as scout/pre-review in their first heading (no overstrong tokens) and cite an R-### tag BOUND to a docs/RECORD.md row naming the file; (b) SURFACE RULE — changelog/RECORD/TODOS/STATE lines mentioning such artifacts fail unconditionally on overstrong phrasing and otherwise need an honest-frame marker outside the filename or a bound tag | 0 clean / 1 findings |
