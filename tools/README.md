...
| `tools/value_ledger.py` | The Owned Agent's client value ledger ENGINE (founder-directed 2026-08-04 — the agent product's clients, not this repo's org): `log` appends every task the agent performs for a business (hours saved + $ value, estimate basis required) to that CLIENT's shared Excel workbook (`--path`; owner-editable rate in its Config sheet) and regenerates the "Weekly ROI" sheet + deterministic CSV audit mirror; `report --as-of` prints the weekly "you saved $" summary for the client contact; `verify` refuses when workbook/weekly/mirror disagree; committed default is the labeled DEMO workbook (`docs/strategy/examples/`); deterministic (dates caller-supplied); needs openpyxl (`tools/requirements.txt`) | 0 ok / 1 refusal-defect (named) / 2 usage |
