...
| `tools/identity_capture_scan.py` | Counts the per-listing identity carriers (ICS `UID`, schema.org `Event.url`, `@id`/`identifier`, an `<a href>` inside a repeated listing container) present in every HTML/ICS fixture the repo carries, including the ones written inline in `tests/test_*.py` — the re-runnable proof behind Session Contract #57's inventory table and R-103 | 0 always; the table is the output (it asserts nothing) |
