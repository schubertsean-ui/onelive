...
| `tests/test_social_carousel.py` | 50 | `social/carousel/` engine: trust selection (confirmed/likely only, unknown states loud), publish-gate custody (AI-cannot-approve, hash-bound approvals, current-state re-check), autonomy record fail-closed L0/L1/L2, agent_loop→publish_gate import guard, bandit determinism/learning/decay, volume tiering, GEO bundle | none (hermetic, no network) |
