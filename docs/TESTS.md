...
| `tests/test_carousel_launch.py` | 5 | V1 launch versions (founder-directed): launch assignments valid/complete; each scenario's launch deck reproduces the founder-seen hook through the real engine; fresh validated copies; corrupt table fails loud; bandit warm-start favors launch levels without locking (prior, not gate) | none (hermetic) |
