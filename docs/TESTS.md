...
| `tests/test_construction_gate.py` | 25 | Construction Loop Stage 3 gate: deliberate [S3:token] contract citations (bare mentions never pass); path AND content trigger matching; index self-protection (deleted token / narrowed triggers fail closed, bootstrap explicit); duplicate tokens fail closed; stale base-history citations never pass (real temp git repo); unresolvable diff base fails closed; unreadable/empty index fails closed; real-index coverage; base-ref FRESHNESS proof, time-free (oid comparison against the remote tip is the proof; a fetch that succeeds while leaving the ref behind FAILS; an unreachable remote accepts ONLY a two-parent HEAD whose first parent equals the base ref, so a local merge commit and an ordinary commit both fail closed) plus the SHIPPED probes against real git in two temp repos (explicit-refspec fetch really moves the remote-tracking ref; end-to-end convergence; parent counting; unreachable remote returns None) | none (hermetic, temp git repos) |
...
| `tests/test_adversarial_review.py` | 22 | Evaluator gate v1+v2: strict verdict parse (ambiguity = hard fail), key-absent skip vs --require, exit-code convention; v2 panel — po determinism + seed-varying, --panel requires --po-seed, ANY-lens-red = red, absent Gemini seat printed EXPLICITLY, both seats run two lenses when keyed, unparseable lens = hard fail, prompt encodes the ratified escape hatch + CLASS mandate, env model resolver fails closed on empty/Claude ids | none (hermetic, transport monkeypatched) |
| `tests/test_reviewer_scorecard.py` | 6 | M9 reviewer scorecard: mechanical ledger arc parsing (round-1 recall, sibling-misses, novelty), true round-1 semantics (an arc with no r1 row is an explicit unmeasurable), exact 9-part ledger schema enforced (an extra raw pipe RAISES), uncounted class mentions excluded, no-classed-findings guarded (no division error), the real shipped ledger parses clean | none (hermetic) |
