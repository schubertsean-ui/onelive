...
| `tests/test_construction_gate.py` | 13 | Construction Loop Stage 3 gate: deliberate [S3:token] contract citations (bare mentions never pass); path AND content trigger matching; index self-protection (deleted token / narrowed triggers fail closed, bootstrap explicit); duplicate tokens fail closed; stale base-history citations never pass (real temp git repo); unresolvable diff base fails closed; unreadable/empty index fails closed; real-index coverage | none (hermetic, temp git repos) |
