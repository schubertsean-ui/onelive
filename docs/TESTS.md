...
| `tests/test_adversarial_review.py` | 25 | Evaluator gate v1+v2: strict verdict parse (ambiguity = hard fail), key-absent skip vs --require, exit-code convention; v2 panel — po determinism + seed-varying, --panel requires --po-seed, ANY-lens-red = red, absent Gemini seat printed EXPLICITLY, both seats run two lenses when keyed, unparseable lens = hard fail, prompt encodes the ratified escape hatch + CLASS mandate, env model resolver fails closed on empty/Claude ids, the panel prints its own po seed and derived provocations | none (hermetic, transport monkeypatched) |
| `tests/test_reviewer_scorecard.py` | 8 | M9 reviewer scorecard: mechanical ledger arc parsing (round-1 recall, sibling-misses, novelty), true round-1 semantics (an arc with no r1 row is an explicit unmeasurable), exact 9-part ledger schema enforced (an extra raw pipe RAISES), uncounted class mentions excluded, no-classed-findings guarded (no division error), the real shipped ledger parses clean AND agrees with known merge facts (merged arcs report their true rounds-to-green, so a forgotten close row fails here), an unclosed arc is named rather than guessed | none (hermetic) |
