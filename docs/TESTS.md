...
| `tests/test_construction_gate.py` | 19 | Construction Loop Stage 3 gate: deliberate [S3:token] contract citations (bare mentions never pass); path AND content trigger matching; index self-protection (deleted token / narrowed triggers fail closed, bootstrap explicit); duplicate tokens fail closed; stale base-history citations never pass (real temp git repo); unresolvable diff base fails closed; unreadable/empty index fails closed; real-index coverage; base-ref FRESHNESS proof (fetch-succeeds-here and recent-recorded-fetch both accepted; stale record, no record at all, and an unresolvable ref inside the window all fail closed) | none (hermetic, temp git repos) |
...
| `tests/test_reviewer_scorecard.py` | 3 | M9 reviewer scorecard: mechanical ledger arc parsing (round-1 recall, sibling-misses, novelty), no-classed-findings guarded (no division error), the real shipped ledger parses clean | none (hermetic) |
