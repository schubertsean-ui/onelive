...
| `tests/test_adversarial_review.py` | 30 | Evaluator gate v1+v2: strict verdict parse (ambiguity = hard fail), key-absent skip vs --require, exit-code convention; v2 panel — po determinism + seed-varying, --panel requires --po-seed, ANY-lens-red = red, absent Gemini seat printed EXPLICITLY, both seats run two lenses when keyed, unparseable lens = hard fail, prompt encodes the ratified escape hatch + CLASS mandate, env model resolver fails closed on empty/Claude ids, the panel prints its own po seed and derived provocations; the PR-owned workflow supplies NEITHER seat's model (absence invariant — the subject must not choose an input to the review judging it), a non-blessed override id raises against the base-owned allowlist, the shipped default is itself allowlisted, and every floating alias in that allowlist is bound to an OPEN RECORD row | none (hermetic, transport monkeypatched) |
