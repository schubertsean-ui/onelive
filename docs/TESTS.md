...
| `tests/test_construction_gate.py` | 23 | Construction Loop Stage 3 gate: deliberate [S3:token] contract citations (bare mentions never pass); path AND content trigger matching; index self-protection (deleted token / narrowed triggers fail closed, bootstrap explicit); duplicate tokens fail closed; stale base-history citations never pass (real temp git repo); unresolvable diff base fails closed; unreadable/empty index fails closed; real-index coverage; base-ref FRESHNESS proof (oid comparison against the remote tip is the proof; a fetch that succeeds while leaving the ref behind FAILS; unreachable remote falls back to a REF-SCOPED write record, never a repo-wide fetch record; stale record, no record, unresolvable ref and malformed base ref all fail closed) | none (hermetic, temp git repos) |
