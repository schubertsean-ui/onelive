...
| R-013 | 2026-07-15 | Extraction remains BLOCKED (router flag False; provider consults it at its entry point) — the 1% bar is ratified (R-006) but the golden-set gate that PROVES it does not exist: no golden data, no live-exam runner, no CI wiring | docs/KAIZEN.md §M7 — no extraction change ships without exam evidence at a valid sample size | Step 6 build (next focused session, contract-first, evaluator mandatory): ≥40-example golden set (~320 facts ≥ the 1% sample floor) + live-exam runner + blocking CI job; flag flips in the commit that attaches a PASSING result for the starting model | OPEN — gate SHIPPED and rate bar MET, floor evidence pending: the 77-example/322-fact golden set + exam runner + release-blocking CI exist, and routed claude-opus-4-8 passed the RATE bar on cycle 11 (hallucination 0.0068, recall 0.9702, zero injections — run 29424147665, artifact 8346621357) — but that run asserted 295 facts vs the ≥300 asserted-fact floor the 1% claim is powered by (fail-open floor mismatch caught by evaluator r6 BEFORE the flip shipped; gate corrected, set widened to 322 expected facts). Flag flips only citing a run that passes the corrected gate |
...
| R-014 | 2026-07-15 | `extraction-eval.yml` ran PR-controlled code with the repo-wide `ANTHROPIC_API_KEY` on `pull_request` (r5 nit, escalated to BLOCKER by evaluator r7: the release gate itself was a credential-exfiltration surface) | §11.4 AI-supply-chain / secret-exposure hardening (deep review); fail-closed auth | — | RESOLVED (PR #25 r7: workflow now runs in the `extraction-exam` protected environment and references ONLY the environment-scoped `ANTHROPIC_API_KEY_EXAM` — the repo-wide key is unreachable through this workflow, mechanically; exams fail closed until the founder configures the environment (required reviewer + secret), after which every secret-bearing run of PR code needs an explicit approval) |
