...
6. **Reuse or create** — Existing fix wins. New fix only if there is no match. If the cause is a hold or block in a product file, the fix is delete that hold in **that** file. Do not create a wrapper that clears it later (FL-007).
7. **Apply** — Put the fix in the product file that sets the bad rule. No stub. No SEE_FILE. No second module around it.
8. **Confirm the change exists** — The file on master contains the fix. The old hold string is gone. SHA named.
...
11. **Verify the failure is gone** — Read https://1live.co `Showing N of M`. That line is proof. A green job is not.
...
- Wrapping a hold instead of deleting it in the file that sets it.
