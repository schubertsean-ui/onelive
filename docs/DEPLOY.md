...
| `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` | Only for the real stealth gate — **and only together with `CLERK_SECRET_KEY`** | Middleware + client | `NEXT_PUBLIC_` | No (it's a public client id anyway) | Half of the Clerk gate. On its own it does NOT turn the gate on (see the row below). |
| `CLERK_SECRET_KEY` | **Required whenever the publishable key is set** | Middleware (server) | plain (no prefix) | Yes — Sensitive is fine | Clerk's server calls (`clerkMiddleware`, `auth()`, `clerkClient()`) cannot run without it. Setting ONLY the publishable key used to make the middleware THROW — `500: MIDDLEWARE_INVOCATION_FAILED` on every route, including `/api/health`. Fixed 2026-07-26: the gate now requires BOTH keys, so a half-configured preview falls back to host-protected open and PRODUCTION fails closed with a readable 503 instead of crashing. |
