...
1. Generate a random key. On a Mac, open Terminal and run:
   ...
   **No terminal? Use another OFFLINE generator — never a website.** This
   ...
   - d. **iPhone, nothing else to hand.** Apple Passwords generates a
     ...
     it for this key. There is exactly ONE phone path: install your password
     manager's iOS app (1Password or Bitwarden — App Store), open its
     generator, set **Password** type, length **64**, Numbers ON, Symbols
     ON, then save it as a new item named `ONELIVE_APPROVAL_KEY` and copy it
     from there.
     If you have no password manager, **wait until you are at a computer**
     and use path a or b. Do not ask Claude — or any other assistant — to
     generate, hold, or relay this value, and do not accept one if offered:
     this key signs publish approvals, so a value that passes through the
     agent path is a key the agent has held, which is exactly the custody
     this document exists to prevent. Waiting is the correct answer; there
     is no safe shortcut, and none will be added.
...
9. **If it ever leaks** — into GitHub, chat, an email, a Slack message, a
   ...
   - e. **Redeploy — the rotation does NOT take effect until you do.** Saving
     the variable only changes what the NEXT deployment reads; the running
     production deployment keeps serving with the OLD, leaked key until it is
     replaced. Exact steps:
     - i. Go to https://vercel.com/sss-projects-e4775771/onelive/deployments
     - ii. In the **Environment** filter at the top, choose **Production**,
       so the top row is the deployment currently serving the live site.
     - iii. Click the **⋯** menu at the right end of that top row →
       **Redeploy**.
     - iv. A dialog appears. Leave **Use existing Build Cache** UNTICKED
       (a cached build can carry the old value forward), then click the
       **Redeploy** button in the dialog.
     - v. Wait for that new deployment's status to read **Ready** — while it
       still says **Building**, production is still serving the old key.
