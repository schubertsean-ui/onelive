...
1. Generate a random key. On a Mac, open Terminal and run:
   ...
   **No terminal? Use another OFFLINE generator — never a website.** This
   ...
   - d. **iPhone, nothing else to hand.** Apple Passwords generates a
     ...
     it for this key. Either wait until you are at a computer for path a/b/c,
     or, if you must do it from the phone:
     - i. Install your password manager's iOS app (1Password, Bitwarden) and
       use its generator, which DOES let you set length 64 with numbers and
       symbols.
     - ii. If you have no password manager, tell Claude "need approval key,
       phone only" and you will get a one-time value generated by the
       repo's own CSPRNG in a form you can paste — never invented by the
       model, and never reused.
     This is the one generation path that was previously left below the
     stated floor; the r19 review caught it after an earlier sweep of this
     section had claimed to be complete.
