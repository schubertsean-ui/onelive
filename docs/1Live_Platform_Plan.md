...
- Alignment checks (M vs Vision and Goals) run every hour. They are not a ticket. They do not wait on A–G.
- Tickets A–G run continuously. B is not paused.
...
- Fix Loop (docs/ONE-LIVE-FIX-LOOP.md) is how any failure is closed.
...
Work all of these continuously. Order is priority when two collide, not a queue that stops the rest.

A — Populate. Trusted door + title is enough.
B — Density. 100% of that locale’s counted aggregator union on 1Live.
