...
| unfollowable-citation | research, citation, sources, primary, synthesis | KAIZEN 2026-07-26 founder-caught ("you lie and make things up") — a cited source carries a resolvable URL and an explicit verification-status token saying whether the primary was actually READ; unverified is legal, silently unverified is not. Enforced by tools/source_verification_lint.py in tools/validate. Self-reports are the hardest case: a claim about the agent's own process/coverage/performance has no natural adversary, so it needs an external anchor (a command someone else can run, a test that fails if it is false, a non-Claude verdict) or it does not get stated |
