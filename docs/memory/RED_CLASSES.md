...
| unusable-credential-tier | model, api_key, seat, provider, quota | KAIZEN #72 r1/r3 — a minted credential is not a usable one, and a model name is never guessed from an error string: ENUMERATE what the key can actually call, preflight the pin against that list before the gate depends on it, and print the list so the fix is chosen from evidence. An absent credential stays an explicit empty seat, never a red |
