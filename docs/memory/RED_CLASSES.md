...
| gate-self-defect | adversarial_review, SYSTEM_PROMPT, verdict, evaluator, prompt | KAIZEN PR #75 ×2 — an EXAMINER's own defect can red a clean subject: two consecutive reviews reported "no blocking issues" yet the gate failed, because our SYSTEM_PROMPT displayed the two `VERDICT:` options as consecutive copyable lines while telling the model to emit one, and the (correct) ambiguity rejection then fired. When a gate fails on a subject that reviews clean, suspect the gate's own INPUT text before the subject; fix the instruction, never the assertion — relaxing the parser would be a gate-threshold relaxation (founder-crucial). Pin the input side with a test and demonstrate it red on the historical text |
