...
| unverifiable-outward-claim | deploy, site, url, preview, live, render, shareable, verify | KAIZEN founder(Red) #68 r15, RE-COMMITTED by the fix itself at r16 — THE TEST: an assertion about what a HUMAN sees must be verified from what a human can see (rendered text), never from markup, metadata or a count. If your evidence is a different KIND of thing from your claim, the claim is unsupported: say so, or go get the right evidence. Corollary: if the surface is unreachable from here, move the CHECK to where the network works — do not report the nearest number you can reach |
...
| test-codifies-the-bad-contract | tests/test_, assert, pytest | KAIZEN #68 r7/r8/r11/r13 — a test for BAD INPUT asserts the refusal, never the mechanism of the fix, and uses the input that would actually EVADE the guard rather than a convenient variant; a class fix is not credited until one test drives the real production entry point with only the network stubbed |
| incomplete-enumeration | manifest, allowlist, list, table, catalog, token | KAIZEN #28 r22-r26 + #68 r9/r14 — a hand-maintained list guarding a trust property is a defect class waiting to repeat: DERIVE it, or cross-check it with a test against the source of truth. Two such lists with no invariant between them (selection vs assertion) drift the same way — collapse them into one table with derived views |
