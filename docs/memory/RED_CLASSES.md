...
| self-weakenable-review-model | review_model, evaluator, seat, workflow, override | KAIZEN #72 r8 — the reviewed subject must never choose ANY input to the review that judges it, and a model id is such an input: an override may only SELECT from a BASE-OWNED allowlist, never introduce a model. A test comparing two PR-controlled copies of the value catches drift, not an attacker |
