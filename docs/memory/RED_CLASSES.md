...
| unusable-credential-tier | model, api_key, seat, provider, quota | KAIZEN #72 r1 — a minted credential is not a usable one: verify the TIER can call the exact model before a gate depends on it, and prefer a model the tier can actually reach over a stronger one it refuses |
