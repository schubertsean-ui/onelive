...
| missing-credential-nondisclosure-boundary | token, secret, credential, key, runbook, paste | KAIZEN #73 r6 — every founder-facing step that mints or moves a BEARER credential states its non-disclosure boundary at BOTH the mint point and the paste point (never into chat, repo, commit, email; what to do if it leaks; clear it after use), and says explicitly when an adjacent value is NOT secret. A boundary written once in one section does not cover the others: the highest-risk credential in this repo's runbook had none while a lower-risk one did |
