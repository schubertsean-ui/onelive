...
the Record rule), never for judgment; a blanket rule proposed as a class fix
is a smell; the class watch is the single index.

**Provenance chain, complete:** drafted by the Generator from the
founder-directed Kaizen review (Part 6) → mislabeled "ratified" in the first
PR #62 draft → blocked by the evaluator (r1, ratification-scope-overreach) →
relabeled PROPOSED and merged as such → ratified by the founder in a
separate, explicit decision on the exact text ("as written"). The text the
founder ratified is byte-identical to the merged proposal; no wording
changed between proposal and ratification.
