...
- The exam's FIRST REAL GRADE (claude-haiku-4-5, 74 questions): FAIL, far above the 1% bar — and the per-field predicted-vs-expected detail separated three classes: (1) CONVENTION GAPS — the prompt never defined title/time/city precisely, so the model composed headline-titles, attached dates to clock times, appended states to cities; fixed with explicit "Field conventions" in EXTRACTION_SYSTEM_PROMPT (PROMPT_VERSION 2026-07-15.1); (2) MY KEY ERRORS — 7 answer keys violated the very conventions being tested (billing-line titles, multi-event listings keyed as one) — corrected, set still 301 expected facts; (3) GENUINE MODEL ERRORS the traps were built for — expanding a Prince tribute into artist "Prince" (rule-4 violation), an artist name as venue, an artist as city; these stay failing until the model passes.
