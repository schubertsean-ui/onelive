...
- Nits fixed: tie wording in the rubric (MARQUEE/CURRENT do tie at 35 — tie-breaker stated); `.badgeline` class renamed `.card-tags` (no badge vocabulary even in code); light-mode renders recorded as R-014 (objective trigger: winner's light render + baselines ship with the Step 9 PR) instead of an unstated gap.
...
- The R-014 light-mode deferral was REJECTED by the gate (the session contract's own scope said dark+light; founder selection deserves complete comps). Cured rather than argued: all three directions now implement light mode in-file (`html.light` variable overrides + svg attribute-selector recoloring, accents deepened for AA on light), light renders committed for ALL three; R-014 flipped to RESOLVED in the same PR.
...
- Round-5 nits: canon-copy test now checks VISIBLE text (comments can't satisfy it); README boundary rewritten (trust affordances never stub, remaining placeholders are genuinely non-trust); R-014 row annotated as historical description; Step-9 acceptance item added to TODOS (visual-only controls must become real elements, mechanically tested). Suite: 34 assertions.
