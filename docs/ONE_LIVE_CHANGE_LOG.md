...
- Escalation ladder logged in `docs/memory/decisions/2026-07-15_nearby-tiered-data-source.md`: Tier 2 = in-app OSM/Overpass (free, ODbL) behind an objective usage trigger (Nearby tap-through top-5 on a live Step 9); Tier 3 = commercial Places API, founder-crucial (money + key), only on measured Tier-2 shortfall. Trust screen: link-out results are the maps provider's own — no pay-to-rank surface can exist; any future in-app list is distance-sorted, never sponsored.
