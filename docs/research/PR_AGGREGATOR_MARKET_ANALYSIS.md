...
**Date:** 2026-07-14 · **Status:** RESEARCH (standalone new-venture analysis; multibagger is context + prospective customer #1, not a required integration) · **Session Contract:** #15, orig #6 (STATE.md) · **Companion doc:** `PR_AGGREGATOR_RESEARCH.md` (sources, licensing, legal posture — R-016 still gates any spend)
...
The archive is the cold-start weapon (H14/P4 convergence). Cheapest-viable backfill stack, in build order — all subject to R-016 licensing checks where non-regulatory sources are involved:
...
4. **Free international regulators:** EDINET (free API, for-profit reuse reported), NSM (reuse-permissive terms reported), info-financiere (license unconfirmed) — historical statutory disclosures for JP/UK/FR [companion §5, all pending R-016].
...
| C. **Promise ledger, chasm-first, agent-native, archive-backed** (per §§10–12) | Empty quadrant; three doors on one corpus; archive moat compounds; customer-zero committed | Slowest to first revenue; extraction precision is existential and must be proven on a golden set first | **Recommended** — gated on R-016 (pricing/ToS re-verification before any provider spend) and R-017 (real EDGAR-seeded golden set before any extraction work); this row is directional research, not a build order |
...
3. **Licensing lockdown** — wires restrict as revenue falls [F]. Mitigation: regulatory-substrate-first design already assumes it (R-016 gates every commercial source).
