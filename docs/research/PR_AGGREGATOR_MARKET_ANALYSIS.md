...
Run per `docs/skills/po_provocation.md` with `tools/po_battery.py --seed 20260714`, random word **"beehive"**, all operators P1–P8.6, ≥2 movement techniques per provocation. The raw generator output is committed at `market_analysis_sources/PO_BATTERY_RUN_20260714.txt` (the tool emits prompts; the movement work below is the human/agent response to them). **Everything in this section is stimulus, not fact.** Full working notes condensed; the harvest table is what feeds §§10–11. M6 ledger row recorded.
