...
| G5 | A fresh clone can run the suite, and the gate distinguishes a broken environment from broken code. | one documented command, 0 manual steps; **0 red rows that mean "environment incomplete"** | `requirements-dev.txt` + `tools/bootstrap_dev.sh` + `tools/env_preflight.py` (informational, wired `|| true`) + `tools/validate`'s interpreter selection (`tests/test_validate_interpreter_selection.py`) | ENFORCED | **MET 2026-07-26** — one command (`bash tools/bootstrap_dev.sh`) creates the venv, installs every declared dev dependency and un-shallows the clone; `env_preflight` names MISSING-TOOL / UNPROVABLE-HERE at the top of every run so no red row is ambiguous. Verified against bare system python: it names exactly the three package rows the audit found. **No check was loosened** — the preflight always exits 0 and changes no verdict, asserted by test (R-058). **The '0 red rows' half was NOT yet true when this row was first graded MET, and the gap was measured, not argued:** `bootstrap_dev.sh` builds the venv outside the repo and ends by printing a `PATH=`-prefixed command, so running `bash tools/validate` without that prefix took `pytest`, `blocking_failure_check` and `perf` red on the system interpreter — three FAIL rows meaning "you omitted a prefix". Observed on the SAME tree (`git_head 274facb`) at 22:53:46Z, all-PASS at 22:59:21Z with the venv on PATH. `validate` now selects the bootstrapped venv when the default interpreter cannot import the test dependencies, announces the choice, and always yields to an explicit `PYTHON=`. It can only cause MORE checks to execute; 14 tests bind that, including a static one asserting the block references no threshold, verdict, skip counter or exit code. **AND THE FIRST VERSION OF THAT FIX DID NOT WORK, which this row records rather than quietly correcting:** the fallback looked in `$HOME/.venvs/onelive` while `bootstrap_dev.sh` creates `$REPO_ROOT/.venv`, so it could not fire on the one path it existed for — and it measured as working only because a venv from a sibling branch happened to be on this machine. Caught by three reviewer seats (R-084). The repo-local venv is now the first candidate, and the test DERIVES that path from `bootstrap_dev.sh` rather than restating it, so the two cannot disagree again. This row has now been graded MET on an untrue claim twice; the derivation is what makes the third grade checkable |
