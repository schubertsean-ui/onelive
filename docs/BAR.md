...
| H7 | There is a live deployment and its URL is recorded. | 1 URL on disk | `.github/workflows/site_health.yml` (reads `/api/health` AND the product surface from a runner) | ENFORCED | **NOT MET — two reasons left of the original three.** A preview URL is on disk (`docs/V1.md` §Deployment) and run 30217359539 measured `http_status: 200` on `/api/health` with the bypass HEADER, `supabase.reachable: true`, **`eventCount: 1532`**. So the feed's content is proven. STILL OPEN: (1) nobody has fetched `/tonight` or the friend-shareable query-parameter link on this deployment — that run used the older `/api/health`-only workflow, and the product-surface and friend-link checks were added after it, so "a stranger can open it" is INFERRED, not measured (R-077); (2) the URL is a branch alias that dies on merge (R-070). Both triggers fire on the same event: the merge, which puts the new checks on the default branch where they can run |
