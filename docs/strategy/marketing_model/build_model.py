...
<tr><td><b>HOW</b></td><td>Paste one link → a complete PREVIEW in minutes, with no accounts connected. Activation is progressive: the owner connects only the channels they choose (some require platform authorization or approval — see the connector registry, §11). The agent learns four inputs — calendar, photo library, writing voice, brand — then drafts campaigns per event. Every send requires the owner's tap; ads run on their accounts and budgets. Data flows into 1Live as a verified first-party channel that still passes the gate.</td></tr>
...
<tr><td><b>WHY THAT WHY MATTERS</b></td><td>Discovery is shifting to search+AI surfaces fed by exactly the layers nobody maintains, and every commercial fix is priced above the tiers that need it ($300–$5,000/mo). Removing the execution cost of the high-return channels changes the outcome without new spend. For 1Live: each adopting business adds verified supply, which improves the consumer feed, which increases the value of adopting — a compounding loop.</td></tr>
<tr><td><b>EXPECTED OUTCOMES</b></td><td><i>[PILOT TARGETS — illustrative, not observed]</i> For them: higher event attendance (bar: 38 tracked door entries from one campaign), list growth (winery: 41 signups vs 6 baseline/mo), sales (class sold out; 9 club conversions), at ~8–15 minutes/month of owner time and $0 cost during the initial free period. For 1Live: rising claimed-entity count, feed accuracy, and consumer reliance — measured via claim rate, coverage, and accuracy metrics. These become OBSERVED only after a live pilot with tracking and retained records.</td></tr>
</table>
<div class="rail"><b>Constraints (invariant unless marked):</b> every send requires the owner's tap · promotion runs on their accounts and budgets — no percentage of ad spend at setup or during the free period · the agent is a data source into 1Live's gate, never a publisher · nothing paid or free affects 1Live ranking · leaving keeps the Tier-1 basics they built (listings, calendar, widget, list); campaigns/creative are service outputs. <b>Pricing (direction, 2026-08-01):</b> Tier 1 maintenance &amp; standards is free permanently; Tier 2 ongoing demand generation is free for an initial period and may then be priced (flat monthly or percentage-based) below the documented market alternatives — model, rate, and timing are open founder decisions.</div>

<div class="pg"></div><h2>How to read this document</h2>
<p class="guideintro">This document makes one argument in three layers, in plain terms: the <b>research</b> (why local event marketing mostly doesn't happen, and what it costs), the <b>agent</b> (what 1Live builds for the businesses and artists who supply events), and the <b>proof</b> (the same machinery run on a real venue's public data). Fictional examples are stamped ILLUSTRATIVE; the one real run is stamped DEMONSTRATED; every number carries a claim-ledger badge.</p>
<ol class="guide">
...
<li><b>§9 The full 1Live data model</b> — how a fact becomes verified truth and where that truth flows.</li>
...
<div class="pg"></div><h2>9 · The full 1Live data model — ingestion, verification, distribution, and the loop</h2><p class="pgdesc">How a fact travels: read from public sources, corroborated, gated, then distributed as verified truth.</p><img class="flow" src="{img64('flow_model.png')}"/>
...
<tr><td>1Live listing · hosted event pages · site widget + JSON-LD · link-in-bio</td><td>DIRECT PUBLISH (our surfaces)</td><td>claim verification; site install</td><td>unique URL per event; validated markup; visible page must match structured data; gated as ever</td></tr>
...
<tr><td>Nextdoor</td><td>PARTNER-DEPENDENT → DIRECT PUBLISH</td><td>API approval + authenticated business profile</td><td>content attributable to the business, not 1Live</td></tr>
...
<div class="rail"><b>Claim ledger:</b> every load-bearing number above and in the body has a row in `docs/strategy/CLAIM_LEDGER.md` (source, population, approved wording, badge, review date). Retired wordings (C-03, C-04): the unsourced ChatGPT source-share percentages and the Bing-share architecture assumption — replaced by documented mechanisms (OAI-SearchBot; database presence). <b>Standing rules (canon):</b> their accounts stay theirs · no percentage of ad spend at setup or during the free period · nothing paid or free affects 1Live ranking.</div>
"""
