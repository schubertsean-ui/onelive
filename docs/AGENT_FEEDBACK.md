...
## 2026-08-03 — PR #147 shepherd-to-merge session (Contract #34)
- WORKED WELL: the open ritual's cost is now front-loaded and mechanical — reconcile + staleness_check + complete reads took the first third of the session and produced zero surprises; the merge decision then took minutes because both protocol conditions were provable from two API calls (run log + check runs).
- FRICTION (REPEAT, environment class): `session_reconcile.py` exits 2 UNVERIFIED in every CCR sandbox (no `gh`, no DB DSN) — the GitHub-MCP fallback works for PRs, but DB facts have now been UNVERIFIED across multiple consecutive sessions. Structural fix candidates: add the Supabase connector to this environment, or teach the reconciler a read-only Supabase REST path. Until then every close must keep saying "row counts not re-asserted."
- FRICTION: `mcp__github__actions_list` ignores the `branch` filter and returns repo-wide runs (~280KB per page) — finding one PR's review rounds took three queries + local JSON slicing. Worth noting for the next session that per-branch run history is cheapest via the saved-file + python slice pattern.
- DEFECT (prior-session, caught at this close): #148 and #149 merged with NO Kaizen ledger rows — the standing "every merged PR gets its row" rule was missed at the previous close. Backfilled honestly (rounds not re-derived, marked as such). Counter-measure candidate: staleness_check-style mechanical check that the ledger names every merge commit since the marker; until built, the close checklist item stands.
- NOTE: R-002 (visual-regression skip) nits appeared on a visual PR again, and its trigger (deployed URL exists) has fired — queued work, not silently carried.
