...
## 2026-07-23 — C2 decision layer, 18-round merge (PR #54)
- FRICTION (loop speed, relevant to the founder's "troubleshoot faster" directive): GitHub sometimes took ~10 minutes to CREATE the check-runs for a fresh push (the runs simply weren't listed, not pending) — indistinguishable at first glance from "CI didn't trigger". Lesson: on an apparently-missing check, confirm the commit exists (`get_commit`) and the remote ref advanced (`git ls-remote`) before suspecting a trigger problem; then wait it out — it's queue latency, not a failure. Don't re-push to "nudge" (creates duplicate runs).
- FRICTION (MCP ergonomics): `mcp__github__actions_list list_workflow_runs` reliably overflows the tool token limit (~70–280k chars) and dumps to a file; every status check became "call → parse the saved JSON file for name/status/conclusion/head_sha". And `head_sha`/`branch` filters are not honored server-side (returns mixed heads). Workaround that worked: `get_workflow_run_usage` for a single run's job_id, then `get_job_logs` tail for the verdict/digest. Note: the direct GitHub REST API via `$GITHUB_TOKEN`+curl does NOT return run status here (the MCP server uses its own auth) — don't build monitors on it.
- FRICTION (protocol cost): un-drafting the PR to merge re-triggered the adversarial review (`ready_for_review` is in its `on:` types) — one extra ~5-min review cycle on byte-identical code. Trust-gate does not re-trigger on that event, so only the review re-ran. Expect and budget for it when moving an agent-merges-on-green PR out of draft.
- WORKED WELL: the ground-truth-recompute fix (r17) DEFINITIVELY closed the audit-record self-verification class the evaluator had chased for many rounds — deriving the decision from `mode_probs×matrix` with an exact anchor left no tolerated middleman, and r18 was a clean APPROVE. Distilled to a reusable gotcha (docs/memory/gotchas/2026-07-23).
- WATCH (process, ledger M2): 18 rounds is extraordinary. The counter-measure is real and stated up front next time — land audit-record self-verification as its own small PR with the "recompute from ground truth" invariant declared, not discovered. A large single diff also visibly compounded per-round review cost (the evaluator said so twice).
