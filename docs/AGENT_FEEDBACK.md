...
- DEFECT (self-caught, 2026-07-18, ledgered): pushed cb87a84 on a red validate — `sed` in the pipeline swallowed the gate's exit code before `&&`. Rule going forward: validate runs BARE and its exit is checked before any commit/push chain. Automate next: pre-push git hook running `bash tools/validate --allow-skips` (install via tools/install_hooks.sh) so the shell mistake class is mechanically impossible.


## 2026-08-03 — UI/UX quality-gates session (Contract #35)
- FRICTION (REPEAT class: env heals): the sandbox again needed pip installs (pytest, pydantic, fastapi, cffi/cryptography re-pin) and `git fetch --unshallow` before the python suite could run — the same heal list as Contract #23. Automate next: a setup script (or environment config) that pre-installs worker/api test deps and unshallows, so every session stops re-discovering it.
- FRICTION: the harness resets the shell cwd between calls, and `npm i` at the repo root silently CREATED a root package.json + node_modules instead of erroring — cost one confused round. Gotcha filed (2026-08-03_chromium-ci-apparmor-and-npm-cwd.md); rule: verify node_modules/<pkg> at the intended path after any install.
...
## 2026-08-05 — discovered-events engine session (Contract #44, PR #191)
- FRICTION (cost a full ~10-minute cycle): a branch-dispatched `ingest.yml` smoke run shares the single `concurrency: group: ingest` with the master CRON (7,27,47). My dispatch at 22:04 queued behind the :07 scheduled run and was CANCELLED outright, so the arming re-bind stalled with no failure to diagnose — a cancelled run looks like nothing happened. Automate next: key the group by ref (`ingest-${{ github.ref }}`), so a branch smoke run never contends with the production cron. Deliberately NOT done in this PR: `ingest.yml` is always in the armed runtime closure, so touching it forces the very re-bind it would be fixing — it belongs in a PR that is already re-binding for another reason.
- FRICTION (structural, 5 re-binds this PR): every touch of a file in the armed runtime closure invalidates `ARMING_SMOKE_RUN.json`, and each re-bind costs a paid ~8-minute ingest run plus a full CI round. Reviewer-driven fixes to `worker/datetime_resolve.py` therefore cost one cycle EACH. The binding is correct and should not be weakened; the cheap improvement is batching — collect all reviewer findings for a round, fix them in ONE commit, then re-smoke once.
- WORKED WELL: the adversarial panel split cleanly — both gemini lenses APPROVE, both openai lenses REQUEST-CHANGES with five findings, and all five were REAL (a weekday-only claim taking a fabricated midnight; an unchecked raw-claim weekday; a stale listing rolled a year forward; an end_time before its start; an unguarded write onto already-public rows). Panel diversity earned its cost on this PR: the lens that found them found nothing the other family did.
- WORKED WELL / WATCH: one pre-existing test ASSERTED the roll-a-year-forward defect (`test_past_beyond_grace_rolls_to_next_year`). A test can pin a bug as though it were a contract; when a reviewer finding contradicts a green test, the test is a suspect too, not an alibi.
- NOTE: the sandbox egress proxy blocks BOTH venue sites and 1live.co, so no live page could be fetched to ground-truth why the date resolvers refuse every live claim. That is why the refusal ledger (structured per-refusal reasons emitted by the pipeline itself) is the committed next workstream rather than more local analysis — in this environment the pipeline's own logs are the only ground truth available.
- DEFECT (self-caught, 2026-08-06, ledgered): a subagent simulating merges set `user.name=sim` / `user.email=a@b.c` in the repo's LOCAL git config and never restored it, so six consolidation commits — including the head — were authored by a junk identity. The visible symptom was not a git error: Vercel BLOCKED the preview deployment with "GitHub couldn't verify an account for the commit", which reads like a permissions problem and not like something the agent did. Fixed by restoring the identity and rewriting author/committer metadata over `origin/master..HEAD` (tree hash verified byte-identical before the force-with-lease). Class: subagent-mutates-shared-repo-config. Automate next: subagents that need a git identity must set it per-invocation (`git -c user.name=... -c user.email=...`) or work in an isolated worktree, never `git config --local`; and a pre-push check should assert the committer email matches the session identity, since the failure surfaces downstream in a vendor UI rather than at the commit.

## 2026-08-06 — founder stop mid-flight (Contract #44, PR #191 still open)
- FRICTION (self-inflicted, cost one full review cycle): I pushed the R-092 commit while a round-2 adversarial review was running, which superseded and cancelled it. Rule for next time, stated so it is not re-learned: once a review is dispatched, no push until it returns — the review's subject must not move underneath it.
- FRICTION (repeat, now 7 re-binds on one PR): every reviewer-driven fix to a file in the armed runtime closure costs a paid ~8-minute ingest run plus a full CI round. Already logged 2026-08-05; recording the second instance because it recurred exactly as predicted. The binding is correct and must not be weakened. The cheap fix remains batching — collect ALL findings for a round, fix in ONE commit, re-smoke once.
- DEFECT (self-caught, mine): I told the founder a smoke run had hung for 70+ minutes and cancelled it. The log showed it had run ~6 minutes and processed 3 of 8 sources. It was pagination saturating the block cap, not a hang. Corrected to the founder explicitly and recorded as R-092, which also shipped the observability (`pages_fetched`, `sources_skipped_time_budget`) that made the real behaviour visible — and proved itself on its first live run (`pages_fetched: 6` vs `fetched: 5`, the first-ever evidence that pagination fires). Class: diagnosed-from-absence-of-output — a quiet job is not a stopped job; read the log before naming a failure mode to the founder.
- DEFECT (self-caught, mine, trust-adjacent): I recommended anchoring the whole `local_media` class to the founder. The adversarial pass proved 4 of its 14 rows are user-submitted Trumba widgets that our OWN source catalog annotates "treat unverified" — under my recommendation, anyone filling in a TV station's submission form would have published to the live site alone as `confirmed`. The catalog knew; the gate did not; I did not read the catalog before recommending. Automate next: any change to `ANCHOR_CLASSES` must print the live per-class row provenance from the catalog as part of its own evidence, so a class cannot be promoted to anchor without its member rows being looked at.
- WORKED WELL: retirement beat patching. `resolve_time_only_from_block` accumulated five fabrication paths across four review rounds while its MEASURED yield over seven live smoke runs was zero. Because R-093's trigger demanded that comparison rather than a judgment call, the deletion was evidence-backed and uncontroversial. Recording it as the pattern: when a component's trigger asks "measure the realised yield," the measurement can end the argument in either direction.
