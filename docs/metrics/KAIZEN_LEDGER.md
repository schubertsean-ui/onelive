...
| 2026-07-17 | #31 | 3 | evaluator r1: untruthful-record ×1 (R-013 marked RESOLVED on promised future evidence — REPEAT of the watched class; fixed to present-truth staging), r2-r3: evidence-custody visibility gap ×1 (the reviewer structurally cannot see bound exam evidence — spawned PR #32) | reviewer-evidence supply (PR #32, in review); records-state-present-truth reaffirmed | ~3 evaluator calls + 1 attended exam (~$1.50) + 1 zero-cost fail-closed probe | THE FLIP PR: extraction certified on (run 29585733298, 0.64%/97.2%/314 facts); founder-merged through the documented reviewer-visibility red (#11/#25 precedent), squash 29f26c6 |
