...
| 2026-07-17 | #29 | 1 | — | — | ~1 evaluator call | bookkeeping (ledger row for #27 + contract close, docs-only); founder-directed merge — the last founder-clicked merge before the autonomy directive. PR #30's own row is deliberately NOT self-included: #30 is not bookkeeping-only (decision record + process change ride along), so per the convention it introduces, its row folds into the next session's PR — evaluator r1 on #30 caught exactly this (class: convention-scope-overreach) when a self-row was attempted |
