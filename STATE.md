...
SCOPE: (1) multi-source secondary research, ≥2-source cross-checked, per the PR-aggregator precedent; (2) written scout at `docs/strategy/ONE_LIVE_EPIPLEXITY_SECONDARY_SCOUT_v1.md` (MemoHarness-review format: what the paper is, reported formal core, theory-landscape comparison, critique, per-work-stream PROVISIONAL readings); (3) one P3 item queued (compression-gain `structural_yield` ingestion observability metric — stdlib zlib, no gate coupling, justified independently of the paper); (4) cross-reference recorded (not new canon): the standing trust invariants already exclude structure/compressibility signals from gates/confidence/trust surfaces; (5) bookends.
NON-GOALS: no code, no pipeline changes, no gate or threshold changes, no spend, no new services; no interference with the live Step-5/R-023 watch (owned by the other session).
DONE-CRITERIA: scout committed on the designated branch · R-024 recorded same-commit · TODOS item queued · bookends updated · validate green (skips logged) · draft PR opened (evaluator rides mechanically) · contract closure beyond the scout deliberately deferred to R-024's trigger (recorded, not silent).
