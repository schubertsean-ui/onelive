...
`passed: 2`, $0.5614), and after the panel's round-2 fixes the re-bind 33799033633 at head
312d3ec, conclusion success — 2 sources, 2 fetches, 60 candidates, `passed: 1` + `held: 1`
(Waterloo Greenway PASS beside Meetup HELD on "Insufficient corroboration (have 1; need
2)" — the corroboration floor in live output), 0 errors, `listings_updated: 0`, the dry
writer disabled in the loop's own output, $0.3211. Its two scope limits are stated FIRST
...
[S3:grant-not-content-bound] Must-do 5's grant is one dry 2-source smoke, and it is spent as exactly that and no more: run 33794455238 at `max_sources=2`, `source_class=B`, `follow_pages_per_run=2`, `listing_update=dry`. THREE runs executed, the second and third being the standing re-binds the grant's own second sentence authorizes, each forced by a panel round that edited `worker/segment.py`: 33794455238 (head 5749c24), 33796653453 (head 3f02d95, after round 1) and 33799033633 (head 312d3ec, after round 2). An earlier dispatch (33793257593, head 01a3d15) was cancelled while still QUEUED — no job started, nothing spent — because the adversarial re-read found two defects in the very code it would have bound, and binding a one-run grant to a head about to be superseded would have consumed it for nothing; that cancellation is recorded in the evidence file rather than left to be inferred from a gap in the run list, the same reading PR #217 applied to its own cancelled dispatch. The standing dry re-bind covers later rounds of THIS PR only and authorizes no armed run and no merge.
