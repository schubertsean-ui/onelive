...
## Session Contract #76 (2026-09-07, founder — the default walk is every public pack door; skip only what is already PUBLIC, branch claude/default-doors-config-3ypswy) — OPEN

Contract #75 CLOSED at merge 3d46070 (PR #244); its records-only follow-up merged 8ac512d (PR #246).
WHAT: `tools/desk_ingest.py` (`default_doors()` replaces the two-id `DEFAULT_DOORS`; `walk_doors` NAMES a door it cannot walk instead of stopping the run; the founder's 10-column walk table; `ingest()` skips only what is already PUBLIC), `.github/workflows/desk-ingest.yml` + `.github/workflows/desk-split-dryrun.yml` (doors default `all`, ceilings that cover the wider walk), `tests/test_desk_ingest_default_doors.py`, `tests/test_desk_ingest_skip_to_promote.py`, `tests/test_desk_ingest_workflow_contract.py`, `tests/test_event_page_dryrun.py`.
HOW: the door list is DERIVED from `sources/locale_packs/<locale>.json` through `pack.public_desks` (public + a read path + a door type a happening may be listed from), most trusted first, with `--door` still the override; a door with no catalog row or no committed fixture is reported by name with its remedy and skipped, and a run that walks NOTHING still raises; the skip test becomes "was it ever published", and a standing candidate is promoted rather than re-created.
WHY: the default walked 2 of the pack's 26 readable doors, and the 2026-09-07 write's own 977 skipped keys include candidates that were never published and, under the old rule, never could be.
WHY-IT-MATTERS: both are one defect — what is on the map decided by our bookkeeping instead of by what the desks printed — and the reason THAT matters is that neither announces itself: a starved walk and a permanent skip both report success, so the catalog silently stops being a map of what is on.
EXPECTED OUTCOMES: (a) a run with no `--door` walks every public door in the pack, derived, most trusted first; (b) a complete triple from a trusted door that exists only as a candidate promotes, while a hole still holds and the gate still decides; (c) one row per door walked — door_id, door_type, fetched?, pages, rows_split, mash_n, dated_n, placed_n, 403_n, unsplit_n — with mash_n 0 and a 403 printed as UNKNOWN; (d) draft PR, dry-run only, no merge without the founder's word.
OUT OF SCOPE: `--write` on any branch, adding the 12 unlabelled publishers to `sources/master_sources_catalog_120.json` (it is inside the armed cron's runtime closure — founder call), `tools/desk_union.py`'s own two-door default, the `--city Austin` report default, web/, ingest.yml / run_once.py / ai_extract.py / import_licensed.yml / import_structured.yml, ticketmaster-discovery / axs / facebook / instagram, category weighting, Kaizen/ledger/hats, merging.
STATUS: OPEN.
...
HOW: the workflow keeps its one-token diff; the derivation and the evidence live in the test, in the idiom of the existing `tests/test_ingest_workflow_contract.py` — read the yaml, assert the write job's ceiling is at least 60, and assert the walk inputs it must cover (`max_pages` 40, `DEFAULT_FOLLOW_PAGES` 200 at the 2.0s politeness delay) still fit inside it with the write's own time left over. No follow-cap change, no politeness change, no second writer, no batching rewrite.
WHY: the authorized write (run 34079785167) walked both desks, printed a ~1584-row plan at 03:39Z, and was still writing 20 minutes later when `timeout-minutes: 30` made GitHub cancel the job at 03:59Z (run_duration 1822s). The walk alone spends ~8 minutes in politeness sleeps before the first row is written.
...
STATUS: CLOSED — merged e9e441d (PR #243). The live dry-run that followed (run 34077247305, master) came back publish_n 94, mash_n 0, tba_public_n 0. The authorized write dispatch (34079785167) printed a ~1584-row plan at 03:39Z, then GitHub cancelled the job at the 30-minute wall (run_duration 1822s) while python was still writing — Contract #75 raises that wall.
...
WHY: the Ticket D run read 1571 rows and could knock on 40 of them, leaving 1552 holed — 40 pages cannot fill 1571 rows.
WHY-IT-MATTERS: while the budget is 40, every "how many rows could a friend act on" number measures the cap and not the desks, so the reader's ~75% yield stays a claim about 40 pages instead of a measured answer about the catalog.
EXPECTED OUTCOMES: the live table comes back with `pages_followed` up to 200 and a smaller `still_null_n`; friends still see nothing (no `--write`, no `--real` DSN, no catalog row); worst case ~12 min of runner time (241 fetches at the 2.0s politeness sleep), inside the job's 20-minute timeout.
...
WHY: Ticket B's rows carry holes where the night and the place go, and Ticket C's reader that fills them from the page's own words is merged but wired to nothing — it runs only in tests.
WHY-IT-MATTERS: an undated, unplaced row cannot reach Tonight at all, so until the reader runs on the real walk we have no measured answer to "how many of these rows could a friend actually act on" — and every coverage number stays a claim about pages instead of rows.
EXPECTED OUTCOMES: page states date+venue -> row carries both; clock-only page -> `when` stays NULL; a 403 is a hole (queued, one knock) and never a mash; the dry run writes nothing (`--write` still needs `--real` AND a DSN, and neither the CLI default nor the workflow supplies them); the live table comes back from the runner.
OUT OF SCOPE: Kaizen/ledger/hats, new importer, catalog upsert, `ai_extract`, Ticketmaster, raising the list-walk cap, arming a schedule, Tonight redesign, PRs #232/#235.
STATUS: CLOSED — merged as 7357116 (PR #238). `bash tools/validate` green on the committed tree (evidence in the PR body); the two non-PASS rows are the recorded R-068 visual_regression env SKIP and a commit_sweep advisory naming only already-merged commits. LIVE table from run 34058897688 on this branch (GitHub runner; the sandbox proxy still 403s both desks): Chronicle 1571 rows off 40 list pages, 40 event pages followed (the full cap), 30 nights and 20 places FILLED, 1552 rows still holed, mash_n 0; Do512 answered 403 on page 1 and is queued — an unread desk, never an empty one. Evaluator r1 on head 58ee5cb: 3 of 4 seats APPROVE, one REQUEST-CHANGES with two findings — (1) REAL and fixed: the dry run follows event pages and plans from the filled rows while `--write` does not follow, so a section headed "The write plan" showed an operator writes the real write path will not produce; the heading and a derived caveat now say so on the run itself. (2) NOT reproduced: rows sharing one permalink are all counted as asked (one knock, four rows answered, not_asked 0) — checked, evidence in the PR reply, pinned by a test. Evaluator r3 on head cb517ef: 3 of 4 APPROVE again; the one blocking finding was a TAUTOLOGICAL TEST I wrote in r2 — one assertion split across two lines left `assert f"..."` standing alone, always truthy, on the row separating walls MET from pages we declined to knock on. Fixed as ONE assertion on the whole row, mutation-checked (wrong numbers -> red), and an AST scan of the file reports no always-truthy or weak-`or` assertions left. Its nit taken too: the end-to-end heading test ran `--follow-pages 0` ("followed nothing") instead of the contradiction case, and now substitutes only the fetched bytes so pages ARE followed and change no row. Evaluator r2 on head f26d807: same 3-of-4 APPROVE, and the one seat found BOTH of its blocking defects inside the r1 fix itself — (1) the §4 heading was guarded on "a page was followed" while the caveat under it was guarded on "a row changed", so a run that followed and changed nothing printed a heading contradicting the line beneath it; one predicate now (`changed_rows_n`). (2) the budget-spread sentence counted `len(visits)` — ROWS — so three rows sharing one permalink read as three pages spent of the founder's cap; the unit is now a KNOCK (`_knocks`), excluding reused, not-knocked and off-host visits. Both pinned. Three further defects were found and fixed after the first push, each pinned by a test: a desk with no fetcher was handed a share of the budget it could never spend, the .ics cost of a followed page was unstated, and the table claimed a round-robin spread on a run where one desk was walled and every page went to the other. No merge without the founder's word.
...
STATUS: CLOSED — merged as ad3a578 (PR #237). Built and pushed at head 7b5d016. `bash tools/validate` green on the committed tree (evidence block in the PR body); the two non-PASS rows are the recorded R-068 visual_regression env SKIP and an advisory naming only already-merged commits. Four defects were found and fixed while building, each pinned by a test: a one-outbound-link `<html>` read as a foreign card (dropped every date), `"calendar"` read as an end time, one instant written two ways read as a disagreement (PR #229 r8's defect), and a venue elaboration read as a different venue. The live table is NOT in this PR: the sandbox proxy 403s both desks (probed once, `Tunnel connection failed: 403 Forbidden`), so the 20-row table is a fixture run and says so on every print. No merge without the founder's word.
