...
SCOPE AS DELIVERED (final; the original scout-only scope was stage 1 and is subsumed — evaluator r4 nit: no mixed historical/final reading): (1) stage-1 secondary research + scout `docs/strategy/ONE_LIVE_EPIPLEXITY_SECONDARY_SCOUT_v1.md` (retained, superseded — R-024); (2) stage-2 primary verification: founder-supplied arXiv:2601.03220v2 PDF read in full → completed review `docs/strategy/ONE_LIVE_EPIPLEXITY_REVIEW_v1_1.md` with claim-by-claim verification ledger and a manifest-style provenance record (sha256-pinned) for the out-of-repo primary; (3) one P3 item queued (compression-gain `structural_yield` ingestion observability metric — justified independently of the paper); (4) cross-reference recorded (not new canon): standing trust invariants already exclude structure/compressibility signals from gates/confidence/trust surfaces; (5) gate tightenings forced by review rounds (provenance_lint + arming-binding partition correction), each evaluator-reviewed in-PR; (6) bookends.
