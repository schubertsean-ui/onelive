...
STATUS: CLOSED — merged 7febba9 (PR #241). NO WRITE HAPPENED, and that is the finding, not a failure to finish. Live dry run 34073072428 (master head 7febba9, write=false, default doors, max_pages 40, follow 200): publish_n 94 | hold_n 1490 | skip_n — | mash_n 0 | 403_n 1 | dated_n 162 | placed_n 130 | tba_public_n 0. The founder's two gate numbers are 0, so the table is clean — but the SAME run's write-plan caveat prints "162 night(s) and 130 place(s) here came from an event page and would be NULL under `--write`". dated_n (162) EQUALS nights-filled-by-following (162) and placed_n (130) equals places-filled (130); both count rows and the run planned 1584 rows off ONE desk (0 multi-desk), so the counts are 1:1 and the equality proves ZERO nights and ZERO places came from a list page. `tools/desk_ingest.py:1010` is `if args.write:` -> following is skipped entirely on a write. So the authorized write would plan 1584 rows carrying no night and no place, the merged filter would hold every one, and publish_n on the write would be 0 — not the 94 Must-do 4d requires it to match. The one-shot authorization was NOT spent on it. A second cost, beyond futility: an unkeyable row takes `url:<listing_url>` as its ingest key (`desk_publish.ingest_key`), so writing now would store 1584 candidates under `url:` keys that the follow-wired write would later re-write under `night~place~title` keys — duplicate candidates the de-dup cannot reconcile, because it matches on the key. SMALLEST UNBLOCK, founder's call: wire `event_page.follow()`/`apply()` into the `--write` path (the `if args.write:` branch above), which Ticket D deliberately excluded as "a catalog change". Until that lands, the desks' LIST pages state no nights and no places, so the write path cannot publish anything under this filter.
