...
desk-ingest.yml gains a 6-hourly `--real --write` schedule on master with the
dead-man + budget shape ingest.yml already uses; an UNREADABLE desk becomes a
FAILED check with an ops-visible report, never "0 events" and never a delete;
the desk fetcher gains one identity (1Live + contact URL), robots/Crawl-delay,
and conditional GET (ETag/If-Modified-Since -> 304).
HOW: `tools/desk_coverage.py` live_fetcher -> a polite fetcher module
(`worker/locale/desk_fetch.py`): robots per host, crawl-delay, validator+body
cache persisted between runs by actions/cache; `tools/desk_ingest.py` exits
non-zero on an unreadable desk; `.github/workflows/desk-ingest.yml` gains the
schedule, its own dead-man assertion and page budget. `tools/desk_ingest.py`
stays the ONLY write seam (create_candidate -> add_evidence -> promote_candidate);
no second publisher, no new parser, no gate change.
WHY: the write path exists and only a human dispatch fires it, so the catalog
only grows when the founder pushes a button.
WHY-THAT-WHY-MATTERS: a publish path that needs the founder is not a publish
path; it is a demo. Everything measured after it describes what we ran by hand.
EXPECTED OUTCOMES: a dispatched write=false run on master prints the Desks
table from that runner (pages read per desk); if pages read > 0, one write=true
run with before/after /tonight counts from production; if pages read = 0 the
run FAILS and the report names the worker to add; CLAUDE.md states agents never
fetch desks from a sandbox. No merge until the founder says so.

**construction_gate Stage 3 (founder-capped at 5 lines):**
[S3:contract-scope-violation] [S3:build-before-plan] [S3:permission-for-ratified-work] [S3:governance-ambiguity] [S3:status-narration-not-progress] [S3:hygiene-narrows-coverage] [S3:stalled-state-needs-active-diagnosis] Scope is the founder's five Must-dos: the schedule, the UNREADABLE-is-red rule, fetch identity/robots/conditional-GET, the runner proof, and the CLAUDE.md paragraph. The contract above was written before the first product file. Refused in the PR body: a Tonight change, a generic card parser, a Chronicle reader rewrite (named as R-114 instead), a second publisher, a new vendor. The one thing this branch cannot do — reach a desk — is diagnosed to the sandbox proxy and answered by DISPATCHING the workflow twice on master, not waited on.
[S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:self-weakenable-review-model] [S3:final-gate-trusts-generator] [S3:untested-gate-branch] [S3:release-path-weaker-than-generation] [S3:rule-stronger-than-mechanism] [S3:fail-open-on-custody-misconfig] [S3:weak-key-accepted-at-custody] [S3:caller-suppliable-custody-inputs] [S3:unusable-credential-tier] [S3:mutable-model-alias] [S3:workflow-tool-version-skew] [S3:volatile-safety-store] No gate, threshold, reviewer binding, action pin or model route is touched, and the schedule makes the path STRICTER in three places, each with its own test: an unreadable desk now exits 3 and `pipefail` stops `tee` swallowing it; the schedule's write decision is resolved from `github.event_name` (not caller-influenceable) because `inputs.write` is empty on a schedule event and would have skipped DSN validation on exactly the runs that write; the dead-man assertion fails CLOSED on a missing check, missing key or period mismatch, so an unwatched scheduled writer cannot start. Every rule ships with its mechanism named beside it (tests/test_desk_ingest_workflow_contract.py, 15 cases). The fetch cache is a convenience that can never become an outage: a corrupt or missing file is an EMPTY cache, and a 304 whose body we no longer hold is reported UNREAD, never as an empty page.
[S3:fabricated-qualitative-copy] [S3:copy-outruns-registry] [S3:false-price-claim] [S3:semantic-claim-not-rederived] [S3:destructive-normalization] [S3:swallowed-corrupt-data] [S3:partial-write-whole-row] [S3:missing-cardinality-check] [S3:pagination-integrity-gap] [S3:db-type-mismatch-invisible-to-hermetic-tests] [S3:nonfinite-numeric-accepted] [S3:nonfinite-decimal-accepted] The one product-data change is a REFUSAL, not a generation: `front_door_reason()` HOLDS a row whose identity is any URL the walk itself read as a LIST — the bare origin, the catalog's events index, the door the walk started at, and every page it paginated or `next`-linked through (the evaluator's r1 blocking finding: the first three were special cases and a composite keyed at page 2 still published; the fourth closes the class) — because the master runner proved that row is every title on the page glued together — a happening nobody is holding, which is fabrication arriving through the front door. The test is on the row's URL, never on how its text looks, so no heuristic judges copy. HELD is not DROPPED: candidate and evidence are still written and the row stays in the ops queue (pinned), nothing is deleted, no count is inflated, and a real listing under the same host still publishes (pinned). No SQL type, Decimal or numeric is parsed in this diff.
[S3:deliverable-visual-qa] [S3:env-dependent-hermetic-test] [S3:founder-path-unprobed] [S3:retyped-evidence] [S3:pushed-on-red] [S3:stale-base-widens-range] [S3:green-on-stale-base] [S3:scripted-transform-order] [S3:heal-drops-guard-marker] [S3:excluded-surface-widening] The founder's path is probed on the founder's infrastructure, not argued: two dispatched master runs (33986288662, 33988204239) and their tables are quoted from the runs' own output, never retyped from memory. The 22 fetch tests and 15 workflow tests are hermetic — no network, no clock, no sleeping, no DB — and the fetcher's transport is injected. Full suite 2992 passed after the container's shallow clone was unshallowed (6 certification tests need real history; they are green, not excused). Base is master tip 8a4d1ffd576f by the gate's own freshness line. No visual surface is touched. No script rewrites a file in place, no guard marker is rewritten, and no scanner exclusion is widened.
[S3:deferred-trust-work] [S3:records-sharing-one-trigger] [S3:parallel-record-id-collision] [S3:stale-redclass-count] [S3:missing-record-read-as-state] [S3:malformed-ledger-row] [S3:stale-live-incident-state] [S3:founder-verbatim-corrected] Three records open, each with its own objective trigger and no shared one: R-112 the UA's contact URL is the site root (trigger: the next PR touching a public route), R-113 Do512 refuses the runner so the armed check is red until the founder answers (a)/(b)/(c) (trigger: that answer, or a run recording the desk readable), R-114 the Chronicle desk reads 40 live pages and yields no publishable row (trigger: the first scheduled report showing the front-door hold). Ids taken above the highest committed row (R-111), so no parallel session collides. No ledger count or live-system state is retyped anywhere; every number here is pasted from a tool's own output. The founder's ticket text is quoted verbatim, uncorrected.
...
```json
{
  ...
  "reconciled_through_commit": "8a4d1ffd576fcb9b75f69db4e7360aec326acf25"
}
```
<!-- GROUND_TRUTH:END -->
