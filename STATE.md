...
STATUS: OPEN — built and pushed, PR #235 (draft, NOT merged; the founder merges). Ticket C's five Must-dos are delivered: fields read from the event page, the 40-page follow budget with fail-closed walls, the three acceptance tests, the live dry-ingest table, and In scope / Refused on every commit.
EVIDENCE (the round-by-round record, 27 evaluator rounds, lives there and is not duplicated here): `docs/evidence/2026-09-06_permalink-follow.md` §7–§13ac. Open residuals: R-115 (word clocks in Latin-script languages the vocabulary lacks; 0 of 40 live exposure) and R-116 (an unheaded, unlinked block supplying a field). R-116's exposure was reported at r27 as "8 of 40" and that count was CONFLATED — it merged date-from-prose with place-from-a-labelled-element, and the latter is how most desks state an address; the two are counted separately since r28 and the corrected numbers are R-116 DATE 0 of 40, R-116 place 8 of 40, R-115 0 of 40 (run 34035078704 on 57eaf23) — the date exposure both seats block on is ZERO on these desks — both bounded, both pinned in the suite, both COUNTED on every run.
LIVE TABLE (run 34032558118 on 925a19b, no `--write`, no credential): Chronicle rows_n 1572, dated_n 27, still_null_n 1545, 403_n 0, mash_n 0; Do512 403_n 1 (class D on first contact — list UNKNOWN, not empty). 1532 rows were never asked (budget), so dated_n is a FLOOR and still_null_n a CEILING.
WHY THIS NOTE IS SHORT: it had grown to a single 54 KB line, rewritten every round, which pushed the branch diff past the founder-ratified 800 KB review cap and BLOCKED the mandatory independent review (`pr_size_check`: 786 KB vs 781 KB). The founder's Operating Law caps this note at 8 lines — "Code before essays … minimum tokens spent on STATE novels" — and it was the rule being broken. Nothing was lost: the narrative is in the evidence doc, so no evidence left the reviewed diff (OPERATING CHARTER 3.7).
