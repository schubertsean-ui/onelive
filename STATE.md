...
STATUS: OPEN — built, pushed, PR #229 (draft). Panel r1/r2/r3, each 3 APPROVE / 1 REQUEST-CHANGES, each one layer further out on ONE class — a hole that reaches a reader as the wrong hole. All three FIXED: r1 a re-run compares the desk's STATEMENT, not only the key; r2 a superseded row is marked DISPUTED; r3 the three shapes of a NULL clock are separated before they collapse into one display (no date stated publishes, contested times publish DISPUTED, date-only is HELD). R-110 and R-111 carry the two residuals, both with objective triggers.
