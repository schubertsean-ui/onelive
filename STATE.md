...
STATUS: OPEN
WHAT: Fair crawl. New `worker/crawl_state.py` (per-source door/due/fingerprint/fail-streak, DERIVED from the existing `raw_fetch` + `event_candidate` rows — no new table, column, or vendor); `worker/orchestrator.py` fetches 1-2 doors per source (best_url, else top-scored events/calendar path, else start) and skips extract on 304/fingerprint-match; `worker/run_once.py` picks the next K *due* sources by rotation cursor; `.github/workflows/ingest.yml` armed-cron knobs; `tests/test_crawl_state.py` + orchestrator/run_once tests; run table printed per run.
HOW: due_at/last_success/last_body_fingerprint/fail_streak read off `raw_fetch` (successes carry sha256 + ETag/Last-Modified; attempts carry `attempt:*`), best_url off the newest `event_candidate.source_url` re-checked same-site; conditional GET (If-None-Match/If-Modified-Since) makes 304 real; 401/403 -> class D one knock, 429/503 -> exponential backoff (before demote, so `WALL_STATUSES` is untouched); K = the existing `--max-sources` ceiling, default 12 on the cron.
WHY: two fat calendars currently eat the whole 30-page follow budget in source order, so the tail of the catalog is never crawled.
WHY-IT-MATTERS: Coverage Law makes a missing locale/category a defect; a budget that starves all but two sources is that defect in scheduler form.
EXPECTED OUTCOMES: many sources per wave x <=2 doors each; unchanged bodies cost a conditional GET and zero AI calls; walls knock once; table `source | url fetched | changed? | candidates | skipped-unchanged | blocked` from fixtures. Out of scope: Tonight redesign, catalog upsert, census rewrite, category weighting, new vendor, R-089, vision-doc edits.
