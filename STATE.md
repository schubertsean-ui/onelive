...
[S3:contract-scope-violation] [S3:status-narration-not-progress] [S3:governance-ambiguity] [S3:permission-for-ratified-work] [S3:stale-redclass-count] [S3:missing-record-read-as-state] [S3:malformed-ledger-row] [S3:deferred-trust-work] [S3:hygiene-narrows-coverage] Scope is the founder's four Must-dos: the write path, the source label, the UNREADABLE stop with the one command, and the before/after table. Refused in the PR body: Daily/Weekender, a Tonight redesign, a third desk, a union-only change, any STATE heal. R-110 is opened at r1 for the one residual the evaluator's blocking finding leaves (a published row is not corrected when its desk changes its word — the mutation belongs to `worker/listing_update.py`), with an objective, now-unblocked trigger; no ledger count is retyped.
