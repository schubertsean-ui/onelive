...
EXPECTED OUTCOMES: many sources per tick x <=2 doors each, no category weighting anywhere; unchanged pages cost a conditional GET and zero AI calls; walls knock once; nothing published is mutated; table `source | queue | url fetched | changed? | verified? | candidates | skipped-unchanged | blocked` plus a measured fetches/extracts/$ outcomes line, from fixtures. Out of scope: Tonight redesign, catalog upsert, census rewrite, category weighting, new vendor, R-089, vision-doc edits, and any path that mutates a published event (trust-invariant — founder's call).
