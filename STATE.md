...
## Session Contract #76 (2026-09-07, founder — named windows on the existing feed, branch claude/feed-named-windows-8vl8xa) — OPEN

Contract #75 CLOSED at merge 3d46070 (PR #244).
WHAT: `web/lib/feed.ts` `dayTabs` gains six named windows — Tonight, This week, This weekend, This month, Next week, Next month — plus `splitByTiming` (on-now / upcoming), `resolveTab` (fail-closed) and `emptyWindowNote` (honest gathering copy); `web/app/(public)/tonight/FeedApp.tsx` consumes them; `web/lib/feed.test.ts` pins the bounds.
HOW: market-clock helpers (`atMarketHour`, `addMarketDays`, `startOfMarketWeek/Month`) derive every bound in America/Chicago, DST-safe by probe, exactly as `startOfLocalDay` already does. Today / Tomorrow / All upcoming keep their keys, order position and physics; the named windows are added around them, never in place of them.
WHY: the feed can only be asked "what is on today / tomorrow / a named date / ever". A person asks "tonight" and "this weekend", and today the site cannot answer either.
WHY-IT-MATTERS: the vision is the fastest way to see what is on, here, when — the question a person actually asks is the unit of the product. The reason THAT matters is that an unanswerable question reads as an empty catalog: with no Tonight window, a thin night looks like a thin site, and the honest gathering state never gets a chance to say otherwise.
EXPECTED OUTCOMES: (a) nine chips answer the six named asks with Today still the default; (b) inside a named window on-now leads and the split sums to the input; (c) a zero-row window says we are gathering, N of M, walls unknown — never "0 events in Austin"; (d) a bad `?when=` token renders Today; (e) draft PR, no merge without the founder's word.
OUT OF SCOPE: worker/, tools/desk_ingest.py, ingest.yml, ai_extract.py, import_licensed.yml; new importer/vendor; pay-to-rank; infinite scroll; for-you hide; replacing Today/Tomorrow/All upcoming; TBA on the default view; Kaizen/hats/ledger; merging.
DONE TEST: `cd web && npx vitest run` (303 -> 333 tests, 21 -> 22 files) + `bash tools/validate` — every check PASS on this tree (commit_sweep ADVISORY names only historical commits). The 6 pytest failures on the first run were a SHALLOW CLONE in this sandbox, reproduced identically on clean master 3d46070 and gone after `git fetch --unshallow`; not this PR's.
TRADEOFF ON THE RECORD: six chips joined the row, so the phone tab row went 2 wrapped lines -> 4 (tightened metrics under 560px pulled it back from 5). The 2026-08-04 directive "Thursday is flowing out of the screen" forbids a scrolling rail, so nothing hides. The dated d2-d7 chips are now largely redundant with This week / This weekend; removing them would take the row back to 9 chips — NOT done, the founder did not ask for it.
STATUS: OPEN.
