...
## Session Contract #26 (2026-07-25, `claude/moat-sources-importable` → PR #68 — founder: get the local-content moat actually importing, then "just get it done so I can begin sharing with others to gather feedback")

GOAL: make the 38 curated moat sources (venues, artists/orgs, all categories) genuinely fetchable and correctly categorized, on a twice-daily schedule with a dead-man alarm and budget caps.
SCOPE: source selection token widening (18 → 64 selectable), canonical feed discovery (link-rel autodiscovery → platform endpoints → conventional paths), a platform-JSON reader for Tribe/Localist, the curated cultural_domain → promote-time category signal, and the import-structured workflow. NON-GOALS: no gate/threshold change; no extraction-surface edit; no new outward-facing surface.
DONE-CRITERIA: validate green · evaluator APPROVE · scheduled run lands events with >1 category.
STATUS: IN REVIEW (r14). Events 85 → 112 → 168, categories 2 → 5 on the last live run. Fourteen evaluator rounds; blockers 3 → 1 → 1 → 1 → 1 → 1 across r9–r14. Rebased twice onto a moving master (#67 construction gate, then #69/#70) — the second time diagnosed from mergeable_state=dirty after `pull_request`-triggered checks stopped being created while the `pull_request_target` one kept running.
STAGE 3 CITATIONS (this PR's change surface — deliberate [S3:…] tags per the r6 gate semantics; each class retrieved from docs/memory/RED_CLASSES.md and answered against THIS build):
[S3:failure-reads-as-empty] THE class of this PR — six rounds. A source DENIED (403), THROTTLED (429), errored (5xx), refused by robots, or misconfigured is now a FAILED source, named, non-zero exit; only a guessed 404/410 is skippable. Newly indexed here so it is retrievable before the next importer is written, not after.
[S3:silent-data-loss] Three instances (Localist occurrences[0], integer ids colliding, bare JSON-LD read as zero). Class fix shipped: `_account()` makes every reader state objects-seen vs objects-that-produced-a-row, counted per INPUT event so occurrence fan-out is not an alarm. Newly indexed.
[S3:test-codifies-the-bad-contract] Three instances (r7/r8/r11) of MY most expensive habit: asserting the mechanism of my fix instead of the outcome the finding was about, so a green suite argued FOR the defect. Rule adopted and applied to the last commit before pushing. Newly indexed — the one I most want the next session to retrieve.
[S3:false-confidence-gate] r10 caught this in my own words: R-052 claimed "every fail-open path logs a WARNING" while stdlib RobotFileParser.read() swallowed a 404 silently. Claim now matched by `_fetch_robots_lines`, four red tests, and the RECORD row amended with the correction rather than rewritten.
[S3:fail-open-on-custody-misconfig] Same shape one layer out: a catalog row asserting two incompatible wire formats warned and sniffed. Now `CatalogContradiction`, checked before any fetch, all bad rows named in one pass, exit 2.
[S3:swallowed-corrupt-data] The closed skip-list is the mechanism: every status except a guessed 404/410 propagates, parametrized over 500/502/503/408/451/423/401/403/429 + DNS + TLS.
[S3:final-gate-trusts-generator] Untouched — this PR adds no promote/publish path; promote's assert_promotable still re-runs the full trust gate on its own snapshot.
[S3:release-path-weaker-than-generation] N/A to this change surface: licensed_event rows never enter the AI-extraction → candidate → gate → promote path. Stated rather than left silent.
[S3:featurability-dimension-missed] Provider provenance kept distinct per reader (ics / jsonld / platform_json, migration 0014) so a shape drift is attributable to the reader that caused it.
[S3:nonfinite-decimal-accepted] Prices pass through the shared normalizer unchanged; this PR adds no price arithmetic.
[S3:false-price-claim] No price copy in this change surface.
[S3:semantic-claim-not-rederived] The provider assertion IS a semantic claim, and it is now re-derived against the bytes (`_matches_asserted_shape`) rather than trusted from config.
[S3:fabricated-qualitative-copy] No outward copy; titles and categories come from source data or stay NULL (honest 'Other'), never a guess.
[S3:caller-suppliable-custody-inputs] No custody inputs in this surface; robots is evaluated for the UA we actually send, not a caller-supplied token (r7).
[S3:weak-key-accepted-at-custody] No keys in this surface.
[S3:volatile-safety-store] No safety counters in this surface; the run's failure tally is per-run by design and drives the exit code.
[S3:grant-not-content-bound] No autonomy grants in this surface.
[S3:deferred-trust-work] Nothing parked: R-052 (robots fail-open boundary) and R-053 (test-file split) both carry objective triggers, written in the same commit as the work they qualify.
[S3:retyped-evidence] Suite counts in commit messages come from the local run that produced them; the CI numbers differ (different base) and are not restated as mine.
[S3:governance-ambiguity] R-053 states precisely WHY the split was declined mid-review (diff noise over fixes under review; a botched split silently loses tests), not merely that it was.
[S3:self-weakenable-gate] This PR only ADDS rows to RED_CLASSES.md; no token renamed, no trigger narrowed.
[S3:rule-stronger-than-mechanism] The rule I adopted this round ships with its application: I ran the bad-input assertion audit on my own tests before pushing r11, and said what it found.
[S3:stalled-state-needs-active-diagnosis] Applied: when trust-gate went red on a suite my local run had passed, I probed the base rather than re-pushing — the cause was this branch being behind master's new construction gate.
[S3:stale-live-incident-state] The 168-events/5-categories figures are from the last live run, not from earlier prose; re-verified before restating.
[S3:incomplete-enumeration] Newly indexed (r14). Two hand-maintained lists with no invariant between them — what the selector FETCHES and what the table ASSERTS — drifted, leaving `jsonld_if_offered` classified but unfetchable: a token literally advertising JSON-LD that no row could be selected by. Collapsed into ONE `_TOKEN_SEMANTICS` table declaring both properties per token, with both former names as DERIVED views, and a PRODUCTION-OUTCOME test (one row per selectable token through the real main()) plus its converse.
[S3-green] Green examples retrieved and REUSED: `tools/kaizen_trends.py`'s repeat-class alarm (it blocked my r9 commit until the class fix shipped — the pattern of a gate refusing self-certification); `worker/promote.py`'s non-fabricating fallthrough (absent signal → NULL, never a guessed category) → `cultural_domain_for_source` returning None for unknown/untagged sources; `tools/pr_size_check.py`'s read-the-cap-from-the-workflow trick → the derivation test that reads catalog tokens rather than mirroring them by hand. No green example existed for provider-assertion handling — stated explicitly per the no-silence rule, and this PR's `_TOKEN_PROVIDER_CLASSIFICATION` is offered as the first one.
