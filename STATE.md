...
STATUS: OPEN — built, pushed, PR #229 (draft). Panel r1-r5, each one layer further out on ONE class: a hole reaching a reader as the wrong hole. All FIXED. r1: a re-run compares the desk's STATEMENT, not only the key. r2: a superseded row is marked DISPUTED. r3: the three shapes of a NULL clock are separated before they collapse into one display. r4: a failed dispute fails the RUN. r5: the contested clock is labelled by the PUBLISHER inside the insert's own transaction — competing instants now reach the gate as `start_times` evidence instead of being resolved away by the producer, and the walker's post-promote dispute call is deleted. R-110 and R-111 carry the two residuals, both with objective triggers.
