...
SCOPE:
  (1) Schema migration `0010` — extend `event` (title, category[=domain], subsegment, price_min/max, currency, is_free, ticket_url, image_url, source_provider, external_id, on_sale_status) + `venue` (lat, lng, address, area, postal_code); UNIQUE(source_provider, external_id) for idempotent upsert + cross-source dedupe. Additive/nullable only — cannot break existing RLS or reads.
  (2) Licensed-feed importer `worker/importers/{ticketmaster,seatgeek}.py` + `normalize.py` + `domain_map.py` — deterministic, non-AI, fetch CAPCOG-area events, normalize to canonical, upsert as confidence='confirmed' with provenance to `audit_log`; idempotent; unit-tested against captured JSON fixtures so it is fully built/tested WITHOUT the live key, then run live when keys land.
