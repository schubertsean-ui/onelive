...
[S3:pushed-on-red] [S3:env-dependent-hermetic-test] [S3:scripted-transform-order] [S3:deliverable-visual-qa] [S3:founder-path-unprobed] No tests, CI, or transform scripts change; trust_gate, lint, deferral_scan, staleness_check ran green on this tree; the PR opens as draft and merges only on the founder's word.
...
WHY: the list split landed (1565 Chronicle `/event/…` rows, mash_n=0, run 34000699427) and those rows carry Date TBA — a card with no day cannot be decided in one look.
WHY-IT-MATTERS: a hole filled by guessing is worse than the hole, so the only honest way off Date TBA is the page the desk itself pointed at; that is what makes Ticket E's display possible without inventing a day.
EXPECTED OUTCOMES: (a) list card + event page "Sat Sep 6 • 9:00PM" -> dated; (b) event-page clock with no date -> still NULL; (c) a list-page date never attaches to an event-page clock; live dry table rows_n/dated_n/still_null_n/403_n/mash_n with mash_n = 0 and 3 sample rows; no `--write`, no merge.
OUT OF SCOPE: Tonight redesign, catalog upsert, `ai_extract` rewrite, new vendor, login, Planomato, PRs #230/#231/#232, raising the 40-page list walk.
STATUS: OPEN — built and pushed, PR #235 (draft, NOT merged). `bash tools/validate` green on every blocking check (evidence block pasted in a PR comment; the two ADVISORY rows are historical commit_sweep churn on already-merged commits and construction_gate, which prints PASS on its own; visual_regression SKIPs for R-068 — this change renders nothing). Evaluator r1 REQUEST-CHANGES (openai/absence-only, 3 of 4 lenses APPROVE): no event-scope check, so a footer's "last updated" stamp joined to the show's own clock published `2026-09-03T21:00:00` — reproduced, then closed structurally (HTML sectioning, not a list of chrome words). Probing that fix found the same class one nesting level deeper (a card `<footer>` inside the page `<footer>` re-opened the page), and the first LIVE run found it a third time from the other side: scope was being checked AFTER cardinality, so any page with a footer stamp counted two dates and refused — 40 of 40. Scope is now asked first and the clock settles against its own statement. Evidence: `docs/evidence/2026-09-06_permalink-follow.md`. LIVE table, run 34006786824 on head 7925f00: rows_n 1568, dated_n 39, still_null_n 1529, 403_n 0, mash_n 0 — 39 of the 40 pages the budget reached were dated (1 stated no date), and the 1528 remaining are UNASKED rather than dateless. Samples: `/event/boeing-boeing-14285657` -> 2026-09-18T19:30:00-05:00 at TexARTS, `filled_from_detail=(when, place_text)`. Evaluator r2 (both openai seats, BLOCKING, both reproduced): a sidebar's schema.org node dated another row, and a same-origin redirect onto the desk's index became the field source — both closed by binding the statement to the row and the landed address to the asked one. The first cut of that bind was too blunt (dated_n 40 -> 11); the diagnostic printed the addresses those nodes name (/backtotheranch, /texarts_26_BB_ac, /events/269428 — the desk's own vanity links for the SAME happening), and the bind now asks the committed identity table whether the named address is ANOTHER happening. Evaluator r3 (openai/absence-only, BLOCKING, reproduced): `event_scoped()` asked `bool(mine)` — a page-level answer to a per-node question — so a permalink whose own node carries no `startDate`, beside a sidebar node for another happening, published `2026-12-25T20:00:00-06:00` with NO refusal code; scope is now decided per HIT against the bound nodes' own instants, and an unmatched hit refuses as `structured-hit-not-bound`. Two traps inside that fix were caught by re-running it rather than reading it: naive-vs-aware instant keys would have holed every desk omitting an offset (naive is now read as UTC, matching `_to_utc_z`), and a RELATIVE `url` bound nothing (addresses are resolved against the page first). LIVE table on the SHIPPED code, run 34011052450 on head f436fe3 (`--max-pages 3 --follow-budget 40`): rows_n 118, dated_n 38, still_null_n 80, 403_n 0, mash_n 0 — of the 40 pages the budget reached, 38 dated and 2 stated no date; 78 followable rows were never opened, so dated_n is a FLOOR. The identity correction is visible in the diagnostics: `structured-not-bound` 17 -> 3 and `date-in-plumbing` 14 -> 2 against the r4 head, i.e. the coverage the agreement test destroyed is back. Places improved as a side effect of the card boundary — rows that read 'Venue Details Dougherty Arts Center 1110 Barton Springs Rd., ... 10 events' now read 'Dougherty Arts Center'. (An earlier table, run 34007753540 on 9e3029a, read rows_n 120, dated_n 39, still_null_n 81, 403_n 0, mash_n 0 and predates rounds 4 and 5; the small `rows_n` drift between runs is the live desk's own list changing, not the code.) Reading that run's own output found two defects, neither in a published field: (a) the page-level `clocks-ambiguous` scan was recorded even when a structured carrier had already stated the whole instant, so the report claimed 7 of 40 opened pages (17%) needed a clock repair on rows that were complete — fixed and pinned from both sides; (b) "86 tests" was typed into the r3 commit message and the evidence doc where the derived count was 82. Evaluator r4 (openai/attacker-smuggle, 2 BLOCKING, both reproduced pre-fix; the other three seats APPROVE): (a) r2's weak arm read "the identity table cannot classify this address" as "therefore this node is ours", so a promotional Event at an unpatterned URL published `2026-12-25T20:00:00-06:00` at 'The Other Room' with `codes=()` — the arm now needs a POSITIVE half too (the page's own content states the day the node claims), and the refusal names which half failed; (b) a `<script>` payload was matched to a prose segment BY DATE, so a bound `startDate: 2026-09-06` took a box-office notice's clock and published `2026-09-06T10:00:00` with no refusal — a document-level carrier now owns no segment, which is what the code's own comment had claimed since r1. Two traps caught by running rather than reading: the corroboration first compared a UTC-normalised node day against locally-printed page days (wrong by one for every evening show west of Greenwich — both sides now read through `same_page_dates`), and the first fixture for (b) said "Sep 6" with no year, which resolves to no date, so the test passed without reaching the defect. THE LIVE RUN THEN REJECTED MY OWN r4 FIX AND WAS RIGHT (run 34008921609 on bb6380a vs 34007753540 on 9e3029a, identical breadth): `structured-not-bound` went 2 -> 17 pages (42% of those opened) and `date-in-plumbing` 1 -> 14, so 15 of 40 pages stopped being dated. The desk's own words named it — `/event/prodigal-sun-14267156`'s node states Sep 4 while its page displays Sep 6, both about Prodigal Sun, because a RUN of performances has more than one date. The evaluator asked an IDENTITY question and I built an AGREEMENT test; the check is now the identity test it should have been (the node's title must name what the page's own h1/title names, case/punctuation/spacing as noise, containment either way), which closes the evaluator's case more directly — its node calls itself 'Something Else' on a page headed 'Dominic Fike'. Every hermetic test passed on BOTH versions: the fixtures encoded my model of the desk, not the desk. THE r8 CHECK MET THE LIVE DESK AND THE DESK CORRECTED IT TWICE (run 34013696335 on e3f5d7f): the prediction held — `dated_n` 38 -> 23 with `card-contradicts-its-own-markup` on 15 of 40 opened pages (37%), and the cause was the predicted one, runs and multi-day festivals (`boeing-boeing` card states Sep 19/20/24 against markup Sep 18). Reading those sentences found the rule broader than its own message twice: (1) `card_days - node_days` fires on ELABORATION — a card naming three performances and markup naming one is the desk agreeing at different resolutions — narrowed to `node_days - card_days`; (2) the narrowed rule STILL refused because R-030 credits each date to the STRONGEST carrier that stated it, so a day the card prints AND the markup names comes back as `jsonld` and vanished from the filter — the card's days are now read FROM THE CARD, segment by segment. A VERIFIED PREDICTION IS NOT A VERIFIED RULE. What the residual 37% means is the NEXT run's to say, and this ticket's final number waits for it. Evaluator r9 (openai/absence-only BLOCKING, both gemini APPROVE, reproduced): the card rule's THIRD default, written the permissive way like the other two — a page printing no heading at all set the subject to None and `_inside_the_card` read that as 'exclude nothing', so an image-headed page with an unrelated promo `<section>` published that section's date and venue. A heading-less page with exactly ONE top-level card now takes that card as its subject; with two or more, only page-level statements count. The first fix was PAGE-LEVEL-ONLY and three existing tests caught it as too blunt before any live run could — the third time this ticket a fail-closed instinct over-corrected and the suite found it. Evaluator r8 (openai/attacker-smuggle, 2 BLOCKING, both gemini APPROVE): the STRUCTURED TIER WON WITHOUT ASKING WHETHER THE PAGE'S OWN CARD CONTRADICTS IT — a stale or series `startDate` published as the settled day while the page visibly stated another, and a node `location` overriding a contradictory visible venue. Fixed with the cross-tier conflict check r1 deliberately refused, and the reason it can be made now is the CARD BOUNDARY: `said` is no longer text somewhere on the page but this happening's own card, so a visible date there is the same desk contradicting itself rather than a competing tier. NEW RED CLASS `rule-outlived-its-reason` — r1's absolute was right about the page it was written for, and three rounds of scope work made a narrower rule possible. It SPLIT the r3 test rather than deleting it: the node still BINDS (its venue comes through) and no longer settles the DAY, because identity and certainty are different questions. Places compare with `_same_name`, so a card printing the venue WITH its address is not a contradiction. EXPECT `dated_n` TO FALL on the next live run: that fall is the measurement of how often this desk's markup disagrees with its own pages — most often a RUN, whose modelling is the next ticket's work. Evaluator r7 (openai/absence-only BLOCKING, both gemini APPROVE, reproduced): a STALE `<title>` could rescue a node the visible heading contradicts — `_headings` fed title and subject into ONE list and a node need only match ANY of them, so a poisoned Event naming a stale CMS title published 2026-12-25T20:00:00-06:00 at 'The Other Room'. The visible subject now wins outright and `<title>` is read only where the page prints no heading (its own test: a desk headed by an image must not lose every bind). The gemini NIT the same round had the SAME fix and is the FOURTH `one-rule-expressed-twice` instance, found by a reviewer ONE ROUND AFTER I opened that class: `_headings` was a regex over raw HTML while the scanners suppressed plumbing, so `<nav><h1>Browse Events</h1></nav>` was a name this page answered to. Headings now come from the segment scan — one walk, one precedence, one answer. Evaluator r6 (both openai seats BLOCKING, both gemini APPROVE, all three reproduced) found THE CARD BOUNDARY I ADDED IN r5 OPEN AT BOTH ENDS: (a) the subject was the first heading of ANY of h1/h2/h3, so a promo block placed ABOVE the real article became the page's subject and pushed the real content out of the card (published the promo's 2026-12-25T20:00:00 at 'The Other Room', codes=()); (b) a heading with no sectioning ancestor set the subject to None, which I had written as the permissive 'whole page is one card' — disabling the boundary entirely and letting an unlinked promo `<section>` supply the only date; (c) TWO DEFINITIONS OF THE PAGE'S HEADING HAD DRIFTED — the boundary treated h2 as a subject while `_headings` read only h1/title, so a page whose visible subject is an h2 had nothing to contradict a poisoned node with. Fixed: h1 wins with h2/h3 standing in only where no h1 exists; the card rule now carries BOTH clauses (the heading's sections must be a prefix of the statement's, AND a statement in any section when the heading is in none is not ours) because writing one alone breaks the other; both heading definitions read one `_HEADING_TAGS` precedence, with a page's subheadings excluded where it has an h1. NEW RED CLASS `one-rule-expressed-twice` — this is the third occurrence in this ticket (r3's UTC-vs-local keys, r4's heading sets, r6's). The review of 54247f2 earlier caught the SECOND HALF OF MY OWN r5 FIX: I built the card boundary for PLACES and left the DATE path reading every non-plumbing segment, so an unlinked promo block printing the only date and clock dated the row (`2026-12-25T20:00:00`, no date refusal at all). `_SegmentScanner` now carries the same numbering and subject-scope, so it is ONE boundary over both fields — the second time in this ticket that a fix followed its finding's FIELD instead of its finding's CLASS. It also reversed an older test toward MORE coverage: a page whose article states 'Sat Sep 5 - 9:00PM' beside a month grid is now dated, because the grid is outside the page's card — that is the exact shape that refused 40 of 40 pages on the first live run, and both directions are pinned (grid inside the card still refuses). Evaluator r5 (both openai seats BLOCKING, both gemini APPROVE, both reproduced): (a) a node naming THIS address was accepted on the address alone, so one claiming this permalink while calling itself 'Some Other Show' on a page headed 'Dominic Fike' published 2026-12-25T20:00:00-06:00 at 'The Other Room' with codes=() — and THE TEST COVERING THAT SHAPE WAS ASSERTING THE DEFECT, green through four rounds; the bound arm now asks only that the node not CONTRADICT the page (absence is not disagreement: no heading or no node name keeps the bind, its own test), while positive agreement is required only where there is no address to stand on. (b) R-112's residual, blocked by both seats and correctly — deferred-trust-work says a RECORD row is not a safe harbour when its stated bound does not cover the harm it names. THE CARD BOUNDARY R-112 CALLED IMPOSSIBLE WAS ALREADY IN THE HTML: the page's own heading sits in a sectioning element, that element is the card, a labelled place outside it is about something else — the same vocabulary the date path's scope rule had read since r1. R-112 RESOLVED in the same PR that recorded it; r4's other-happenings rule kept for the sectionless page. An attempt to close (b) by deleting the unbound place fallback broke 18 tests incl. the founder's own case (b), which was the useful answer: the defect was never the fallback but the absence of a boundary around it. The review of bb6380a had BOTH openai seats name the same defect independently — confirmation rather than my own rationalisation — plus a half still open: the arm for a node naming NO address never faced the identity check, because r2 had accepted it as 'a permalink page publishing an Event about itself'. A promotional node that omits `url` is exactly as unidentified as one carrying a vanity link, so the address check is gone from that condition entirely and every lone node faces the same question. That broke three fixtures for two reasons both worth keeping: two nodes were named 'A' (containment either way makes a single letter match 'A Show' and every heading with an article in it — the comparison is now on whole WORDS with a floor of one two-character token or two tokens), and one fixture page carried no heading at all. The review of the next head also found the SAME CLASS ONE FIELD OVER (openai/attacker-smuggle, BLOCKING, reproduced): four rounds hardened the DATE path and left the PLACE fallback taking 'the one labelled venue anywhere in the content', so a related card's `<div class="venue">` published as this row's place with codes=() — tied now by the split ladder's own discriminator (the card links to another happening's permalink, which the committed table matches), collected in the SAME scanner pass that finds the places so there is one definition of this page's content. A first attempt filtered per SEGMENT and broke 13 tests at once: `segments()` returns TEXT, so a scanner needing venue markup finds nothing in it. Residual recorded as R-112, not implied: a promotional block with venue markup and NO link is still read as this row's place, and closing that needs a CARD boundary this module does not have (the two substitutes — a chrome-word list and a title match — are both refused on the record). Evaluator r10 (openai/attacker-smuggle, 2 BLOCKING, both reproduced on head 6449e63): (a) r8's cross-tier check compared DAYS ONLY, so a card printing 8:00PM above markup stating 19:30 agreed about the day and published a precise time the visible page contradicts — the card's printed wall clock is now compared against the node's, LOCAL as written (converting to UTC first would compare 00:30 with the card's 8:00PM and call every Central-time desk a liar), and since the day is agreed the DAY STANDS while only the clock is holed. (b) `_same_name` normalised with `[^0-9a-z]+`; the seat reported the LOOSE half ("Кино Night" -> "night", matching an unrelated node called "Night") and reproducing it found the TIGHT half, unreported and worse — a page headed "Кино" alone collapses to the EMPTY STRING, so no node can ever match it, every lone node refuses and every bound node reads as contradicting the heading. A desk in Cyrillic, Greek, Chinese, Hebrew or Thai could not fill one field: A LOCALE REFUSED IN CODE, which Coverage Law forbids outright, invisible because every fixture and every desk in this repo is English. THE CLASS WAS ALREADY WRITTEN DOWN WITH MY EXACT INPUT AS ITS EXAMPLE: `destructive-normalization` (PR #214, three rounds deep) names "Кино Night -> night" and "Café -> caf" verbatim, and the remedy sits two modules away in the same package. So the fix is an IMPORT, not a new tokenizer — `desk_union._hard` exported as `name_key`, `_name_tokens` is `name_key(name).split()` — and my hand-rolled first pass (`str.isalnum()`) is recorded as wrong too: it closed the erasure half and left "Café du Nord" and "Cafe du Nord" as different venues, WHICH MY OWN TEST THEN ASSERTED as the expected answer. Residual recorded as R-113 with its blast radius rather than tightened blind: the over-match half (a node named "Night" binding into a page headed "Кино Night") stays, because every available tightening — leading-token requirement, token-ratio floor, stopword list — costs coverage this desk uses, and a chrome-word list is refused on the record at r1; its mirror, scriptio continua (`東京` does not bind into `東京ホール` by containment, though exact match binds), is pinned in the test both ways. `do512-today` stays walled at its list page (403, class D, queued). Known and not fixed here: where no structured node speaks, the place is the page's whole labelled venue block — honest but not a usable Place; place-text normalisation is the neighbouring ticket. FINAL LIVE TABLE, run 34015650549 on the shipped head 7676b59 (`--max-pages 3 --follow-budget 40`): rows_n 109, dated_n 34, still_null_n 75, 403_n 0 (1 for the walled do512), mash_n 0. Of the 40 pages the budget reached, 34 are dated and 6 state no date; the 69 unopened rows are UNASKED, so dated_n is a floor and still_null_n a ceiling. Samples: `/event/back-to-the-ranch…` -> 2026-09-26 at Lyndon B. Johnson National Historical Park, `/event/texas-renaissance-festival-14311742` -> 2026-10-10T09:00:00-05:00, `/event/zz-fest-w-amplified-heat…` -> 2026-09-12T14:00:00-05:00. THE r10 CLOCK FIX FIRED ON A REAL PAGE ON ITS FIRST LIVE RUN — sample 1 carries a date and no clock because the card prints 10 pm while the markup states 18:00, so the agreed day survives and only the contested clock is a hole; before r10 that row published 18:00 under a page printing 10 pm. The r8 correction is now MEASURED rather than predicted: dated-of-40 went 38 (r7, no check) -> 23 (r8 as first written) -> 34 (r8 corrected + r10), and `card-contradicts-its-own-markup` 37% -> 12%. Three quarters of what r8 refused was the desk agreeing with itself at two resolutions; the residual 5 pages are genuine disagreement — runs and multi-day festivals, which the pipeline has no model for, and that 12% is the input the run-modelling ticket needs. Evaluator r11 (both openai seats BLOCKING, both gemini APPROVE, both reproduced on fc6cd6c) — BOTH FINDINGS ARE IN CODE THIS TICKET WROTE, ONE ROUND AFTER WRITING IT. (a) r10's clock check compared the node against `page_clock`, which is None the moment a card prints TWO clocks, and the multi-clock diagnostic beneath it is suppressed once a carrier states the whole instant (this ticket's own diagnostics-as-data fix) — so the gap sat exactly between the two rules and a card printing "8:00PM; doors 7:00PM" beside a 19:30 node published 19:30, neither of the times on the page, with codes=(). Fixed by MEMBERSHIP rather than equality: the card's clocks are the times the desk says are involved and the markup says WHICH one starts the show, so one of them being the node's is agreement at two resolutions (the r8 lesson applied to clocks) and none of them being the node's is contradiction; unresolvable clocks are dropped rather than counted against the node, and `_clocks_printed` is split out so `_clock_claim` and the check cannot drift. A page printing "7:30PM; doors 7:00PM" beside a 19:30 node is now DATED — coverage that used to be lost to `clocks-ambiguous` for no reason. (b) `_address()` dropped the URL query while `desk_read._identity_of` KEEPS it and says why in its own docstring ("two desks do use `?date=` to address two instances of one series, and collapsing those would delete a night"), so on a run-addressing desk every night shared one address here: a redirect from one night to another passed `same_identity` and a node naming another night spoke for this one. The query now joins the address and r2's `?ref=` tolerance moves to `same_identity` where it can be ASYMMETRIC — every parameter the ASKED url carries must come back unchanged, added ones are tolerated, because an added parameter cannot change which happening was addressed and a changed or dropped one can; `_shown()` puts the query in the refusal message too, or a correct refusal on such a desk reads like a bug. FIFTH `one-rule-expressed-twice` in one ticket, and the worst-lit: the other half was not merely already written but already JUSTIFIED IN PROSE one import away, naming the exact harm. Both new tests PROVEN to fail on the pre-fix head and pass after. THE SHAPE BOTH r11 FINDINGS SHARE, which no question I had been asking catches: each rule was CORRECT ABOUT THE EXAMPLE IN FRONT OF ME and silent one step outside it — one clock became two, one query value became another. Evaluator r12 (both openai seats BLOCKING, both gemini APPROVE with one NIT; all three reproduced on bd5b2b9). (a) `owned_by_this_happening` had a third arm — a hit whose DAY appears among the card's days is ours — and A DAY IS NOT A FINGERPRINT: R-030 credits each date to the STRONGEST carrier that stated it, so a card printing a bare "September 18, 2026" beside an unrelated sidebar node naming 2026-09-18T23:00 yields ONE `jsonld` hit carrying the SIDEBAR'S CLOCK, and that arm called it ours — the other show's time published as this row's, beside a `structured-not-bound` refusal saying none of the nodes dates this row. Fixed in two parts because DELETING THE ARM ALONE LOST A REAL PAGE and the suite said so in one run: `jsonld` is refused outright (the comment has claimed since r4 that a `<script>` payload owns no statement; now it is a line) and `card_dates()` reads the card's own segments, because the arm was also recovering the card's printed date when the document-wide scan lost it — a page printing "Sat Sep 5 • 9:00PM" beside a 31-cell calendar widget comes back from the whole-document scan with the widget's days and not its own. THIRD time in this ticket a document-wide scan had to become a per-card one. The fallback is gated on the r8 contradiction flag after r8's own test caught it re-supplying the card's half of a disagreement. (b) THE CARD BOUNDARY FAILED CLOSED OR OPEN DEPENDING ON MARKUP STYLE: r9's `()` means "page level only, every section excluded", which is restrictive on sectioned markup and its exact opposite on `<div>` markup, where there are no top-level sections and every statement sits at page level — an unrelated promo `<div>` on a title-only page published 2026-12-25T20:00:00 at 'The Other Room' with codes=(). `()` and `None` are now two answers (page level IS the subject vs no subject identified). Blast radius stated before landing: the VISIBLE-TEXT path is refused on such a page and the STRUCTURED path is untouched (a title-only div-soup page publishing its own schema.org node still fills both fields, pinned); three fixtures that broke were scaffolding for other rules leaning on the permissive default and now carry a heading. (c) the gemini NIT was a coverage regression R11 ITSELF INTRODUCED — `field_read` took the LANDED url, harmless while `_address` dropped the query and a hole the moment r11 stopped, so a desk redirecting to `?ref=cal` made every node naming the canonical address stop matching; the row's own address is the identity inside `field_read`. SIX FINDINGS ACROSS r10–r12, FIVE IN CODE THIS TICKET WROTE, EVERY ONE THE SAME SENTENCE: a rule correct about the example in front of me, silent one step outside it. THE LIVE TABLE IN §13l IS SUPERSEDED — the date-ownership and subject rules both moved, and a fresh run on the shipped head is what this ticket's final number waits on. Evaluator r13 (both openai seats BLOCKING, both gemini APPROVE; all three reproduced on d7a5c3e). (a) THE FINDING IS THE RECORD I WROTE AT r10: R-113 declined to tighten single-token name containment and bounded the risk as "a wrong FIELD on a page that is otherwise the RIGHT happening" — reproduced, a lone node called "Night" on a page headed "Jazz Night" BINDS and publishes 2026-12-25T20:00:00-06:00 at 'The Other Room' with codes=(), on a page stating no date at all: a FABRICATED INSTANT, a different harm class from the one the row described. Second `deferred-trust-work` in this ticket (R-112 at r5 was the first, and both seats blocked on that one too). The fix is the option R-113 DECLINED, narrowed to the case carrying the defect — a single-token name must OPEN the longer name, multi-token containment unchanged anywhere in the string — which answers every objection the row raised, because all of them were multi-token ("Live at the Continental Club" vs "Continental Club" still binds; "Gandahar" opens "Gandahar (1988)"; "Кино" opens "Кино Night"). R-113 RESOLVED with its bound corrected on the record. (b) `PLACEISH_RE` was a bare alternation, so `placeholder` and `replacement` read as the page labelling a venue and an empty layout div inside the event's own card became its place; now a WORD test with camelCase split first (`venueName` still reads), one definition still shared with `desk_read`. (c) `same_identity` still tolerated ADDED query parameters on my r11 argument that a query-less asked url proves the desk's identity is in the path — but a list page can link a RUN's page rather than a night's, so `/event/show` -> `/event/show?date=2026-09-19` is the desk choosing a night for us; host, path and query now match exactly. COST STATED: a desk redirecting with a tracking parameter is queued with its reason rather than read, because separating a tracking parameter from an identifying one needs a registry of parameter names (the r1 chrome-word list one domain over), and the `?ref=` case was always a hypothetical no run has seen. ACROSS r10–r13: nine findings, seven in code this ticket wrote, every one a rule correct about the example in front of me and silent one step outside it; and (c) is the r11 argument being wrong for the THIRD distinct reason, which is the signal that the argument was never sound. Evaluator r14 (openai/absence-only BLOCKING and reproduced on 6b5b4ab; the other THREE seats APPROVE — the first round this ticket where three of four cleared): the r12 clock comparison resolved each printed token with `resolve_same_page_datetime(token, block_text=said_text)`, which needs a DAY to make an instant, so a card printing "Show 8:00PM" and NO date produced an empty comparison set and the node's 19:30 published as settled under a page visibly saying eight o'clock. My own comment beneath it ("an unreadable statement is not a contradicting one") conflated two things — "8:00PM" is perfectly readable as a wall clock, it simply has no day of its own. Each printed clock is now anchored to the NODE'S day (scaffolding for the parser, never a claim; the days are already known to agree because r8 empties both tiers when they do not), and anchoring rather than writing a clock regex keeps the ARMED `same_page_dates` as the one reader of what "doors 7 pm" means. THE FIX BROKE THE LIVE TABLE'S OWN ROW AND THE COST IS PINNED RATHER THAN ABSORBED: `/event/boeing-boeing-14285657` prints "Matinees 4:45 pm. Late show 10:15 pm." beside a node stating 19:30 — those clocks could not be anchored before, now they can, the node's is none of them, so its clock holes and the day stands. Kept as ONE rule rather than special-cased, because it is the same answer r8 gave for DAYS and separating "matinees/late show" from "doors/curtain" needs the label-word list refused at r1; the honest price of never publishing a time the visible page does not state is holing the clock on a desk that describes a RUN. EXPECT `dated_n` TO FALL AGAIN, and that fall is this ticket's measurement of how much of this desk is runs. Evaluator r15 (three of four seats APPROVE; the one blocking finding DOES NOT REPRODUCE): openai/absence-only says a same-day carrier outside the card can supply this row's clock "because the check accepts the carrier by matching only the date to the card" — that check was REMOVED at r12 for exactly this class, and five shapes of the attack (sibling article, aside, body level, visible prose in a sibling card, page footer) all already behaved on the reviewed head. Answered in writing rather than coded, WITH the seat's own nit taken, because the invariant is real even though the defect is not and it was not PINNED: a 5-shape regression test, proven non-vacuous by restoring the arm r12 deleted (it goes red). Both doc nits taken (§13l retitled HISTORICAL, a duplicated bold run removed). THEN THE LIVE RUN OF r14 FOUND TWO DEFECTS IN MY OWN LAST TWO ROUNDS, neither reported by any seat: (a) `/event/boeing-boeing` was reported as contradicting itself because its card prints "7:30" while its markup states 19:30 — AND 19:30 IS HALF PAST SEVEN; a bare clock FACE was anchored as 07:30, so the desk was called a liar for agreeing with itself. A token carrying no am/pm now agrees with either reading (an ambiguous statement is not a contradicting one — the principle r12 stated and r14 applied backwards); a token that says am or pm is still compared exactly, so r10's finding is untouched; cost stated (a bare "9:00" no longer contradicts 21:00 markup). (b) the same page's PLACE holed because its card labels "Venue Details TexARTS 1110 S RR 620, …" against a node saying "TexARTS" — r13's leading-token rule arriving in the place check, where a false NO invents a contradiction; split into `_names_within` (containment) vs `_same_name` (strict), which is the lesson `destructive-normalization` r9 already recorded in this repo and I did not read at the moment it applied. FINAL LIVE TABLE, run 34019858312 on 52a5e30 (every rule through r14): rows_n 120, dated_n 33, still_null_n 87, 403_n 0 (1 for the walled do512), mash_n 0; of the 40 pages opened, 7 stated no date and 80 were never asked. The r15 corrections above RELEASE clocks and places that run holed, so 33 is a floor in one more way than the budget already makes it. dated-of-40 across the arc: 38 (r7, no check) -> 23 (r8 as written) -> 34 (r8 corrected + r10) -> 33 (r12 + r14). Evaluator r16 (three of four seats APPROVE; openai/attacker-smuggle 2 BLOCKING + a gemini NIT, all three reproduced on c70f547). (a) THE CARD BOUNDARY HAD NO INSIDE: `sections[:len(subject)] == subject` is a prefix test — deliberate at r6 so a card's own `<section class="details">` keeps its date — and it also made a promo card NESTED inside the main `<article>` after the real `<h1>` this happening's, publishing 2026-12-25T20:00:00 at 'The Other Room' with codes=(). Closed by the split ladder's OWN discriminator, the one r4 already uses for places: a nested section that links to another happening's permalink (committed identity table) is excluded with its subtree; only sections nested INSIDE the card are eligible, or the article's own "see also" link would mark the whole article foreign. RESIDUAL OPENED AS R-114 — a nested block with NO link is R-112's residual one level in, and the row is bounded AGAINST THE HARM rather than the mechanism (the lesson from R-112 and R-113, both of which this ticket had to reopen), naming the four live guards between it and a wrong field and the one shape that survives all four. (b) `event_scoped` returned True for every `ics` hit because "a calendar file served at this address is this happening's" — true of the case that sentence was written for, false of the one it guarded, since `same_page_dates` runs over the whole DOCUMENT and a DTSTART inside an HTML page (a download widget, an add-to-calendar block) was scoped unconditionally and won the tier over every card and plumbing check. The test is now what the BODY IS (`BEGIN:VCALENDAR`). THIRD time in this ticket a comment claimed a mechanism the code lacked, and all three had the same tell: a sentence justifying a rule by describing ONE input. (c) NIT taken — `_names_within` is symmetric, so a desk whose markup carries the address beside a card printing the plain venue no longer reads as contradicting itself. Housekeeping: `_href_of` is one definition of "is this a link" for both scanners; the now-unused `_labelled_places` is deleted. Evaluator r17 (three of four seats APPROVE; openai/absence-only BLOCKING and reproduced on 8b700c0) — AND THE FINDING IS THE SHAPE OF THE WHOLE TICKET HELD UP TO ME. Seventeen rounds of rules decide whether a STATEMENT belongs to THIS PAGE (plumbing, card boundary, node bind, cross-tier checks, nested-card exclusion) and NOT ONE asked whether the PAGE belongs to the ROW: the identity ladder chose the address, nothing checked that the page which answered is about the happening the list card named, so a stale or recycled permalink headed "Some Other Show" filled a row titled "Dominic Fike" with 2026-12-25T20:00:00 at 'The Other Room' with codes=(). Closed with the node check's own rule one level up, reusing `_same_name`: ABSENCE IS NOT DISAGREEMENT — a row with no title or a page with no heading denies nothing, and only two names that both exist and name nothing in each other are a denial, so a card reading "The Yellow Wallpaper" beside a page headed "Trinity Street Theatre presents The Yellow Wallpaper" still reads. Asked PER ROW in `apply_read` (the page's names ride on the `FieldRead` so the question uses the walk `field_read` already did), and a denied row is QUEUED with both names because a hole with no reason is the diagnostics-as-data defect this ticket opened. MY FIRST DRAFT PUT THE CHECK IN `field_read` — which knows the page and not the row — so two cards sharing an address left one title unchecked, and making it fail-closed over both then refused the page for a row it was plainly about; the suite caught it in one run, the fourth over-correction this ticket found by existing tests before a live run could. Three scaffolding fixtures were given titles matching their pages (same shape as r12). Evaluator r18 (three of four seats APPROVE; openai/absence-only 2 BLOCKING, the second echoed as a gemini NIT; both reproduced on b74c3df). (a) THE FINDING IS R-114, OPENED ONE ROUND EARLIER AND BLOCKED ON THE NEXT — the third time in this ticket a recorded bound was reopened by the very next review (R-112 at r5, R-113 at r13, this at r18), and the sharpest form: THE ROW WAS WRONG ABOUT ITS OWN TRIGGER. It said the sub-card test needed "a per-card DOM subtree … which would let a nested block be asked whether it carries its own heading" — and both scans ALREADY RECORDED every heading with the sections enclosing it, so I described a capability the module already had as one it lacked and used that to justify not fixing a defect I had just reproduced. Closed with HTML's own outline rule (a sectioning element with a heading starts a section of the outline) plus the name check this module already makes twice, now a third time one level in: a nested section carrying its own heading and naming something this page is not about is a card in its own right, and neither its date nor its place is read. COST STATED AND PINNED: a subsection headed with a LABEL rather than a name ("Details", "When", "Tickets") is excluded too, so a page whose ONLY date sits in one loses it — paid for never publishing a neighbouring show's date, and the next live run measures it in `date-in-plumbing`; headless subsections unchanged (r6). R-114 RESOLVED with its trigger corrected on the record. (b) r17's denial returned the row with `detail_url` set and `null_reasons()` checks `detail_url` before the queued list, so a stale or recycled permalink was filed under "page stated no date" instead of "page could not be read" — an identity failure reported as a desk's silence; the row is now returned untouched and `follow()` queues it with both names. THAT FIX ALSO REVEALED r17 WAS UNDER-TESTED: two fixtures went red only once `detail_url` stopped being set, because the r17 denial had been masking their title mismatch. `_PlaceScanner` gained heading-TEXT capture so it asks the sub-card question about ITS OWN section ids rather than trusting two numberings to agree. Evaluator r19 (BOTH openai seats BLOCKING, both gemini APPROVE, both reproduced on 65c7fa2) — AND BOTH FINDINGS ARE ONE RECORDED LESSON EACH, IN A PLACE I APPLIED IT SOMEWHERE ELSE AND STOPPED. (a) `_same_name`'s loose containment served the POSITIVE bind and the DENIAL, whose dangerous answers point opposite ways — a page headed "Dominic Fike at The Other Room" carrying a lone JSON-LD Event named "The Other Room" passed the positive check (the node names the VENUE inside the heading, not the happening) and published 2026-12-25T20:00:00-06:00 at 'The Other Room' with codes=(). That is `destructive-normalization` r9's own rule, applied at r15 to split `_names_within` out for places and never swept for. `_claims_this_name` split out: a claim must OPEN the name it claims to be or be it exactly — r13's single-token rule, which was always the general rule and had been applied to one arity. Strict for `_names_this_page` and the sub-card test; LOOSE kept for the two denial checks, so the `<presenter> presents <title>` shape a desk really prints stays readable because it reaches the denial side. (b) `_headings` returned EVERY heading at the strongest level while `_pick_subject` decides which is the page's SUBJECT — so a stale page headed "Some Other Show" with a nested related `<h1>Dominic Fike</h1>` answered to both names, the r17 row/page check passed on the nested one, and the stale page's fields filled the Dominic row; `_headings` now reads the subject and the card by the same walk and the same `_foreign_sections` exclusions everything else uses (EIGHTH `one-rule-expressed-twice` in this ticket). THE SWEEP WAS THEN DONE IN THIS ROUND RATHER THAN LEFT TO r20: every helper in the module with two or more call sites was asked the r15 question, and the table is in evidence §13u — nothing else is shared across callers whose failure modes disagree. Rounds 15–19 produced five findings and THREE were a lesson already recorded in this repo, applied once and not swept; writing the lesson is not the work, sweeping the tree for it is. Nothing written; the founder merges.

**construction_gate Stage 3 (founder-capped at 5 lines):**
[S3:swallowed-corrupt-data] [S3:fail-open-on-custody-misconfig] [S3:grant-not-content-bound] [S3:condensed-thinking-run] Nothing is filtered away silently: every page the tick opens lands in a printed bucket with a stable CODE and up to six of the pages' own sentences beside it, which is how the live run's `structured-not-bound` was traced to the desk's vanity links rather than dismissed as noise; the plan table's 25-row cap is on PRINTING only and states its own total. Every new refusal here fails CLOSED: an unreadable identity table raises, a node that binds to nothing yields no fields rather than the page's best guess, and a landed address that is not the one asked for is queued rather than read. Nothing grants a statement standing it did not earn from its own content — that is the round-2 class itself. No thinking-tool output is summarised in place of its artifact: the two findings are quoted verbatim with their reproductions, in the evidence doc and the ledger.
[S3:whose-statement-is-this] Round 2's class, and the fix binds BOTH ends rather than the one that was reported: a structured node speaks for this row only when it does not name a different address, and the page that ANSWERS must be the address we asked for (host+path, so a desk's own `?ref=` is still the same page). The r1 tier fix caused the first half — precedence without asking whom the statement is about — and the first cut of this fix left the unbound node holding the scope exemption after removing it from its tier, caught by re-running rather than trusting it. Round 4 was the same class again, from the other direction: a NEGATIVE test (not in the table) standing alone admits everything the table has never heard of, so the weak arm now carries a positive half beside it — and round 4's second finding is `page-scope-read-as-event-scope` recurring as A COMMENT THAT WAS NOT A MECHANISM, the prose claiming a rule the line beneath it did not apply. Round 3 was the same class at a finer GRAIN: `speaks_for()` answered per node and `event_scoped()` then asked `bool(mine)`, so any bound node licensed every JSON-LD date on the page; scope is now decided per HIT by matching the node's own instant, an unmatched one refuses as `structured-hit-not-bound`, and the class carries that trigger. 15 tests.
[S3:page-scope-read-as-event-scope] [S3:stale-live-incident-state] [S3:final-gate-trusts-generator] [S3:db-type-mismatch-invisible-to-hermetic-tests] [S3:excluded-surface-widening] The class this ticket OPENED (evaluator r1) is answered by the code that closes it, not by prose: scope is asked before cardinality, plumbing is excluded structurally, and the clock settles against its own statement — three tests per rule. Every live claim here is re-read off the run that produced it (the dispatched workflow's own log), never restated from an earlier round; no gate trusts this generator (`promote_candidate` is untouched and nothing is written); no schema, type or query changes, so the hermetic-test blind spot has nothing to hide; no scanner exclusion or allowlist is widened.
[S3:missing-cardinality-check] [S3:pagination-integrity-gap] [S3:partial-write-whole-row] [S3:destructive-normalization] [S3:semantic-claim-not-rederived] [S3:hygiene-narrows-coverage] The tick's three subjects are all cardinality: two dates, two clocks or two places on one page is a REFUSAL with its reason, never the first of a list (tested as the zero/one/many three-way split); the 40-page budget is a floor, so unasked rows are counted apart from dateless ones in their own table; `when`/`when_precision`/`when_text` are written as ONE unit so a filled row is coherent, not merely filled. Normalisation: CRLF->LF on the page body, so a line-anchored ICS rule can see a line it already knows. The NAME comparison was destructive and is corrected rather than claimed clean — `[^0-9a-z]+` erased every non-Latin script (r10, evaluator; the class's own worked example was my live input), and names now compare through the reviewed `desk_union.name_key`, which folds Latin diacritics onto their base and keeps every other script whole. One normaliser in this package, not two. On-origin is a rule about which URL is OPENED: no row is dropped, no door demoted, the catalog is exactly as wide after this as before.
[S3:build-before-plan] [S3:contract-scope-violation] [S3:permission-for-ratified-work] [S3:status-narration-not-progress] [S3:governance-ambiguity] [S3:founder-verbatim-corrected] [S3:stalled-state-needs-active-diagnosis] [S3:founder-path-unprobed] Scope is the founder's five Must-dos; this contract was written before the first product edit and In scope / Refused is printed in the PR body. The founder's example string "Sat Sep 6" is kept VERBATIM as its own test and is not edited into the fixture: Sep 6 2026 is a Sunday, so the weekday checksum refuses it, and that is reported as a finding rather than silently corrected. The sandbox's 403 is diagnosed with the one command that fixes it (a dispatch of desk-split-dryrun.yml), not waited on.
[S3:heal-drops-guard-marker] [S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:self-weakenable-review-model] [S3:untested-gate-branch] [S3:rule-stronger-than-mechanism] [S3:workflow-tool-version-skew] [S3:mutable-model-alias] [S3:unusable-credential-tier] [S3:weak-key-accepted-at-custody] [S3:volatile-safety-store] [S3:stale-base-widens-range] [S3:green-on-stale-base] [S3:pushed-on-red] [S3:env-dependent-hermetic-test] [S3:caller-suppliable-custody-inputs] [S3:release-path-weaker-than-generation] [S3:nonfinite-decimal-accepted] [S3:nonfinite-numeric-accepted] [S3:compounded-ground-contrast] This round REMOVED a condition from a guard (`named and ...` in `speaks_for`'s lone-node arm) and that direction is the safe one: the check now fires on every lone node instead of only those naming an address, so nothing lost a marker — the guard got wider, and three tests that previously passed now assert the stricter answer. The one numeric input, `--follow-budget`, is checked as a non-negative int at both the CLI and the module and cannot be a float, a string or a negative; no custody input is caller-suppliable (the identity table and the pattern host bound are read from committed data, and the URL opened is the ROW's own address, never one this tool composes); no release path is weakened because nothing here publishes. No gate, threshold, reviewer binding or model route is touched, and nothing here relaxes one: the tick ADDS refusals where the list card left a hole. `worker/same_page_dates.py` is an ARMED-CRON file — imported, never edited, closure re-computed unchanged (42 files, `desk_follow` absent). Every branch of the new code has a test including the wall (all five statuses), the redirect, the budget and the fetcher that raises; all 47 are hermetic (no socket, no DSN, `as_of` injected). `bash tools/validate` ran bare on the committed tree; base is origin/master tip 01ea243 as the gate's own freshness line printed it.
[S3:fabricated-qualitative-copy] [S3:false-price-claim] [S3:copy-outruns-registry] [S3:featurability-dimension-missed] [S3:deliverable-visual-qa] [S3:rule-outlived-its-reason] [S3:one-rule-expressed-twice] [S3:retyped-evidence] [S3:diagnostics-as-data] [S3:stale-redclass-count] [S3:malformed-ledger-row] [S3:missing-record-read-as-state] [S3:deferred-trust-work] [S3:records-sharing-one-trigger] [S3:parallel-record-id-collision] [S3:scripted-transform-order] Every published field stays the page's own word: no descriptor, price or qualitative claim is generated, and provenance rides on the row (`detail_url`, `filled_from_detail`) so a filled hole can never read as something the list card printed. `rule-outlived-its-reason` is the class r8 opened, and this contract is its founding case: r1's 'never mix tiers' was justified by a specific live failure (a month grid outvoting a node on 40 of 40 pages), the card boundary fixed that failure three rounds later, and the absolute stayed — hiding the desk contradicting itself about the same show. The rule is now narrowed to what its reason still supports, and the class says to re-ask any absolute whose comment cites a past failure. `one-rule-expressed-twice` is the class r6 opened and this contract's own third instance: the card boundary and the identity check each decided what 'the page's heading' means, and the two drifted until an `<h2>`-subject page had nothing to contradict a poisoned node with — both now read one `_HEADING_TAGS` precedence, as the instant keys were made to read one `same_page_dates` convention at r3. `diagnostics-as-data` is the class the r3 LIVE run opened and this contract's own report caused: a refusal code explains a HOLE, and recording the page-level `clocks-ambiguous` scan on a row a structured carrier had already given a complete instant made the table claim 7 of 40 opened pages needed a clock repair when nothing was missing — no published field wrong, and still the count Law §9.3 picks the next ticket by; it now records only when the time is absent, tested from both sides. `retyped-evidence` ESCAPED this contract's own claim and is corrected rather than restated: "86 tests" was typed into the r3 commit message and the evidence doc where the derived count was 82, so the doc now derives it ONCE with the command printed beside it and every other section refers to that instead. No ledger count is restated, no RECORD id opened or shared, and no trust-path gap is deferred. No deliverable is rendered by this change — the deliverable is a markdown table.
...
HOW: new pure `worker/locale/identity_patterns.py` loader; `_select_rows` in `worker/locale/desk_read.py` replaced by the ladder; the banned class-substring splitter and the "innermost dated" guess DELETED; `listing_selectors` become pack data; mash/unsplit counters on read + walk + the dry table.
WHY: the live desk-ingest run mashed 40 Chronicle pages into ONE row keyed on the site root; every coverage number downstream describes pages, not rows a friend can see.
WHY-IT-MATTERS: until split is real, more desks and more write runs only publish blobs — and a blob cannot carry Place, Actor, kind, via, or a next step, so display and Heartbeat are both blocked behind it.
EXPECTED OUTCOMES: two `/event/...` links -> two rows with those listing_urls; an unidentified blob -> ZERO rows + `unsplit`; the old blob-accepting tests go red and are rewritten; dry ingest prints rows_n/unsplit_n/mash_n/403_n; no `--write`, no merge.
OUT OF SCOPE: Tonight redesign, catalog upsert, `ai_extract`, new vendor, login, Planomato, PRs #230/#231/#232, branch `grok/chronicle-event-href-rows`.
STATUS: OPEN — built and pushed, PR #234 (draft, NOT merged), head 9b07745: CI green, mergeable_state clean, adversarial-review APPROVED after three rounds (r1 a single-identity page read from its wrapper; r2 patterns matching a path PREFIX so an event's subpages became events, and any host in the table declaring identities on any page; r3 an identity link in the page's own plumbing). A red db-integration on f20effb was a pre-existing clock flake in three integration tests (the ingest key carries the local NIGHT; a correction crossing midnight changes the key — 225 of 1440 UTC minutes a day), fixed here rather than waited out. Evidence: `docs/evidence/2026-09-05_identity-split.md`. The live acceptance numbers are NOT in this PR: the sandbox proxy 403s both desks, so they need a dispatch of `.github/workflows/desk-split-dryrun.yml` on this branch.

**construction_gate Stage 3 (founder-capped at 5 lines):**
[S3:build-before-plan] [S3:contract-scope-violation] [S3:permission-for-ratified-work] [S3:status-narration-not-progress] [S3:governance-ambiguity] [S3:stalled-state-needs-active-diagnosis] [S3:founder-verbatim-corrected] Scope is the founder's five Must-dos; the contract above was written before the first product edit and In scope / Refused is printed in the PR body. The ladder's ONE judgement call — rung 1 is not an HTML splitter, so a promoted JSON-LD Event cannot swallow thirty printed cards — is stated in the reader's docstring, the commit and the PR body rather than resolved silently; the law's text is quoted, never narrowed. The live table's absence is diagnosed (proxy CONNECT 403, printed) with the one command that fixes it, not waited on.
[S3:heal-drops-guard-marker] [S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:self-weakenable-review-model] [S3:final-gate-trusts-generator] [S3:rule-stronger-than-mechanism] [S3:untested-gate-branch] [S3:release-path-weaker-than-generation] [S3:mutable-model-alias] [S3:unusable-credential-tier] [S3:workflow-tool-version-skew] [S3:heal-drops-guard-marker] [S3:caller-suppliable-custody-inputs] [S3:green-on-stale-base] No gate, threshold, reviewer binding or examiner is touched, and nothing here relaxes one: the split ADDS a refusal (unsplit, zero rows) where the old reader emitted a row. Every new rule is mechanical and tested — the mash is pinned by a test asserting `count != 1`, the wrapper case by a nested-selector test, the truncation case by a test that only passes while the reason IS truncated. The new workflow pins actions by major tag like its siblings, holds no secret, and cannot write (`--write` needs `--real` AND a DSN; it passes neither).
[S3:copy-outruns-registry] [S3:db-type-mismatch-invisible-to-hermetic-tests] [S3:missing-cardinality-check] [S3:pagination-integrity-gap] [S3:weak-key-accepted-at-custody] [S3:partial-write-whole-row] [S3:swallowed-corrupt-data] [S3:destructive-normalization] [S3:volatile-safety-store] [S3:hygiene-narrows-coverage] [S3:featurability-dimension-missed] [S3:fabricated-qualitative-copy] [S3:false-price-claim] [S3:nonfinite-numeric-accepted] [S3:compounded-ground-contrast] Nothing is written, promoted or deleted: no schema, no query, no user-facing string changes. Cardinality is the ticket's subject and is counted both ways (rows_n and identities, per page and per walk); the weak key this removes is the list URL, which tied a whole desk to one identity. A malformed pattern table RAISES rather than yielding a shorter one, and `unsplit` is a named coverage defect on a door, never an empty desk — the coverage direction the law forbids narrowing.
[S3:retyped-evidence] [S3:stale-redclass-count] [S3:stale-base-widens-range] [S3:stale-live-incident-state] [S3:records-sharing-one-trigger] [S3:parallel-record-id-collision] [S3:deferred-trust-work] [S3:semantic-claim-not-rederived] [S3:malformed-ledger-row] [S3:missing-record-read-as-state] Every number in the evidence doc and the PR body is pasted from the command named beside it (the mash reproduction, both dry-ingest tables, the validate evidence block); none is retyped and no ledger count is restated. Base is origin/master tip 008d9d2 as the gate's own freshness line printed it. No RECORD id is opened or shared, and no trust-path gap is deferred: the pattern grades are the honest provenance of committed data, not a deferral.
[S3:pushed-on-red] [S3:env-dependent-hermetic-test] [S3:scripted-transform-order] [S3:deliverable-visual-qa] `bash tools/validate` ran bare on the committed tree and its evidence block is in the PR body. The 6 failures seen mid-session were the shallow clone failing closed (`git fetch --unshallow` cleared them, unchanged code); the tests added here are hermetic — their hosts are test hosts and their patterns are passed in, so none depends on the network or on the committed table. No deliverable is rendered by this change; the deliverable is a markdown table, quoted verbatim.
...
with their desk; nothing waits for a second desk.
HOW: new pure `worker/locale/desk_publish.py` (UnionRow -> the exact candidate
write, keyed on the founder's night+place+title-or-performer so a re-run
updates rather than duplicates) + CLI `tools/desk_ingest.py` over the EXISTING
walk/union, and `.github/workflows/desk-ingest.yml` (workflow_dispatch, egress
+ DSN) because this sandbox's proxy 403s both desks. No new importer, no gate
change: both desks are already `local_media` in the committed catalog, which
`worker/gating.py` already promotes on ONE source (founder ruling 2026-08-05).
WHY: the walk parses two desks and writes nothing, so the live site still shows
one listing. A table in a markdown file is not a catalog.
WHY-IT-MATTERS: every coverage number so far describes pages we read, not rows
a friend can see. Until the walk writes, more desks only make a better report.
EXPECTED OUTCOMES: `--dry-run` prints the write plan from fixtures and CANNOT
write (a fixture walk is refused at the DB seam); the dispatch job runs the
live fetch + write and prints before/after counts for `/tonight` and `/events`;
sandbox live fetch is reported UNREADABLE, not worked around; no merge until
the founder says so.

**construction_gate Stage 3 (founder-capped at 5 lines):**
[S3:contract-scope-violation] [S3:status-narration-not-progress] [S3:governance-ambiguity] [S3:permission-for-ratified-work] [S3:stale-redclass-count] [S3:missing-record-read-as-state] [S3:malformed-ledger-row] [S3:deferred-trust-work] [S3:hygiene-narrows-coverage] Scope is the founder's four Must-dos: the write path, the source label, the UNREADABLE stop with the one command, and the before/after table. Refused in the PR body: Daily/Weekender, a Tonight redesign, a third desk, a union-only change, any STATE heal. R-110 (r1) and R-111 (r3) are opened for the two residuals the evaluator's blocking findings leave (a published row is not corrected when its desk changes its word — the mutation belongs to `worker/listing_update.py`), with an objective, now-unblocked trigger; no ledger count is retyped.
[S3:heal-drops-guard-marker] [S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:final-gate-trusts-generator] [S3:release-path-weaker-than-generation] [S3:untested-gate-branch] [S3:weak-key-accepted-at-custody] [S3:caller-suppliable-custody-inputs] No gate, threshold, workflow reviewer binding or model route is touched. `promote_candidate` re-runs the FULL trust gate on every row this writes — the new code calls the publish seam, it never bypasses or re-implements it, and a desk whose committed catalog class is not an anchor HOLDS at `needs_review` instead of publishing. The source class and the public label are read from the committed catalog by domain match, never supplied by this tool.
[S3:fabricated-qualitative-copy] [S3:copy-outruns-registry] [S3:false-price-claim] [S3:semantic-claim-not-rederived] Every published field is the desk's own word: title, place text, and only a clock the desk STATED (a date with no time is written as a NULL start, never midnight). No descriptor, price or qualitative claim is generated. The public label is the registry row's own name+base_url, which is what `promote.py` already binds.
[S3:missing-cardinality-check] [S3:partial-write-whole-row] [S3:pagination-integrity-gap] [S3:destructive-normalization] [S3:swallowed-corrupt-data] A blocked desk writes nothing and says so; a partial walk writes what it read and the count prints as a floor. Nothing is swallowed: every planned row lands in exactly ONE printed bucket (pinned), and answering this class found a real defect in the first draft — a failed evidence write was reported and the row was promoted ANYWAY, publishing on a record already known to be incomplete (the gate reads its source classes from those rows) and double-counting the row in two buckets. Evidence is now written in full before any promote, and a row that loses one stops at `failed` with its candidate id. Re-running is keyed, so a second run adds vias rather than a second row. Fixture titles cannot reach a database: the write seam refuses a FIXTURE-mode union.
[S3:deliverable-visual-qa] [S3:env-dependent-hermetic-test] [S3:retyped-evidence] [S3:pushed-on-red] [S3:stale-base-widens-range] [S3:founder-path-unprobed] [S3:db-type-mismatch-invisible-to-hermetic-tests] The founder's path is probed, not argued: `tests/integration/test_desk_ingest_pg.py` runs the REAL create/evidence/promote path against a real PostgreSQL with this repo's committed migrations (the existing db-integration leg, every PR, no path filter), and asserts the single-desk listing reaches `/tonight`'s own query carrying its desk's label. The hermetic tests read no network, no DB and no clock, and every number in the evidence doc is pasted from the tool's own stdout. Base is the origin/master tip this branch sits on.
[S3:build-before-plan] [S3:excluded-surface-widening] [S3:parallel-record-id-collision] [S3:green-on-stale-base] [S3:stalled-state-needs-active-diagnosis] [S3:self-weakenable-review-model] [S3:scripted-transform-order] [S3:heal-drops-guard-marker] [S3:workflow-tool-version-skew] [S3:mutable-model-alias] [S3:unusable-credential-tier] [S3:volatile-safety-store] [S3:db-type-mismatch-invisible-to-hermetic-tests] [S3:nonfinite-numeric-accepted] [S3:nonfinite-decimal-accepted] The contract above was written before the first product file, and the ONE surface this change widens is named rather than slipped in: `tools/desk_ingest.py` is added to trust_gate's promote allowlist, because the gate itself refused the import and said to add it deliberately. Its compensation is in the same commit and is NARROWER than the hole — `test_the_ingest_cli_publishes_only_through_the_gate` parses the tool and fails if it ever imports a gate call, a confidence decision, or writes `event` itself, so the entry covers exactly the one seam it was argued for. No R id is taken (nothing is deferred), so no id can collide with a parallel session. Base is master tip 8509761 at drift 0 by the gate's own freshness line, and the branch is not stalled: the one thing it cannot do here — reach the desks — is diagnosed to the sandbox proxy's CONNECT 403 by two independent probes and answered with a dispatch job, not waited on. No script rewrites files in place, no guard marker is rewritten, no reviewer binding, action pin, model alias or credential tier is touched, and nothing is cached. No SQL type, numeric or Decimal is parsed anywhere in this diff: the only value converted is an ISO instant the desk stated, which psycopg2 adapts into the existing `timestamptz` column, and a value we cannot read publishes as a NULL start rather than a number.
[S3:fail-open-on-custody-misconfig] [S3:grant-not-content-bound] [S3:rule-stronger-than-mechanism] [S3:founder-verbatim-corrected] [S3:records-sharing-one-trigger] [S3:stale-live-incident-state] [S3:compounded-ground-contrast] Every misconfiguration on this path fails CLOSED, on every branch: a door with no catalog row refuses to write (never an unlabelled listing), two rows claiming one door refuse (never a guessed masthead), `--write` without `--real` refuses, a FIXTURE union refuses at the write seam, an absent `ONELIVE_DB_DSN` refuses, and an evidence write that fails stops the row before promote — each pinned by its own test rather than by a guard on one instance. The dispatch grant is bound to what it runs, not to a standing permission: manual dispatch only, `write` defaulting to false, master-only, one run at a time, `contents: read`, and the DSN assembled inside the single step that needs it and masked; there is no `schedule:` block, so this change arms nothing. Every rule stated here ships with the mechanism named beside it. TWO records are opened and they are the only deferrals in this change, with different triggers that no other row shares: R-110 (r1) — wiring the desk walk into `listing_update`'s adjudication, unblocked because desk rows state a per-listing identity; and R-111 (r3) — a public representation for "this night, time not stated", which is consumer-surface work this ticket's Must-not list excluded. The founder's ticket text is quoted VERBATIM in `tools/desk_ingest.py` and in this contract; the one place it is not is `worker/locale/desk_publish.py`, where a committed gate forbids a brand literal in a locale module — the substitution there is bracketed and its reason stated on the same line, an elision the reader can see, never a silent correction of founder words. No live-system state is claimed anywhere: the UNREADABLE finding is this session's own probe, run twice by two independent means. No visual, color or contrast surface is touched by this diff.
...
[S3:fail-open-on-custody-misconfig] [S3:featurability-dimension-missed] [S3:founder-path-unprobed] [S3:founder-verbatim-corrected] [S3:records-sharing-one-trigger] [S3:condensed-thinking-run] [S3:stale-live-incident-state] r1, the evaluator's one blocking finding, FIXED not argued: an "X only" bucket is a claim about the OTHER desk's WHOLE list, and `board_table` turned buckets to `unknown` for an UNREADABLE desk while still printing an exact-looking count when another desk was merely PARTIAL. Every bucket now carries the direction it can still move (at most / at least / exact), which is the founder's own "403 is not a zero list" applied one degree weaker; two tests pin it and the evidence doc is regenerated from the tool's stdout rather than hand-edited. r2 blocked on the SAME class in the branch r1 did not touch (an unreadable desk turned every bucket to `unknown` while the unique total still printed a bold exact number beside them), so the direction rule is now TOTAL over all four cases — other-partial, self-partial, BOTH partial (`N so far`, neither bound), exhausted — and every floor prints as `at least N`. r3 blocked on the same class one layer OUT — the report's hand-written footer still claimed "every count here is therefore a FLOOR" beside a board printing ceilings — so the sentence is now DERIVED (`certainty_note`) from the same desk facts the buckets are labelled from, with a test holding it to the board it summarises. r4 blocked twice more: the desk-local key fell back to the title, so two unkeyable rows on ONE desk collided (it is ROW-local now — the address tail, else title plus the row's ordinal in its walk), and the headline sentence still stated the total flat one paragraph above the bounded board (`bounded()` + `summary_line()` now render every union count with its certainty, and the tool carries no other count-bearing prose). r5: `_cell` escaped the pipe that ends a column and passed the NEWLINE that ends a row (a live title with a line break forges table rows — it collapses whitespace now), `_keyed` reported an unparseable stated date as "no night stated" (our parser gap printed as the desk's silence — it keeps the reason now), and the evidence refresh cut on a line count of its own output, which is what duplicated the report heading (it cuts on a marker now), so no printed number is a restatement of an earlier one. The panel's po battery is quoted as the seats wrote it, not summarised, and no live-system state is claimed anywhere in this diff. No custody path, no discovery markup, no founder-facing runbook step and no founder-verbatim text is touched by that fix, and it opens no record (the defect is closed here, so nothing shares a trigger).
...
`tools/desk_coverage.py`.
HOW: Reuse #223's walker and reader unchanged for the read path. Two additions
only, both needed by THIS desk: `href_path` category signals in
`worker/locale/kind_map.py` (Do512 routes categories by PATH, not by a query
param, so `href_param` can match nothing here), and, in
`worker/locale/desk_walk.py`, an honest stop when a page's continuation is a
control we cannot follow (a button / `href="#"` "load more") — today that reads
as `no_next_link`, i.e. an EXHAUSTED desk. Fixtures under
`tests/fixtures/desk_pages/do512-today/`; egress to do512.com is denied by this
sandbox's proxy (curl CONNECT 403, recorded at its status endpoint), so every
count is a FIXTURE count and says so, and the `--real` command is named.
WHY: Coverage Law — the catalog holds the UNION of the public desks, deduped, in
OUR model. Do512 is the second desk instance; one page per door is not a desk.
WHY-IT-MATTERS: the failure is silent. A walker that calls a JS "load more" the
end of the list prints a confident number that is a fraction of the desk, and
nothing downstream can tell that number from a complete one.
EXPECTED OUTCOMES: the three tables print for `do512-today`; the mapping loads
with every `our_kind` validated against the pack's kinds and no `desk_observed`
row (nobody has read the live page); live vs fixture is stated on every count;
catalog and Tonight are UNCHANGED (this PR writes no candidate, no promotion, no
user-visible row); no merge until the founder says so; the next desk is NAMED,
not started.

**construction_gate Stage 3 (founder-capped at 5 lines):**
[S3:contract-scope-violation] [S3:status-narration-not-progress] [S3:stalled-state-needs-active-diagnosis] [S3:governance-ambiguity] [S3:parallel-record-id-collision] [S3:records-sharing-one-trigger] [S3:stale-redclass-count] [S3:featurability-dimension-missed] [S3:founder-verbatim-corrected] [S3:founder-path-unprobed] Scope is the founder's six Must-dos and the two code changes they require; the third desk and Tonight are refused in-PR per Must-not, and the next desk is NAMED, not started; each Must-do is answered by a test rather than by prose, and what is NOT built (no promote wiring, no third desk, no Tonight change) is named rather than quietly omitted. The founder's own words — their column names, their Must-not list, their pass criterion — are quoted, never edited into what I would have written. The one command handed to them (`--door do512-today --real`) was RUN end to end this session; what is unprobed, and labelled so everywhere, is only what lies past our proxy: the live desk itself. The 403 was diagnosed once against the proxy's own status endpoint and reported, never waited on or routed around. R-108 and R-109 are fresh ids taken after reading the RECORD tail (R-107 was last), each with its own distinct objective trigger — R-108 on the third committed desk map, R-109 on the first permitted live run; these answers are written against the match list this run printed, not a remembered one.
[S3:heal-drops-guard-marker] [S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:untested-gate-branch] [S3:pushed-on-red] [S3:weak-key-accepted-at-custody] [S3:workflow-tool-version-skew] [S3:mutable-model-alias] [S3:unusable-credential-tier] [S3:stale-base-widens-range] [S3:rule-stronger-than-mechanism] [S3:final-gate-trusts-generator] [S3:db-type-mismatch-invisible-to-hermetic-tests] [S3:caller-suppliable-custody-inputs] [S3:green-on-stale-base] [S3:release-path-weaker-than-generation] [S3:self-weakenable-review-model] No gate, threshold, workflow, action pin, reviewer binding or model route is touched, and no key is read or minted. Both code changes ADD refusals only — `href_path` refuses an unanchored prefix and a numeric segment, and the walk refuses to call a script-continued list exhausted — and every new branch carries its own test (five control shapes, three non-control shapes, both stop reasons, and the link-beats-control case), so no rule here is stronger than its mechanism — including R-108's cross-desk vocabulary rule, which ships as a test in this same commit. The final gate does not trust me: the non-Claude evaluator reviews this PR as it does every PR, and nothing here can be merged on my own word. No SQL, schema or DB type is touched — the only query on this path is the pre-existing parameterised read behind `--store`, which this sandbox never ran (no DSN), so nothing hermetic is standing in for a database here. Base equals the remote tip per the gate's own freshness line (f692c5f), so no check runs against a stale base. No custody input is caller-suppliable here: the evidence grade of a mapping row is committed DATA validated at load, never a value a run supplies, and the reviewer binding stays the base-owned non-Claude evaluator on every PR.
[S3:destructive-normalization] [S3:swallowed-corrupt-data] [S3:partial-write-whole-row] [S3:hygiene-narrows-coverage] [S3:pagination-integrity-gap] [S3:volatile-safety-store] [S3:fail-open-on-custody-misconfig] [S3:missing-cardinality-check] [S3:nonfinite-numeric-accepted] [S3:nonfinite-decimal-accepted] Nothing is written anywhere — no DB, no candidate, no promotion, no partial row — so there is no write to half-apply, and no custody configuration is read. Every loader here fails CLOSED: a mapping that names a kind we do not have, a signal with no prefix, an id row with no href signal, an unknown door type or intake, and an unreadable fixture manifest are each an ERROR, never an empty result that would read as 'this desk has nothing on'. A blocked, empty or malformed page keeps every row already read and records WHY; cross-page repeats fill HOLES only and never overwrite a stated value; rows_seen and unique count are both printed so pagination can neither inflate nor hide rows; and the new stop reason only WIDENS what is reported (a walk that was called exhausted is now called a floor) — it drops no row and skips no page. Cardinality is asserted, not assumed: the page count, rows_seen, unique count, repeats, merges and untitled blocks are each pinned per fixture. No numeric is parsed anywhere on this path — a date is an ISO string the page declared or None — so there is no NaN/Infinity route and no Decimal is constructed.
[S3:fabricated-qualitative-copy] [S3:copy-outruns-registry] [S3:false-price-claim] [S3:semantic-claim-not-rederived] [S3:retyped-evidence] [S3:env-dependent-hermetic-test] [S3:scripted-transform-order] [S3:deliverable-visual-qa] Every mapping row carries its own evidence grade and NO row is `desk_observed`, because nobody here has read a live Do512 page; unmapped categories are PRINTED for a live run to complete. No price, marketing claim or qualitative copy is generated. Every number in the evidence doc is captured from the tool's own stdout or derived from the committed JSON in the same script that wrote the doc, so re-running re-derives them; tests read committed fixtures only — no network, no DB, no clock, no model. The deliverable is markdown tables with no rendered layout to measure.
[S3:deferred-trust-work] Neither recorded row defers trust-path work: nothing in this PR reaches the published data path (no candidate, no promotion, no consumer surface). R-108's drift risk is CLOSED mechanically in this same commit by a cross-map vocabulary test; only the structural de-duplication is deferred. R-109 is a measurement this sandbox's network forbids, and it names the single command that closes it.
...
mapping table as DATA (`sources/kind_maps/`), per-row category reading in
`worker/locale/desk_read.py` (opt-in, default behaviour unchanged), and a
`tools/desk_coverage.py` that prints the page table, the kind table and the founder's
coverage table (on_desk | in_store | gap | reason).
HOW: The walker is pure — a caller-injected fetch, next pages taken ONLY from the page's
own markup (rel=next / a declared next anchor), same-host, cycle-guarded, capped, with
`blocked_reason` recorded per page through the same `demote_on_response` authority the
ingest loop uses. Kind comes from THEIR stated category mapped through the committed
table (unmapped = other); their labels never become our schema, and no brand or place
literal enters `worker/locale/` (the two pack tests grep for exactly that). Egress to
calendar.austinchronicle.com is DENIED by the sandbox proxy (curl CONNECT 403 + WebFetch
EGRESS_BLOCKED), so counts here come from committed shape fixtures and every table
labels live vs fixture honestly; the live column needs the founder-authorized run.
WHY: Coverage Law — the catalog must hold the UNION of the public desks, deduped, in our
model. Today we read one page per door; a desk's list is many pages, and page 2 onward is
simply absent from the store.
WHY-IT-MATTERS: an unpaginated desk read looks green while silently capping coverage at
whatever one page happened to hold — the exact failure mode the Coverage Law calls a
defect, and it is invisible without an on_desk vs in_store table to show the hole.
EXPECTED OUTCOMES: walker + mapping table + coverage tool land green with tests; the
Chronicle door's row count is bounded by pages, not by one page; the live-vs-fixture
split and the blocked host are stated, never papered over; catalog and Tonight are
UNCHANGED by this PR (nothing here writes a candidate, a promotion or a user-visible
row). No merge until the founder says so; the next desk is NAMED, not started.

**construction_gate Stage 3 (founder-capped at 5 lines):**
[S3:contract-scope-violation] [S3:status-narration-not-progress] [S3:stalled-state-needs-active-diagnosis] [S3:governance-ambiguity] [S3:featurability-dimension-missed] [S3:founder-verbatim-corrected] Scope is the founder's six Must-dos; the 403 is diagnosed against the proxy's own status endpoint and reported, never waited on or routed around; Do512 and Tonight are refused in-PR per Must-not and Do512 is NAMED as next per Must-do 6; the founder's own words (their column names, their pass criterion) are quoted, never edited into what I would have written.
[S3:heal-drops-guard-marker] [S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:self-weakenable-review-model] [S3:final-gate-trusts-generator] [S3:untested-gate-branch] [S3:release-path-weaker-than-generation] [S3:pushed-on-red] [S3:rule-stronger-than-mechanism] [S3:weak-key-accepted-at-custody] [S3:workflow-tool-version-skew] [S3:green-on-stale-base] [S3:caller-suppliable-custody-inputs] [S3:mutable-model-alias] [S3:unusable-credential-tier] No gate, threshold, workflow, action pin, reviewer binding or model route is touched, and no key is read or minted; the walker ADDS refusals only (off-host, cycle, wall, cap, non-PageFetch, unreadable door), each with its own test incl. every refusal branch, so every rule this PR states is carried by a test rather than by prose; validate ran bare on a base equal to the remote tip (its own freshness line) and its evidence block is pasted, not retyped.
[S3:destructive-normalization] [S3:swallowed-corrupt-data] [S3:partial-write-whole-row] [S3:hygiene-narrows-coverage] [S3:missing-cardinality-check] [S3:pagination-integrity-gap] [S3:nonfinite-numeric-accepted] [S3:nonfinite-decimal-accepted] [S3:volatile-safety-store] Nothing is written anywhere — no DB, no candidate, no promotion, no partial row — so there is no write to half-apply; a blocked, empty or malformed page keeps every row already read and records WHY; cross-page repeats fill HOLES only and never overwrite a stated value; raw vs unique counts are both printed so pagination can neither inflate nor hide rows; per-page and total counts are asserted per fixture; no numeric is parsed at all (a date is an ISO string the page declared or None), so there is no NaN/Infinity path and no Decimal is constructed anywhere.
[S3:fabricated-qualitative-copy] [S3:copy-outruns-registry] [S3:false-price-claim] [S3:semantic-claim-not-rederived] [S3:retyped-evidence] [S3:env-dependent-hermetic-test] [S3:scripted-transform-order] [S3:deliverable-visual-qa] Every mapping row carries its own evidence grade and no desk category is invented from memory (`language_rule` says explicitly it is NOT a claim about this desk; unmapped labels are PRINTED for a live run to fill); no price, no marketing claim, and no qualitative copy is generated anywhere; every number in the evidence doc is rendered from the committed data or captured from the tool's own stdout in the same script that wrote the doc, so re-running it re-derives them; tests read committed fixtures only — no network, no DB, no clock; the deliverable is markdown tables, with no rendered layout to measure; and the one command handed to the founder (`tools/desk_coverage.py --door austin-chronicle-eventsearch --real`) was RUN this session end to end — what is UNPROBED, and labelled so everywhere, is only what lies past our proxy: the live desk itself was never reached, so no count from it is claimed.
[S3:malformed-ledger-row] [S3:missing-record-read-as-state] [S3:stale-redclass-count] [S3:parallel-record-id-collision] [S3:records-sharing-one-trigger] [S3:deferred-trust-work] [S3:stale-live-incident-state] [S3:stale-base-widens-range] [S3:heal-drops-guard-marker] R-106 and R-107 are fresh ids taken after reading the file's current tail (R-105 was the last), each with its own distinct objective trigger — R-106 closes on a named one-line edit, R-107 on the first permitted live run; neither defers trust-path work (nothing here reaches the published data path); red-class answers are written against the gate's own printed match list this run, not a remembered one; base is origin/master tip bc4995c per the gate's ls-remote line; STATE.md is APPENDED to here, so the marker `staleness_check` fails closed on is left exactly as it was found.
...
[S3:rule-stronger-than-mechanism] The sharp one for this commit, and it landed on me: "armed-cron touch -> smoke" is NOT prose here — tests/test_arming_smoke_binding recomputes it from git in trust-gate (required), which is exactly what caught my PR-body claim that no armed file was touched. The rule had the mechanism; I had the wrong belief. [S3:founder-path-unprobed] The founder's authorized path was PROBED, not asserted: the dry smoke was dispatched and ran (33836342895, success), and its three scope limits are recorded rather than smoothed. [S3:build-before-plan] The plan was printed and this contract written before any code; this commit is that plan's Must-do 6 step. [S3:final-gate-trusts-generator] [S3:release-path-weaker-than-generation] The binding is recomputed FROM GIT by a required check, not taken from my summary, and the release venue is the same trust-gate job. [S3:stale-live-incident-state] Every run fact is read from the live Actions API and the job log in this session, never from memory. [S3:malformed-ledger-row] [S3:missing-record-read-as-state] No RECORD row is claimed and no ledger row is added by this commit. [S3:condensed-thinking-run] Not applicable — nothing is compressed away; the miss and its cause are written down in full.

[S3:grant-not-content-bound] The standing dry re-bind grant is bound, not open: to THIS PR ('for the rest of this PR only'), to the dry 2-source parameters, and to one dispatch per runtime change. Both dispatches used identical authorized parameters (max_sources=2, source_class=B, follow_pages_per_run=2, listing_update=dry), and each is recorded in the evidence file WITH the runtime change that made it necessary, so the grant cannot be read as covering a re-wave or a different shape of run. When the second dispatch sat queued behind the ingest concurrency group, nothing was cancelled and no second wave was sent — the founder's own instruction for that case ('dispatch into the next gap … not a new wave; do not cancel a run that has already started') was already satisfied by waiting. [S3:swallowed-corrupt-data] Two shapes of bad data in this run, NEITHER swallowed. Come and Take It Live's page yielded ZERO events from 53,598 chars: the run took its AI_EXTRACT_ZERO_EVENTS_SOURCE_MAY_HAVE_MOVED branch loudly and recorded a FLAGGED EMPTY candidate for ops rather than dropping the source, and the evidence file states that the gate3 PASS is a verdict on that sentinel row and NOT on a parsed listing. And the provider returned no usage stamps for a call that was made: recorded as spend UNREPORTED, not coerced to zero and not folded into the bound-run total.
...
STATUS: OPEN
WHAT: An audit of every rule that can drop / hold / escalate / refuse a candidate or a
source (table in the PR body), and the code changes the audit demands. (1)
`worker/trust_gate3.py`: a conflicting `start_time` and a dedupe-ambiguity hint stop
being ESCALATE reasons — they become FIELD holes carried on the verdict
(`GateVerdict.field_holes`), so a trusted door still PASSES existence. Start-time
claims are compared as INSTANTS, not strings, so one instant rendered two ways is one
claim. (2) `worker/promote.py`: a candidate whose clock is a hole publishes with a NULL
clock (the feed's existing honest "Date TBA"), never with an invented one; the ±90-minute
same-venue dedupe stops REFUSING publication outright and refuses only an exact identity
(same venue + same start minute + same normalized title), recording a near-collision as
an `audit_log` ambiguity instead of a missing row. (3) `tests/test_gate_existence.py` plus
updates to the four test files that pinned the old rule.
HOW: Existence, field and mutation are answered by three separate tests and never by each
other. Existence is decided by the door alone (`worker/gating.py` anchor classes,
unchanged). A field the evidence disagrees about is written as a HOLE, never guessed and
never used to deny existence. Mutation is untouched: `worker/listing_update.py` and
`worker/crawl_state.py`'s `classify_recheck` keep #214's fail-closed refusals verbatim —
an uncertain time still cannot overwrite a confirmed published time, a collision still
cannot rename A to B, and `may_delete_listing` is still False for every input. ESCALATE
survives for the two reasons that are NOT existence questions: a schema-invalid extraction
(fabrication risk — "the extractor guessing is never a door") and a private/RSVP event
(a publish-appropriateness call). HOLD is untouched: social-alone and scrapers still do
not promote, which is the trust doctrine's own rule.
WHY: two rules were answering "does this happening exist?" with a test about a FIELD.
Measured on the real schema, not argued: `event_candidate.start_time` is `timestamptz`,
so psycopg2 hands back a tz-aware datetime whose `.isoformat()` ends in `+00:00` while the
stored extraction keeps the raw parser string — so ONE clock read two ways scored as two,
and every timed candidate ESCALATEd and could never publish. The dedupe hint fires on any
other live candidate at the same venue and minute, which a second crawl of the same page
creates by construction, so re-reading a calendar permanently escalated every event on it.
WHY-IT-MATTERS: those are not edge cases, they are the default path — the two defects
together mean a correctly-fetched, correctly-parsed, trusted-door calendar produces zero
listings, which is the exact "strangling my display of valid events" failure the map
exists to prevent, and it was invisible because each rule reads reasonable on its own.
EXPECTED OUTCOMES: the audit table (rule | file | blocks | existence/field/mutation |
keep/soften/delete); Scoot-Inn-shaped and Blanton-shaped fixtures proving rows exist, the
source is not banned, and mutation of the uncertain field is refused; every listing-update
refusal test still green. No armed-cron file changes, so no smoke run is required.
Out of scope (Must-not): Tonight beauty, cadence YAML, catalog upsert, new vendor, and any
weakening of the collision/permalink rules (R-095/R-097/R-099).

SCOPE AMENDMENT (2026-09-03, r2 — quoted rather than silently absorbed). The contract above
scopes this ticket to the founder's six Must-dos; it did not name a dependency change. The
r1 round touched `web/lib/*.ts`, which makes `adversarial-review.yml` run its web leg on
this PR for the first time, and that leg's SCA gate went RED on four high advisories in
`fast-uri` (GHSA-5jgf-p345-68v8, -f65p-4m7j-42xc, -fph4-wmhf-6fwf, -jqff-g426-hqxp). The
advisory is NOT this diff's: `fast-uri` reaches the tree at
`@sentry/nextjs -> @sentry/webpack-plugin -> webpack -> schema-utils -> ajv`, none of which
this PR touches; the diff merely made the gate run. The gate's own two exits are "fix it, or
add a reviewed entry" — and the exception route is a gate-threshold relaxation, which
CLAUDE.md makes founder-crucial and never an agent decision. So it is FIXED, minimally and
with the repo's existing mechanism: `web/package.json` already pinned `fast-uri` in
`overrides` at `^3.1.5`, which is the TOP of the vulnerable range `3.0.0 - 3.1.5`; the caret
already admitted a patched version and only the frozen lockfile held it back. The override
now states the security floor explicitly (`^3.1.7`) so a future lock regeneration cannot
slide back into the range, and the lock is regenerated with `npm install
--package-lock-only`. No package is added or removed, no major version is crossed, and
`security/sca_allowlist.json` is untouched. Neither file is in the armed cron's runtime
closure (checked against tools/arming_runtime.py), so the smoke binding is unaffected.
Verified: `tools/sca_gate.py --web-dir web` PASS, 297 web tests, `tsc --noEmit` clean, and
the production `npm run build` green.


**Appendix — construction_gate Stage 3 citations (Operating Law rule 9: minimum the gate will accept):**
[S3:contract-scope-violation] [S3:permission-for-ratified-work] [S3:governance-ambiguity] [S3:status-narration-not-progress] [S3:stalled-state-needs-active-diagnosis] Scope is the founder's six Must-dos and the Must-not list. Not touched: Tonight/beauty, cadence YAML, catalog upsert, any new vendor, and R-095/R-097/R-099's collision and permalink rules (`worker/listing_update.py` is READ and re-pinned by new tests, never edited). No armed-cron file changes, so the must-do-6 smoke is not owed — proven by `tests/test_arming_smoke_binding.py` passing on this tree, not asserted. The plan-first hook and Operating Law's "one ticket, then stop" are surfaced in the PR body rather than resolved silently toward execution.
[S3:founder-verbatim-corrected] [S3:founder-path-unprobed] The founder's must-do 3 phrase "time_uncertain or existing equivalent" is taken at its word: the EXISTING equivalent is used (a NULL `start_time`, which `web/lib/feed.ts` already renders "Date TBA"), so no new column, vocabulary, or migration is invented on top of founder wording. The founder's own path is probed rather than assumed — the "two clocks" case was run through the REAL `_load_gate_signals` against the REAL `timestamptz` shape, which is how the false-conflict defect surfaced at all.
[S3:heal-drops-guard-marker] [S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:untested-gate-branch] [S3:rule-stronger-than-mechanism] [S3:final-gate-trusts-generator] [S3:release-path-weaker-than-generation] [S3:self-weakenable-review-model] This ticket LOOSENS a gate at founder direction, so the loosening is bounded and named rather than general. HOLD is untouched (social-alone and scrapers still do not promote). ESCALATE keeps exactly two reasons and a structural test fails if a third is added or one of the two is dropped (`test_escalate_survives_only_for_the_two_non_existence_reasons`). No threshold, workflow, reviewer binding, credential or model id is edited; `tools/` is untouched. Every new branch is covered: hole-on-HOLD, hole-on-PASS, one-instant-two-notations, unparseable claim, dedupe note, re-publish refusal, neighbour publish, untitled non-match.
[S3:missing-cardinality-check] [S3:partial-write-whole-row] [S3:destructive-normalization] [S3:swallowed-corrupt-data] Asked in both directions. The dedupe split reuses `normalize_title` rather than re-deriving a reduction, so the publish side and the mutation side can never disagree about a name (the exact defect R-095/R-096 cost three rounds); a missing title on EITHER side is a non-match, never a match. The clock hole writes `start_time` AND `end_time` to NULL together, because an end with no start is not a window — the whole-row question R-098 forced onto the mutation path, answered the same way on the publish path. An unparseable start-time claim keeps its raw text as its own identity instead of silently collapsing into a parsed one.
[S3:deferred-trust-work] [S3:semantic-claim-not-rederived] [S3:copy-outruns-registry] [S3:fabricated-qualitative-copy] [S3:false-price-claim] [S3:records-sharing-one-trigger] Two deferrals recorded in the same commit: R-101 (a NULL-clock row is filtered out by the consumer window before the client's own date-TBA handling can show it — PRE-EXISTING, `web/` is Must-not) and R-102 (a published hole is not filled by a later tick, because `start_time` is unwritable by construction under R-099). R-102 is opened knowing the register already clusters here: it names R-095/R-097/R-099's shared trigger explicitly rather than opening a fourth independent record. No user-facing copy, price, or qualitative claim is added anywhere.
[S3:db-type-mismatch-invisible-to-hermetic-tests] [S3:env-dependent-hermetic-test] [S3:pagination-integrity-gap] [S3:volatile-safety-store] [S3:weak-key-accepted-at-custody] [S3:caller-suppliable-custody-inputs] [S3:fail-open-on-custody-misconfig] [S3:mutable-model-alias] [S3:workflow-tool-version-skew] [S3:unusable-credential-tier] The DB-type class BIT, and finding it is this ticket's headline: the old start-time test compared `.isoformat()` of a `timestamptz` against a jsonb string, and every hermetic test had used a NAIVE datetime whose isoformat happened to match — so the fixtures agreed with each other and disagreed with production. The new fixtures use AWARE datetimes, the shape psycopg2 actually returns, and `test_one_clock_written_two_ways_is_one_claim` asserts the two renderings really are different strings before asserting the gate reconciles them. No migration, no new store, no new key path, no model id, no workflow tooling. SQL stays static and parameterized (`trust_gate` PASS); the widened dedupe query adds two selected columns and no new predicate.
[S3:build-before-plan] [S3:nonfinite-decimal-accepted] [S3:nonfinite-numeric-accepted] The contract above was written to STATE.md BEFORE the first product edit, and the plan-first hook enforced it. No numeric or Decimal value is parsed, compared, or written by this change: the only values it reconciles are timestamps and titles, and the timestamp path is total — an unparseable claim returns None from `_as_instant` and keeps its raw text as its own identity rather than becoming a NaN-shaped value that compares equal to nothing or to everything.
[S3:swallowed-corrupt-data] BITES at r2, and it is this round's own finding rather than a pre-emptive answer. Both openai seats blocked on it: the date-TBA fix moved `fromISO`/`toISO` out of a VALUE slot (`start_time=gte.X`, where a stray `,` or `)` is just part of the timestamp) and into PostgREST GRAMMAR (`or=( … )`, where `,` separates disjuncts and `)` closes the group), so a crafted bound could append a disjunct or truncate the predicate and change which rows a PUBLIC FEED returns. The move is what made it dangerous and the move was mine — the r1 fix answered "is the row fetched?" and never asked "what is this string now allowed to mean?". Not currently reachable (the one live caller passes `toISOString()` output) and that is exactly why it needed catching now: both builders are EXPORTED, so any future caller taking a query parameter would have inherited it silently. Fixed by RE-SERIALIZATION rather than validation — `windowBound` parses an instant and emits a fresh `Date(...).toISOString()`, which is `YYYY-MM-DDTHH:mm:ss.sssZ` by construction and therefore cannot contain a comma, a parenthesis or operator text; a blacklist would have been a guess about PostgREST's grammar, this is not. Unparseable input THROWS rather than being dropped (a dropped bound silently widens the window and puts a past event on tonight's feed) or passed through (that is the injection); the live caller's `try/catch` renders it as the error state. One home, imported by both readers, so the two feeds cannot drift. Tested with the seats' own payloads plus a structural test asserting every emitted bound matches the canonical instant pattern.
[S3:excluded-surface-widening] Matches at r2 on the word `allowlist` and it is answered in the direction the class demands: NO scanner-excluded surface is widened. `security/sca_allowlist.json` — the managed, expiring exception list the SCA gate consults — is DELIBERATELY untouched, because adding an entry there is the gate's "or add a reviewed entry" exit and that is a gate-threshold relaxation, founder-crucial and never an agent decision. The advisory is CLOSED instead, by moving the dependency out of the vulnerable range; the gate keeps exactly the strictness it had, and `tools/sca_gate.py --web-dir web` now returns "clean — no high/critical production advisories" rather than "covered by an exception". The change is also NARROWER than the exclusion it sits beside: an explicit `^3.1.7` floor admits strictly fewer versions than the `^3.1.5` it replaces, so the manifest itself now forbids the vulnerable range instead of merely happening to resolve outside it.
[S3:grant-not-content-bound] The smoke grant is treated as bound to its stated content and it is SPENT: must-do 6 says 'one dry 2-source smoke only', and exactly one run was dispatched (33722177933) at exactly max_sources=2, source_class=B, follow_pages_per_run=2, listing_update=dry, into an idle queue so nothing was evicted. Both sources answered 304, so the run bound the runtime WITHOUT exercising a single gate decision — and that is stated first in the evidence file rather than buried, because a green binding that proves nothing about the changed path is exactly the shape a grant-stretching argument would hide behind. A second dispatch is ASKED of the founder in the PR body rather than taken, and PR #214's standing 're-bind after each evaluator round' authorization was explicitly scoped 'for the rest of PR #214 only' and is NOT reused here — including for THIS r1 round, which changes worker/promote.py and worker/trust_gate3.py.
[S3:featurability-dimension-missed] Asked at r1 because the diff now reaches a consumer surface: what does a date-TBA row DO on the feed once it is fetched? Answered by reading rather than assuming — `web/lib/feed.ts` already carries the rule ('date-TBA only shows under All'), sorts NULLs last under PostgREST's asc default, and `web/lib/detail.ts` renders the row by id regardless of clock; the blurb path explicitly says it must not claim a time for a row that has none. So no view's CONTENTS change except that 'All' now receives the rows it was written to render, no new copy is added, and no bucketing logic moves. Verified, not asserted: 297 web tests and `tsc --noEmit` both green on this tree.
[S3:missing-cardinality-check] BITES at r1, and both openai seats found it from opposite sides: the clock hole nulled `start_time` and the duplicate window check ran AFTER, guarded by `if start_time` — so on exactly the path this PR adds, the guard asked NOTHING. Reachable on purpose, not only by accident: a source that adds a second conflicting time to an already-published show forces the hole path and lands a duplicate "Date TBA" row beside the real one. The class's own lesson is the one I missed — ask the question in BOTH directions — and here the second direction was temporal: I checked what the guard reads and never what ORDER it reads it in relative to the write. Fixed by probing identity with what the evidence CLAIMED (`start_time_instants`, every claim, plus the column value when it survived) rather than with what we printed, so the probe cannot be emptied by a later change to what gets written. Pinned by a fixture where the published row sits on the SECOND claim (so probing one would miss it) and by a structural test on the ordering itself, because a fixture alone can be satisfied by code that re-reads the written value.
[S3:deferred-trust-work] BITES at r1 as well, and the seat was right: R-101 was RECORDED and left open on a scope call ("web/ is Must-not"), while this PR's own stated outcome is that a trusted-door candidate stays LISTABLE — and a row nothing fetches is not listable. Recording a gap is not closing it when the gap is the deliverable. Must-not names *Tonight beauty*; a `start_time.is.null` predicate is data reachability, not design. R-101 is RESOLVED in this round, and no founder product call turned out to be needed: `web/lib/feed.ts` already encodes where a date-TBA row belongs and was simply never sent one.
[S3:pushed-on-red] [S3:green-on-stale-base] [S3:stale-base-widens-range] [S3:retyped-evidence] [S3:stale-redclass-count] [S3:stale-live-incident-state] [S3:parallel-record-id-collision] [S3:scripted-transform-order] [S3:heal-drops-guard-marker] [S3:deliverable-visual-qa] [S3:compounded-ground-contrast] Full suite 2551 passed / 40 skipped; trust_gate, lint, deferral_scan, governance_claims, perf, test_audit, pr_size_check all PASS — pasted from `.validate-evidence.txt`, never retyped. Base is origin/master tip bc720f7 at drift 0 (the gate's own freshness line). NOTE, because it nearly became a false report: five certification tests and the arming-smoke binding FAILED on the FIRST run and were NOT this change — the sandbox clone was shallow, so `git cat-file` could not resolve the certified subject commit. Verified by stashing the diff (they failed on the clean base too), then fixed by `git fetch --unshallow`; all six are green. R-101/R-102 are the next free ids on a branch cut from that tip. No renumber script runs, the GROUND_TRUTH marker is advanced by hand (fields preserved, nothing rebuilt), and no visual/contrast surface is touched.
...
[S3:founder-verbatim-corrected] [S3:governance-ambiguity] [S3:permission-for-ratified-work] [S3:grant-not-content-bound] [S3:founder-path-unprobed] The founder's must-do 4 OVERRULES the ratified 404 reading in `docs/memory/decisions/2026-09-02_confirmed-check-may-update-listing.md`, which itself invited a one-line overrule. Recorded as a new decision record with a pointer added to the superseded one — never silently reversed, and the overrule's cost is stated rather than glossed. The plan-first hook (§4a approval) versus Operating Law's "one ticket, then stop" tension is surfaced to the founder in the PR body rather than resolved silently toward execution. The smoke grant is treated as bound to its stated content: 'one 2-source class-B run with ONELIVE_MAX_LISTING_MUTATIONS_PER_TICK=0 ... No second wave' — dispatched at exactly max_sources=2, source_class=B, follow_pages_per_run=2, listing_update=dry. The FIRST dispatch (33694574164) was cancelled while still queued by a scheduled tick and ran zero jobs, so the authorized run had not happened; the re-dispatch completes that one grant rather than opening a second wave, and the separate eviction allowance the founder gave 'this once' LAST session was deliberately NOT reused — the retry was timed into the gap after a tick completed, so nothing was displaced. The founder's own path is probed, not assumed: the dry switch is proven by the run's own printed DISABLED line, and the evidence states explicitly that the zero mutation counters prove nothing on their own because the tick had 0 event-proximity pages due.
...
[S3:destructive-normalization] BITES, and it is this round's own finding rather than a pre-emptive answer: the title reduction `[^0-9a-z]+ -> space` DELETED every non-ASCII letter, so a published `Beyoncé` reduced to `beyonc` while a page's `Beyonce` reduced to `beyonce` and the absence guard answered a confident False about a page still naming the event — the one answer that can license a cancellation. Fixed by folding rather than deleting (`_fold`: NFKD, then drop the combining marks) and by keeping letters that have no ASCII form, so `Кино Night` no longer collapses to the needle `night`. Both sides of every comparison run through the same reduction. Tested with the two sides SPELLING IT DIFFERENTLY (test_an_accent_is_not_an_absence, six pairs), with a non-match (test_a_folded_title_still_has_to_be_the_same_words), on the non-Latin case, and on the match path as well as the absence guard. Residual recorded, not hidden: combining marks outside the Latin/Greek/Cyrillic blocks still become spaces (R-096, OPEN, with its trigger).
[S3:missing-cardinality-check] BITES A SECOND TIME on this contract, in the direction the class had never been read: the adjudicator asked how many listings match this row and never how many rows match this listing. A page returning only the later of two same-title occurrences gave BOTH published rows exactly one match — the same one — so the one-way check did not miss the case, it CERTIFIED it, and the earlier row was retimed onto an event that is not it. Matching is now resolved page-wide before any row is decided and a listing claimed twice identifies neither. Tested end to end (test_one_page_listing_cannot_identify_two_published_rows, plus the writer, the cancel path, and the uncontested control). The class row itself is REINFORCED with the mirror question and with `matches(`/`match_kind`/`hits[0]` triggers so retrieval reaches it next time; the coverage cost is R-097, OPEN.
[S3:malformed-ledger-row] [S3:missing-record-read-as-state] Incidental matches — both trigger on the word KAIZEN, which appears in this round's new RED_CLASSES row as a provenance citation, not as a metric. No ledger row, scorecard, trend, or derived metric is written or read by this change; the two records it adds (R-096, R-097) are RECORD.md table rows carrying no pipes in their prose, and the deferral scan is green.
[S3:partial-write-whole-row] BITES, and it is this round's own finding: `_UPDATE_SQL` writes with `coalesce`, so a diff naming only `start_time` KEEPS the published `end_time` — and a row published 20:00-22:00 whose page moved it to 23:00 without restating an end would have been written as 23:00-22:00, an event ending before it begins that any reader using `end_time` treats as already over. Every test asked whether the FIELD was right; none asked whether the ROW was right afterwards, and the founder's own decision table had been demonstrating that exact mutation as a headline `yes` row for five rounds. Fixed by `_incoherent`, which keys on what the page STATED rather than on what changed — a read restating an unchanged end still updates, a row with no published end still follows a simple move — plus a refusal when the resulting pair is not a window at all, because a gate PASS corroborates a listing's evidence and never that its fields are mutually sane. Pinned by the enumerating test (test_no_update_decision_can_carry_an_impossible_window, all published/parsed combinations) rather than by another fixture. Cost recorded: R-098, OPEN, with the shape that closes it.
[S3:destructive-normalization] BITES A SECOND TIME, on the same file in the same week, and the repeat is the finding: the r6 fix enumerated the combining BLOCKS I could think of (Latin, Greek, Cyrillic), so Hebrew niqqud, Arabic harakat and Indic matras still became spaces and split those words in half. I recorded that residual as R-096 with the claim that it was 'never a wrong mutation' — the claim was FALSE, a split needle produces a confident absence and a gate-passed bracket then cancels a still-listed event, and the seat was right to block rather than accept the record. Now asked of Unicode instead of listed: Mn and Me fold away, Mc is kept because a spacing vowel sign carries a vowel, and `unicodedata.combining()` is deliberately NOT the test since it returns 0 for Mc and Me. R-096 is RESOLVED in this commit, pinned across Latin, Hebrew and Arabic plus a structural test that fails if a hand-listed range returns.
[S3:missing-cardinality-check] Answered again for r7's identity finding: a same-minute match used to be an identity whenever nothing CONTRADICTED it, so a parsed listing with no title identified the row and could write its end_time. An anonymous listing confirms nothing — the gate PASS proves it is real, never that it is ours — so it now identifies nothing AND blocks the cancel path (MATCH_UNTITLED), because something on our start time we cannot tell apart from us is a reason to say nothing.
[S3:records-sharing-one-trigger] BITES, and it is the r8 finding underneath the r8 finding. Both openai seats blocked on one defect from opposite sides — a same-title listing at a different hour moving a published start_time, and a same-title listing with NO time attaching an end_time — and both reduce to "a title is not an occurrence". The fix is a narrowing: a title-only match now writes nothing, so `start_time` is unwritable BY CONSTRUCTION and the founder's "update time" survives only as an end-time correction on a listing that agrees with us on title AND start minute. Recorded as R-099. But the register had already said this: R-094, R-095 and R-097 were opened across five rounds from three different proximate causes and every one resolves to the SAME trigger — a stable per-listing identifier. Three records converging on one unlock was legible after the second, and each round patched its local case instead of naming the missing primitive, so the panel found the fourth instance first. The counter-move is mechanical and starts here: R-099 names the cluster explicitly, and the class row says to grep the register for a trigger before opening a record.
[S3:destructive-normalization] BITES A THIRD TIME, on the same function in the same session, and the third finding is the one that explains the first two. openai/absence-only: `_fold_run` called every Mn mark optional, but the Devanagari VIRAMA and NUKTA are Mn and carry meaning, so `नुक्कड़` and `नुक्कड` — different words — collapsed to one identity and two same-minute listings differing by such a mark could write an end_time onto the wrong published event. The r6 and r7 fixes were better answers to the WRONG QUESTION: there is no single right amount of folding, because the reduction serves two callers whose dangerous answers point in opposite directions — the absence guard fails badly on a false NO (a page naming the event reads silent and a gated bracket then cancels a live row), identity fails badly on a false YES. Folding harder fixed one and worsened the other, which is why each fix produced the next round's finding. `_reduce` now takes `fold_marks`: the absence guard folds Mn/Me away, `normalize_title` keeps every mark. Pinned across Latin, Hebrew, Arabic and the two Devanagari mark classes, plus test_the_two_reductions_disagree_on_purpose, which asserts the split itself so nothing can quietly make them agree again.
[S3:retyped-evidence] The two openai NITS from this round are handled without being absorbed into the fix: the doc contradiction (a decision record claiming every mutation carries a candidate_evidence row, which is false of the cancellation that same record authorises) is CORRECTED in place with the correction stated rather than silently edited; the replay-log decided-vs-applied gap is RECORDED as R-100 rather than fixed, because Operating Law rule 8 scopes a REQUEST-CHANGES round to the blocking defect and the honest fix changes what the replay log means. Neither is dropped and neither widens this round.
[S3:partial-write-whole-row] Answered again at r8 and the answer got smaller: `_incoherent`'s first rule (a moved start with no stated end) is now UNREACHABLE through the adjudicator, because a diff can no longer carry a moved start at all. It is kept rather than deleted, as a guard so a future change to the match rules cannot silently reintroduce the `coalesce` hazard, and it is unit-tested directly since no fixture can reach it — stated in the test's own docstring rather than left as a branch nobody explains. The second rule (a pair that is not a window) stays reachable through the one writing match and keeps its adjudicator test.
...
STATUS: OPEN
WHAT: Incremental crawl. New `worker/crawl_state.py` (three queues — event-proximity / refresh / discover — plus next_due_at, body fingerprint, fail-streak, TickBudget, and the fail-closed verification verdict, all DERIVED from existing `raw_fetch` / `event_candidate` / `event` rows: no new table, column, or vendor); new `worker/spend_report.py` (measured $ from committed prices, summed from the `_usage` the provider already persists — the extraction surface is deliberately untouched); `worker/orchestrator.py` runs a budget-bounded TICK over 1-2 doors per source and skips extract on 304/fingerprint-match; `worker/run_once.py` plans the tick and prints the run table + outcomes; `.github/workflows/ingest.yml` armed-cron budgets; `tests/test_crawl_state.py` + `tests/test_fair_crawl.py` + five existing test files.
HOW: A tick takes what is DUE, most overdue first (round-robin only as the tie-break), and runs until wall-clock / model-budget / fetch-cap stops it — sources reached is an OUTCOME, not an input; `--max-sources` survives only as the founder's "spend/time safety cap". Published events with a start_time put their DEFINING page (`event_candidate.source_url` via `promoted_event_id`) on a T-30d/14d/7d/3d/1d/day-of ladder, one fetch per page however many events sit on it, stopping after end; dateless rows keep the source-door schedule. Conditional GET (ETag/Last-Modified) + body sha256 make 304/unchanged real, so extraction — the only stage that may call Anthropic — runs only on changed pages. 401/403 -> class D one knock; 429/503 -> back off; host politeness defers a source without ending the tick. Every re-check records verified_present / verified_absent / unverified, and UNVERIFIED is the default so a failure nobody has written yet fails closed.
WHY: two fat calendars consumed the whole 30-page follow budget in source order, and a daily re-crawl of everything would spend the budget on events three months out.
WHY-IT-MATTERS: Coverage Law makes a missing locale/category a defect, and a budget that starves all but two sources is that defect in scheduler form; a listing changed by a check that could not actually read the page is the same defect on the trust side.
DONE 2026-09-02: PR #213 green at f37fb3d — trust-gate, db-integration and the four-seat panel (openai attacker-smuggle + absence-only, gemini dataflow-taint + spec-vs-contract) all APPROVE, no blocking findings; the panel's three nits are R-091 and two of them are preconditions on the queued listing-update ticket. Armed-cron smoke re-bound to run 33685561230, which fired the skip-unchanged path live (2 fetches, 1 extract, $0.0986). Merge is the founder's.
EXPECTED OUTCOMES: many sources per tick x <=2 doors each, no category weighting anywhere; unchanged pages cost a conditional GET and zero AI calls; walls knock once; nothing published is mutated; table `source | queue | url fetched | changed? | verified? | candidates | skipped-unchanged | blocked` plus a measured fetches/extracts/$ outcomes line, from fixtures. Out of scope: Tonight redesign, catalog upsert, census rewrite, category weighting, new vendor, R-089 (trigger fired, founder-excluded in writing — noted in the RECORD row), vision-doc edits, and BUILDING the listing-update path — founder ratified its POLICY 2026-09-02 (confirmed check may update time/cancel/postpone/title on same-page evidence; never delete; ambiguous = keep), which is encoded in worker/crawl_state.py and recorded in docs/memory/decisions/2026-09-02_confirmed-check-may-update-listing.md; wiring it writes to the published `event` table and changes armed-cron runtime, which this ticket's smoke budget is spent on. day-of stays 6h under the founder's own escape clause — R-090.

**Appendix — construction_gate Stage 3 citations (Operating Law rule 9: minimum the gate will accept):**
[S3:grant-not-content-bound] [S3:permission-for-ratified-work] the smoke grant is treated as bound to its stated content, not as general: 'one more 2-source class-B smoke, same size as the last green run, no second wave' — the dispatch uses exactly max_sources=2, source_class=B, follow_pages_per_run=2, and no further run is taken whatever CI says. the listing-update policy is RATIFIED and therefore a build instruction, not a question — so it is QUEUED in TODOS.md as the next ticket and will be built without asking again. It is not built HERE because the same founder message caps this session's smoke budget ('No second wave after that') and wiring it changes armed-cron runtime; that is a sequencing fact, not a request for permission.
[S3:build-before-plan] [S3:condensed-thinking-run] [S3:stale-live-incident-state] [S3:swallowed-corrupt-data] the contract was written to STATE.md before any product edit (the plan-first hook enforced it) and amended in place when the founder corrected the design mid-session. each founder correction was re-derived against the code rather than patched onto the previous shape — two ordering bugs (event proximity, queue shares) were caught that way and are pinned by tests. no live-incident or arming-evidence field is claimed from memory; the armed-cron smoke evidence is refreshed by an actual run in this branch, not retyped. nothing is swallowed: a fingerprint-read failure is logged loudly and extracts anyway (coverage over cost), and every unreadable check becomes UNVERIFIED — never a silent 'fine'.
[S3:caller-suppliable-custody-inputs] [S3:green-on-stale-base] [S3:nonfinite-decimal-accepted] no custody input is caller-suppliable here — best_url is re-checked same-site against the REGISTERED start URL before it is used, and the class letter still comes from the catalog, not the fetch. base is origin/master tip 3d4d6a9 at drift 0, compared against ls-remote by this gate. no decimal/numeric parsing is added; the only integers are budget knobs parsed base-10 and fail-closed.
[S3:contract-scope-violation] [S3:status-narration-not-progress] [S3:stalled-state-needs-active-diagnosis] [S3:founder-verbatim-corrected] [S3:governance-ambiguity] [S3:excluded-surface-widening] Scope is the founder's six Must-dos and the Must-not list; nothing else was touched. One deliberate divergence, stated to the founder rather than made silently: K stays 30 (the 2026-08-04 founder freshness escalation) because fairness moved to the per-source door cap, so shrinking K would cost freshness and buy nothing.
[S3:heal-drops-guard-marker] [S3:false-confidence-gate] [S3:self-weakenable-gate] [S3:self-weakenable-review-model] [S3:final-gate-trusts-generator] [S3:rule-stronger-than-mechanism] [S3:untested-gate-branch] [S3:release-path-weaker-than-generation] [S3:fail-open-on-custody-misconfig] [S3:weak-key-accepted-at-custody] [S3:heal-drops-guard-marker] No gate, threshold, workflow gate, reviewer binding, credential or promote path changes. The orchestrator still imports no promote path (pinned by the existing structural test), and the two new budget knobs fail CLOSED through the same _resolve_budget the render cap uses.
[S3:fabricated-qualitative-copy] [S3:false-price-claim] [S3:copy-outruns-registry] [S3:semantic-claim-not-rederived] [S3:missing-cardinality-check] [S3:pagination-integrity-gap] [S3:volatile-safety-store] [S3:nonfinite-numeric-accepted] [S3:compounded-ground-contrast] [S3:featurability-dimension-missed] No user-facing copy, schema, migration, or new store: crawl state is DERIVED from raw_fetch + event_candidate rows the pipeline already writes. Both new queries are read-only and parameterized (a URL read out of fetched content is bound, never interpolated — pinned by a test).
[S3:stale-redclass-count] [S3:retyped-evidence] [S3:stale-base-widens-range] [S3:parallel-record-id-collision] [S3:deferred-trust-work] Base is origin/master tip 3d4d6a9 at drift 0, printed by this gate rather than retyped; the run tables in the PR body are machine-generated by render_run_table — one over a fixture wave, one the LIVE smoke run's own stdout (33685561230); deferral_scan is green, so no deferral is being left unrecorded.
[S3:pushed-on-red] [S3:env-dependent-hermetic-test] [S3:db-type-mismatch-invisible-to-hermetic-tests] [S3:scripted-transform-order] [S3:deliverable-visual-qa] [S3:workflow-tool-version-skew] [S3:mutable-model-alias] [S3:unusable-credential-tier] [S3:founder-path-unprobed] Full suite 2415 passed / 40 skipped on a full-history clone (the 6 earlier failures were the shallow-checkout fail-closed guards, cleared by git fetch --unshallow). New tests are hermetic: fetch, extract, candidate-store AND the crawl-state reads are faked, and the fakes accept exactly what the real adapter accepts (a fake that refused etag/last_modified was fixed, not worked around). The armed-cron edit changes two budget env values and comments — no tool, action, model id, or credential.
...
20 official_list, 5 marketplace, 1 publisher, 1 unknown, 0 junk; WebFetch
