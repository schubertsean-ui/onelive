...
STATUS: OPEN — built, pushed, PR #229 (draft). Panel r1-r4, each one layer further out on ONE class: a hole reaching a reader as the wrong hole. All FIXED. r1: a re-run compares the desk's STATEMENT, not only the key. r2: a superseded row is marked DISPUTED. r3: the three shapes of a NULL clock are separated before they collapse into one display (no date stated publishes; contested times publish DISPUTED; date-only is HELD). r4: a dispute write that FAILS now fails the RUN (exit 1, section 5 naming every live mislabelled event) instead of printing a success report over it. R-110 and R-111 carry the two residuals, both with objective triggers.
