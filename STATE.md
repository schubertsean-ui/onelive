...
## Where we are (updated 2026-07-23, session close: C2 convergence decision layer MERGED — #54 on master; prior 2026-07-22 STEP 5 ARMED; 2026-07-18 certification bootstrap)

DONE 2026-07-23 (the C2 arc — PR #54 MERGED as master 97fd957 under the agent-merges-on-green protocol): the convergence DECISION layer is landed, **shadow-only** (no gate/promote/auth/threshold touch; two-way recursive isolation sweeps enforce it). Three primitives on the merged C1 SL substrate (spec §5/§11): `worker/convergence/scenarios.py` (deterministic world sampling → three-mode classification with per-field attribution → aggregation), `worker/convergence/decisions.py` (`CostMatrix` with zero bundled defaults, `expected_loss`/`decide` returning full auditable arithmetic, `voi` gating spend on strictly-positive net), and the `docs/strategy/ONE_LIVE_COST_MATRIX_DRAFT_v1.md` PROPOSAL. The cost NUMBERS ship as an UNRATIFIED DRAFT — founder ratification is the gate to moving C2 from shadow to live (founder notified at merge with the ratification ask). The review converged over **18 evaluator rounds** (17 REQUEST-CHANGES + r18 APPROVE), each a real correctness defect on the audit-record self-verification path — ending at r17: DecisionRecord recomputes its choice from ground truth (mode_probs×matrix, exact stored-total anchor) so a near-tie cannot be stored as a tie to flip the recorded winner. APPROVE run 29989331325 (+ ready-for-review re-run 29989815033), Trust Gate 29989331277 green, mergeable_state clean, final head 91c26a5; arming bound to verification #18 (run 29988969830). Kaizen #54 row in the ledger (M1 count = 17 blocking rounds, the arc's forgery-resistance-converges-slowly lesson in the M2 watch). The 18-round count was flagged to the founder as extraordinary. **NEXT for C2:** founder ratifies the cost matrix → flip to non-shadow (founder-crucial, do not self-authorize).
