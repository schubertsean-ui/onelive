...
[S3:missing-record-read-as-state] Nothing absent is rendered as a state: the live walk is reported as NEVER RUN (R-084), the candidate column says extraction did not run rather than printing a zero, a page missing from the fixture manifest answers 404 and is reported as a miss, and the DB row counts stay unverified because no DSN is reachable here.
