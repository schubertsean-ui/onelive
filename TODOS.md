...
- [x] (P2) PR-aggregator venture go/no-go — DECIDED 2026-07-15: founder said "Go" (GREENLIT). R-013 trigger fired (letters drafted, live-page pass egress-blocked — see RECORD.md); build phase opened as Session Contract #8; sector decision still open below.
- [ ] (P1) Promise-ledger critical-path unblock — owner: founder — allowlist sec.gov + data.sec.gov + efts.sec.gov for this environment (or provide an unproxied runner): unblocks sprint steps 4–7 (real 8-K pulls → real golden set → extraction evals). Single biggest lever on the venture.
- [ ] (P1) Beachhead sector — owner: founder — memo at `ventures/promise_ledger/docs/BEACHHEAD_SECTOR_MEMO.md` (biotech = best product fit / highest verdict risk; graduated-confidence resolution suggested). On decision: sector po battery + friction attack per charter, then sprint step 9.
...
- [ ] (P2) Contract #5 closure path — owner: founder (1 line) — the pricing/licensing verification sub-scope could not run from this sandbox (egress policy 403s vendor domains). Pick one: (a) accept narrowed scope with R-013 as the completion gate at greenlight, or (b) allowlist the vendor domains listed in the report's "Source pages to re-verify" notes (or provide an unproxied run) and the verification pass finishes the scope as written.
