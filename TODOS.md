...
- [ ] (P2) De-escalation exam (cost-discipline rule 4, logged in friction entry #2): after PR #25 merges with the gate open, run `extraction-eval.yml` workflow_dispatch with `claude-haiku-4-5` against prompt v2026-07-15.3 — if it passes the SAME exam, route extraction back down to the cheap tier in a PR citing that run.
