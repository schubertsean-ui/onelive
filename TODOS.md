...
- [ ] (P1, Step 9) **Voice navigation — FOUNDER REQUIREMENT** (2026-07-15: "speaking (this must be a navigation option)"): every tappable action is voice-addressable; browser-native on-device speech recognition (Web Speech API — no new vendor, no audio leaves the device by default, zero spend); the accessibility labels double as the voice grammar; phrase map published in the direction-review doc — owner: Generator, Step 9 PR (acceptance: a mechanical test that every interactive control has a voice phrase mapping).
