...
6. **Proof or label — never an unverified claim stated as fact.** (Founder directive 2026-07-26, verbatim: "Never make a claim or statement that has not been independently verified and delivered with actual proof. This means you can never make assumptions." Record: `docs/memory/decisions/2026-07-26_proof-or-label.md`.) Every factual statement to the founder is one of exactly two things, and which one is always visible:
   ...
   MECHANISM, since a rule alone is `rule-stronger-than-mechanism`: the class `unverified-claim-as-fact` is in `docs/memory/RED_CLASSES.md` with claim-grammar trigger tokens, so `construction_gate` (which matches on diff CONTENT) blocks any change carrying that language until the Stage 3 citation names the verification for each. A blocking prose scanner was MEASURED as the alternative and rejected on the evidence — 36 of 55 claim-shaped lines would have fired, and a 65%-noise gate is one that gets weakened; the compensating control is the adversarial review, which has caught every instance of this class to date.
