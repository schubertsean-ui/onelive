...
## Fetching desks (founder, 2026-09-05)
Agents never fetch a desk from a sandbox. This build environment's egress proxy
403s the desks, so nothing an agent reads here is the desk's own page, and a
fixture may never stand in for one. The way an agent reads a desk is to DISPATCH
`.github/workflows/desk-ingest.yml` (manual dispatch, `write` defaults to false;
it also runs itself every 6 hours on master with `--real --write`), or to extend
the fetch worker that runs there. `tools/desk_ingest.py` stays the only write
seam — `create_candidate` -> `add_evidence` -> `promote_candidate`; do not add a
second publisher. An unreadable desk is a FAILED check with a report naming the
desk and why, never "0 events" and never a deletion. And a publish ticket is
UNDONE until the Action or worker log is the evidence: a passing test proves the
code, a green PR proves the review, and neither proves that a row reached the
catalog. Link the run.
