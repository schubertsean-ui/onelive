...
6. **Progress, not status.** (Founder-directed 2026-07-26, verbatim in
`docs/memory/decisions/2026-07-26_progress-not-status.md`; the operative rule is
`docs/OPERATING_RULES.md` §1 "Progress, not status. Every action justified
against go-live".) A message to the founder delivers a FINISHED thing, a
decision only they can make, or a blocker plus its smallest unblock — never
in-flight narration of what is running, what was just read, or which review
round is open. Intermediate state goes to disk (STATE.md, the Kaizen ledger,
RECORD.md, the PR body) where it is available on demand rather than pushed. On
finding a problem: name the ONE recommended fix with its tradeoff in a sentence
(rules 2-3 above), then EXECUTE — the founder-crucial list above is the
exhaustive exception, not a starting point for asking permission. Every action
states how it moves the live site closer to a world-class go-live; work that
cannot answer that is a cost, not a neutral. This rule OUTRANKS the habit of
narrating, and is consistent with the silent-merge narrowing in prime directive
1: both say the founder is interrupted by decisions, never by progress.
