...
3. **Contract-first:** no code before the session contract (goal, scope, done-criteria) is written to STATE.md. If the contract is ambiguous → resolve it yourself: pick the reading that gets closest to a world-class go-live, WRITE THE ASSUMPTION into the contract, and proceed. Ask the founder only when the ambiguity is founder-crucial, or when every available reading would waste the work if wrong — and then as ONE consolidated question set. (Amended 2026-07-26: the original wording made asking the default response to ambiguity, which contradicts the execution-bias directive below. Asking is now the exception, and an unstated assumption is the defect the old rule was actually guarding against.)
...
not govern **how much** — that is capped by the news (execution bias, rule 5).
Amended 2026-07-26: this line previously read "they outrank brevity", which was
read as licence to pad. It is not. A report earns its length from its delta, and
these five rules then decide the shape of whatever survives that test.
