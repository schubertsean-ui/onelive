...
burned the turn budget before any product file landed (`error_max_turns`, no branch).

**Do not run `session_reconcile.py`. Do not write a Session Contract. Do not read
`docs/SESSION_START.md`. Do not run hats, Kaizen, construction loop, friction logs,
or `[S3:]` citations.**

Open the Must-do files. Write them. Push a draft PR. Stop.

Trust gates stay: `trust_gate` (includes `tsc --noEmit` on web, no path filter),
lint, pytest, the independent evaluator, `web-compile` (`next build` on every PR),
Vercel. A red Vercel or web-compile or trust-gate check is a failed product.
Do not merge it. A lesson that is only a sentence has not landed.
Ceremony does not.
...
conflicts with ONE-LIVE-COVERAGE-LAW.md on *scope*, Coverage Law wins.
CAPCOG is the test locale and a view filter, not a catalog border.
...
## Locale Launch Law (founder, 2026-09-06)
Canonical copy: ONE-LIVE-LOCALE-LAUNCH.md (identical text).

If any instruction in this file, STATE.md, or a session contract conflicts with
ONE-LIVE-LOCALE-LAUNCH.md on *locale*, *gather-on-demand*, *place search*,
*location privacy*, or *anti-attention*, Locale Launch Law wins.
A locale is a query, not a market we launch. The catalog is the world; a view may
default to a place, it may not delete. Demand starts gather; a pack is cache, not
permission. Do not use IP as consent to locate a person. Do not send precise
person-location off device. Do not implement `precise_egress`.
Coverage Law still wins on *scope*. Operating Law still wins on *how you work*.
...
2. **Ticket first.** Do not run `session_reconcile.py`. Do not write a Session Contract. Do not update STATE.md / TODOS.md / the changelog unless the ticket names that file. Disk is truth for product files; ceremony files are not the ticket.
3. **No contract-first.** A pasted Must-do list IS the plan. Write the files it names.
4. **Validation:** `python tools/validate` must pass before any PR is opened. `--allow-skips` is temporary debt; log every skip. This is a trust gate, not ceremony.
...
## Thinking tools & Kaizen (added 2026-07-14; FROZEN to optional 2026-09-06 at founder direction)
Kaizen may MEASURE; it may not RUN the session. The whole thinking harness — the po battery (`docs/skills/po_provocation.md`, `tools/po_battery.py`), the six-hat registry (`docs/hats/`), the Friction pre-work log (`docs/FRICTION_LOG.md`), the Construction Loop (`docs/skills/construction_loop.md`), the Kaizen ledger (`docs/KAIZEN.md`, `docs/metrics/KAIZEN_LEDGER.md`), and the session arcs — stays in the tree and stays available to any agent who wants it, but it is OPTIONAL: it is never a ticket, it never gates a commit, and a product PR may land with ZERO KAIZEN_LEDGER, RECORD, or session-arc edits. Vision (`ONE-LIVE-VISION.md`) and Operating Law decide what work happens; ceremony does not outrank the ticket, and if a test demands the ceremony files, the test is what changes. Mechanically, so this paragraph claims nothing the tree lacks: `kaizen_trends` is ADVISORY, and `construction_gate` now fires ONLY on diffs that touch the verification machinery it exists to protect (`tools/validate`, `trust_gate`, `adversarial_review`, `docs/memory/RED_CLASSES.md`) — every other diff gets a printed OUT-OF-SCOPE result and owes no `[S3:]` citation. NOT frozen, and unchanged by this: the user-facing trust gates (`tools/trust_gate.py`, `tools/lint.py`, `tools/deferral_scan.py`, full pytest) stay BLOCKING, the independent non-Claude review still runs on every PR with no path filter, and using any thinking tool to argue a gate down is still a gate-threshold relaxation — founder-crucial.

<!-- Retained verbatim for reference; OPTIONAL per the paragraph above — none of the four is required to commit, and nothing below is invoked by a gate.
...
Every stage is independently auditable. Publication is gate-custodied: the AI extraction step never publishes directly — everything passes through the gate, and promotion is human-custodied or earned-confidence auto behind founder-flipped fail-closed flags.
...
- Claude API — used for weak-signal extraction from raw fetched text (and gated descriptor generation); its output reaches users only through the validation gates — auto-publish exists ONLY as the ratified earned-confidence machinery behind founder-flipped fail-closed flags, never ungated
...
**Do not run `docs/SESSION_START.md`.** Ticket first (founder 2026-09-07).
Look at the Must-do files, not the session bookends.
