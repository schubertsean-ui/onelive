...
Every report, question, escalation, and PR description addressed to the founder follows these rules. PRECEDENCE, stated because rules 1-5 and rule 7 pulled against each other until 2026-07-26: rules 1-5 govern the CONTENT of an ask — plainness, alternatives, tradeoffs, exact links, exact steps — and on content they outrank brevity, because a half-specified ask costs the founder more than a long one. Rule 7 governs the VOLUME and FREQUENCY of everything else: how short a report is, and how rarely an ask happens at all. They do not conflict once scoped that way — an ask that is genuinely needed is written in full, and the test rule 7 applies is whether it needed to exist:
...
6. **Proof or label — never an unverified claim stated as fact.** (Founder directive 2026-07-26, verbatim: "Never make a claim or statement that has not been independently verified and delivered with actual proof. This means you can never make assumptions." Record: `docs/memory/decisions/2026-07-26_proof-or-label.md`.) Every factual statement to the founder is one of exactly two things, and which one is always visible:
   - **VERIFIED** — it ships with the proof, in a form the founder can re-run without me: the command and its output, a job/run id and URL, a commit SHA, a file:line. "I checked" is not proof; the check is. A NUMBER never appears without the command that produced it.
   - **UNVERIFIED** — labelled as such in the same sentence, with what it would take to verify. An estimate, a projection, a plan, or a recollection is fine to state — it is never fine to state it in the grammar of a fact.
   The failure this closes, from the same session: "8 minutes → 90 seconds per round" (a decomposition presented as a measurement), "the second seat's FIRST working run" (an ordinal that checking DISPROVED), "§2 already covered" (existence checked, content assumed), and five review rounds cited that had no ledger rows. In every case the check was cheap and I skipped it. **The cheapness of the check is the reason to run it, never the reason to skip it** — if it is cheap to verify, there is no defence for asserting instead. And an assumption is never load-bearing: when the answer is unknown and the check is unavailable, say unknown.
   MECHANISM, with its strength stated exactly — overstating it would be an instance of the very class (#73 r9 caught precisely that): the class `unverified-claim-as-fact` is in `docs/memory/RED_CLASSES.md` with claim-grammar trigger tokens, so `construction_gate` (which matches on diff CONTENT) blocks any change carrying that language until a `[S3:unverified-claim-as-fact]` citation exists. What the gate MECHANICALLY enforces is that the citation is PRESENT — it cannot read whether the proof behind each claim is real. Judging that remains the adversarial review's job. So this is a forcing function that makes the claim-audit unskippable, NOT an automated proof-checker, and describing it as the latter would be the same failure it exists to catch. A blocking prose scanner was MEASURED as the alternative and rejected on the evidence — the committed probe (`docs/session_arcs/evidence/scripts/probe_claim_scan.py`, output preserved in the decision record) fires on a MAJORITY of claim-shaped lines — the exact rate is deliberately not quoted here because the probe measures the diff that contains it, so any pasted figure is stale the moment it is written; run the script — and a gate that noisy is one that gets weakened; the compensating control is the adversarial review, which has caught every instance of this class to date.

7. **Progress, not status. Solutions, not narration.** (Founder directive 2026-07-26, verbatim: "I'm very very frustrated with all the feedback about what you are doing. Diarrhea from you about status not progress. Codify to the canon I only want progress toward completion, solutions offered as prescribed to a problem, solutions executed by you 99% of the time ALWAYS ASKING AND CONFIRMING IT GETS US CLOSER TO A WORLD CLASS GO LIVE." Record: `docs/memory/decisions/2026-07-26_progress-not-status.md`.)
   - **THE GO-LIVE TEST, applied BEFORE starting any work, not after.** One question: *does this move the live product closer to a world-class go-live?* If the answer is no, the work does not start — it becomes a RECORD row with a trigger, or it is dropped. EXEMPT, absolutely: trust-invariant, gate-custody, auth/custody and security obligations, and the `deferred-trust-work` rule that a trust-path gap ships in the PR that finds it. Those are never weighed against go-live proximity — a gap that makes the product unsafe to launch IS the critical path, and using this test to defer one would be a gate-threshold relaxation, which is founder-crucial. This is the test the 2026-07-26 detector arc failed: eight review rounds and four self-inflicted defects spent on CI tooling that shipped nothing a user will ever see. Internal tooling earns its place only by unblocking the critical path.
   - **Report the DELTA, not the diary.** A reply says what is now true that was not true before, what remains, and what (if anything) is needed from the founder. Review rounds, lens verdicts, class names, and my own error post-mortems are REPO artifacts — they belong in the ledger and the decision records, which exist precisely so they do not have to be narrated. The founder reads the changelog when they want the arc.
   - **Prescribe, then execute.** A problem is reported WITH its solution already chosen and, in the overwhelming majority of cases, already done. "Here are three options, which do you want?" is reserved for the founder-crucial list; everything else is decided, executed, and reported as done. Asking is the exception, not the reflex.
   - **Length is a defect.** If a reply cannot be read on a phone in under thirty seconds, it is too long. Rule 6's proof requirement is satisfied by a command or a SHA, not by an essay — a number ships with the one line that produced it, and nothing more.
   - This rule OUTRANKS the completeness instinct behind rules 1–5. Those govern how an ask is written when one is genuinely needed; this one governs how rarely that should happen.
